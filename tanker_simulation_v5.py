...
# or WAITING_MOTHER_CAPACITY) because no mother/roving storage can receive them,
# a mid-day nomination fires once per calendar day at 12:00. The largest-capacity
# waiting shuttle is designated as a temporary storage vessel. The smallest
# waiting shuttle discharges into it and is freed to return and reload. The
# designated transient shuttle holds that cargo and is forced to discharge to the
# mother the following day as its first priority.
# Rules enforced by _maybe_run_multiple_transient_op():
#   - Fires once per calendar day (noon tick); never more than once per day.
#   - Only one transient shuttle per day.
#   - Transient = largest-capacity vessel in the WAITING_BERTH_B queue.
#   - Discharger = smallest-capacity vessel in the WAITING_BERTH_B queue.
#   - Skipped if ANY primary mother or SanJulian currently has space AND is at BIA.
#   - Set at runtime by run_sim() from the app toggle; never edit here directly.
MULTIPLE_TRANSIENT_OPERATION = False
...
class Simulation:
    def __init__(self):
        ...
        for vv in self.vessels:
            if vv.name == "Bedford":
                ...
                # Balham starts berthing GreenEagle at BIA, full load (loaded from Ibom)
    ...
    def _sj_transload_trigger_check(self, t):
        ...
        # T6 — Post-export-return priority drain: mother returned from export
        # with large free space (≥ 60% capacity) AND can hold full SJ volume.
        for mn in self._sj_primary_mothers():
            if not self.mother_is_at_point_b(mn, t):
                continue
            if self.export_state.get(mn) == "DOC":
    ...
    def _maybe_run_multiple_transient_op(self, t):
        """Fire once per day at noon when MTO is enabled and conditions are met.

        Conditions (all must be true):
          1. MULTIPLE_TRANSIENT_OPERATION global is True.
          2. Wall-clock is at or past 12:00 on the current day and this
             calendar day has not yet fired an MTO nomination.
          3. At least 2 daughter vessels are simultaneously waiting at Point B
             (status WAITING_BERTH_B or WAITING_MOTHER_CAPACITY).
          4. No primary mother vessel (Bryanston/Alkebulan/GreenEagle) AND no
             SanJulian currently has capacity to receive even the smallest
             waiting vessel's cargo.  This confirms a genuine blockage rather
             than a queue behind a busy berth.

        Action:
          - Transient vessel  = largest cargo_capacity among the waiters.
          - Discharger vessel = smallest cargo_capacity among the remaining waiters.
          - The discharger's cargo is credited to the transient vessel (vessel-to-
            vessel transfer at the standard discharge rate).  The discharger's
            status is then advanced to CAST_OFF_B so it can return and reload on
            the same day.
          - The transient vessel is flagged (_mto_force_discharge_day) so that on
            the NEXT calendar day the WAITING_BERTH_B handler gives it absolute
            priority over any berth as soon as a primary mother is available,
            effectively forcing it to the front of the queue on day+1.
          - Only one MTO per calendar day and one transient sacrifice per day.
        """
        if not MULTIPLE_TRANSIENT_OPERATION:
            return

        # ── Gate 1: noon tick, once per day ──────────────────────────────────
        wall_hour = (t + SIM_HOUR_OFFSET) % 24
        day_key   = int((t + SIM_HOUR_OFFSET) // 24)
        # Fire at the first half-step at or after 12:00 wall-clock
        if wall_hour < 12.0 or day_key in self._mto_days_fired:
            return

        # ── Gate 2: collect waiting vessels at Point B ────────────────────────
        _waiting_statuses = {"WAITING_BERTH_B", "WAITING_MOTHER_CAPACITY"}
        waiters = [
            vv for vv in self.vessels
            if vv.status in _waiting_statuses
            and vv.cargo_bbl > 0
            and vv.name not in {"Bedford", "Balham"}   # Ibom vessels excluded
        ]
        if len(waiters) < 2:
            return

        # ── Gate 3: confirm that vessels will be genuinely idle ───────────────
        # MTO fires when EITHER:
        #   (a) No primary mother / SanJulian has any space at all — hard blockage
        #   (b) Mothers have space but all berths are occupied with operations
        #       that will complete AFTER today's daylight window ends (18:00),
        #       meaning the waiting vessels cannot berth today regardless.
        _min_waiter_cargo = min(vv.cargo_bbl for vv in waiters)
        _primary_mothers = [mn for mn in MOTHER_NAMES if mn != MOTHER_QUATERNARY_NAME]
        _all_mothers_to_check = list(MOTHER_NAMES)   # includes SanJulian

        # Check (a): hard blockage — no mother has space
        receiver_available = False
        for mn in _all_mothers_to_check:
            if not self.mother_is_at_point_b(mn, t):
                continue
            space = self.mother_capacity_bbl(mn) - self.mother_bbl[mn]
            if space >= _min_waiter_cargo:
                receiver_available = True
                break

        # Check (b): mother has space but berth won't free before daylight ends
        ...
        # Skip MTO if operations can proceed normally today
        if receiver_available and berth_free_before_daylight:
            return

        # ── All gates passed: nominate transient and discharger ──────────────
        self._mto_days_fired.add(day_key)

        # Transient = largest capacity; if tie, pick the one that arrived later
        # (i.e. has been waiting less) so the earlier-arrived vessel gets freed.
        waiters_by_cap_desc = sorted(
            waiters,
            key=lambda vv: (vv.cargo_capacity, -(vv.arrival_at_b or 0)),
            reverse=True,
        )
        transient_v = waiters_by_cap_desc[0]

        # Discharger = smallest cargo on board (not just capacity) — pick the
        # vessel with the least cargo so the transfer finishes fastest and it
        # can return soonest.
        remaining_waiters = [vv for vv in waiters if vv is not transient_v]
        discharger_v = min(remaining_waiters, key=lambda vv: vv.cargo_bbl)

        # ── Execute vessel-to-vessel transfer ─────────────────────────────────
        # Rate: reuse DISCHARGE_HOURS constant (standard pump rate at BIA).
        transfer_bbl    = discharger_v.cargo_bbl
        ...
        # ── Flag the transient for forced next-day priority discharge ─────────
        # We store the day_key on which MTO fired so the WAITING_BERTH_B handler
        # can recognise it and give absolute berth priority from day_key+1 onward.
        transient_v._mto_transient_since_day = day_key

        # ── Advance discharger to CAST_OFF_B so it can return and reload ──────
        # Skip the formal BERTHING/HOSE/DISCHARGING states — the transfer is
        # vessel-to-vessel at anchor; cast-off is the only remaining step.
        ...
        # The discharger now has zero cargo; assigned_mother stays for logging
        # continuity and will be cleared in the normal WAITING_RETURN_STOCK flow.

        # ── Transient keeps WAITING_BERTH_B — it will be prioritised tomorrow ─
        ...
        self.log_event(
            ...
            f"[MTO Day {day_key+1}] Nominated as temporary storage — "
            f"receiving {transfer_bbl:,.0f} bbl @ {_dis_api:.2f}° API from "
            f"{discharger_v.name} | new on-board total: {transient_v.cargo_bbl:,.0f} bbl | "
            f"will discharge to mother with priority from Day {day_key+2}",
            voyage_num=transient_v.current_voyage,
        )
        self.log_event(
            t, discharger_v.name, "MTO_DISCHARGE_TO_TRANSIENT",
            f"[MTO Day {day_key+1}] Discharging {transfer_bbl:,.0f} bbl "
            f"to transient storage {transient_v.name} ({transfer_hours:.1f}h pump) | "
            f"freed to return and reload | cast-off at "
        ...
        if transfer_hours > 0:
            self.log_event(
                ...
                f"Vessel-to-vessel transfer complete | "
                f"{transient_v.name}: {transient_v.cargo_bbl:,.0f} bbl on board",
                voyage_num=discharger_v.current_voyage,
            )
        ...
        while t <= total_hours:
            ...
            for v in _ordered_vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    # ── MTO transient vessel: force to front of queue ─────────
                    # If this vessel was nominated as a transient storage on the
                    # previous day, give it absolute berth priority on day_key+1
                    # onward as soon as any primary mother has capacity.
                    ...
                    if _mto_since is not None:
                        ...
                        if _cur_day_key > _mto_since:
                            # Find any available primary mother (not SanJulian)
                            ...
                            for _mn in MOTHER_NAMES:
                                ...
                                _slot = self.next_berthing_window(
                                    max(decision_t, self.mother_berth_free_at[_mn],
                                        self.mother_available_at[_mn]),
                                    point="B",
                                )
                            ...
                            if _mto_best_mother is not None:
                                # Displace any incumbent and claim the berth
                                ...
                                self.mother_berth_free_at[_mto_best_mother] = max(
                                    self.mother_berth_free_at[_mto_best_mother], _discharge_end_mto
                                )
                                v.status = "BERTHING_B"
                                v.next_event_time = _mto_best_start + BERTHING_DELAY_HOURS
                                v._mto_transient_since_day = None   # clear flag after dispatch
                                self.log_event(
                                    decision_t, v.name, "MTO_TRANSIENT_PRIORITY_BERTH",
                                    f"[MTO] Transient vessel taking priority berth at "
                                    f"{_mto_best_mother} (Day {_cur_day_key+1}) — "
                                    f"{v.cargo_bbl:,.0f} bbl on board",
                                    voyage_num=v.current_voyage, mother=_mto_best_mother,
                                )
                                continue
                            else:
                                # No primary mother available yet — keep waiting,
                                # but log that this is an MTO-priority vessel
                                ...
                                self.log_event(
                                    ...
                                    f"[MTO] Transient vessel holding — no primary mother with space; "
                                    f"reassessing at {self.hours_to_dt(_next_mto).strftime('%Y-%m-%d %H:%M')}",
                                    voyage_num=v.current_voyage,
                                )
                                continue
