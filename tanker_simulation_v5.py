...
class Simulation:
    ...
    def next_berthing_window(self, current_hour, point=None):
        ...
        wall-clock berthing window [BERTHING_START, BERTHING_END).

        Cast-off conflicts are only checked when scheduling an IMMEDIATE berth
        (current_hour is within 1h of now). For future scheduling, cast-offs
        will have completed long before the vessel arrives — applying the check
        to far-future times caused catastrophic berth-lock stacking when
        multiple vessels queued simultaneously (each call could add 14 days).
        """
        wall_h = (current_hour + SIM_HOUR_OFFSET) % 24
        # Only enforce cast-off conflict for near-immediate berthing
        _near_future = current_hour <= getattr(self, '_current_t', current_hour) + 1.0
        _castoff_blocks = _near_future and self.is_any_vessel_casting_off(point)
        if BERTHING_START <= wall_h < BERTHING_END and not _castoff_blocks:
            return current_hour
        ...
        # Only step forward for cast-off conflicts on immediate berths
        if _near_future:
            for _ in range(14):
