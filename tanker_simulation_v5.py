...
MOTHER_NAMES = [MOTHER_PRIMARY_NAME, MOTHER_TERTIARY_NAME]
POINT_B_RECEIVER_NAMES = [SANJULIAN_NAME]
SANJULIAN_CAPACITY_BBL = 450_000
SANJULIAN_TRANSFER_RATE_BPH = 11_000
POINT_B_DIRECT_TO_SANJULIAN = True
...
class Simulation:
    def __init__(self):
        ...
        self.sanjulian_transfer_target = None
        self.sanjulian_transfer_end_time = None
        self.sanjulian_transfer_volume = 0.0
        self.sanjulian_transfer_reason = None
        ...
        for vv in self.vessels:
            if vv.name == "Bedford":
                ...
                # Balham starts berthing SanJulian at BIA, full load (loaded from Ibom)
                vv.status           = "BERTHING_B"
                vv.target_point     = "B"
                vv.cargo_bbl        = 85_000
                vv.assigned_mother  = SANJULIAN_NAME
                ...
                # Bagshot starts hose-connected at SanJulian, ready to commence discharge
                ...
                self.log_event(0, vv.name, "HOSE_CONNECTION_START_B",
                               f"Hose connected at {SANJULIAN_NAME} — ready to commence discharge "
                ...
                # Watson starts hose-connected at SanJulian, ready to commence discharge
        ...
        if POINT_B_DISTRIBUTION_TEST_MODE:
            for vv in self.vessels:
                if vv.name not in POINT_B_TEST_STARTUP_FULL_LOAD_NOMINATIONS:
                    continue
                nominated_mother = SANJULIAN_NAME
        ...
        # Reserve Point B berth timelines for any t=0 vessels already active on SanJulian.
        ...
        for vv in self.vessels:
            ...
            if vv.status == "BERTHING_B":
                ...
                continue
            self.sanjulian_berth_free_at = max(self.sanjulian_berth_free_at, _end)
    ...
    def total_mother_bbl(self):
        return sum(self.mother_bbl.values()) + self.sanjulian_bbl

    def point_b_asset_bbl(self, asset_name):
        if asset_name == SANJULIAN_NAME:
            return float(self.sanjulian_bbl)
        return float(self.mother_bbl.get(asset_name, 0.0))

    def point_b_asset_api(self, asset_name):
        if asset_name == SANJULIAN_NAME:
            return float(self.sanjulian_api)
        return float(self.mother_api.get(asset_name, 0.0))

    def reserve_point_b_receiver_until(self, receiver_name, end_time):
        if receiver_name == SANJULIAN_NAME:
            self.sanjulian_berth_free_at = max(self.sanjulian_berth_free_at, end_time)
            return
        self.mother_berth_free_at[receiver_name] = max(
            self.mother_berth_free_at.get(receiver_name, 0.0),
            end_time,
        )
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        candidates = []
        receiver_name = SANJULIAN_NAME
        if self.sanjulian_bbl + v.cargo_bbl <= SANJULIAN_CAPACITY_BBL:
            berth_t = self.next_berthing_window(berthing_start, point="B")
            start = max(berth_t, self.sanjulian_berth_free_at)
            start = self.next_berthing_window(start, point="B")
            candidates.append((start, berth_t, receiver_name))
        return berthing_start, candidates

    def is_point_b_receiver_available(self, receiver_name, t):
        if receiver_name != SANJULIAN_NAME:
            return False
        return t >= self.sanjulian_berth_free_at - 1e-9

    def same_day_daughter_arrival_pending(self, t):
        current_day = self.hours_to_dt(t).date()
        outbound_statuses = {
            "SAILING_AB", "SAILING_CROSS_BW_AC", "SAILING_BW_TO_FWY", "SAILING_AB_LEG2",
            "SAILING_D_CHANNEL", "SAILING_CH_TO_BW_OUT", "SAILING_CROSS_BW_OUT", "SAILING_BW_TO_B",
            "WAITING_FAIRWAY", "WAITING_DAYLIGHT", "WAITING_TIDAL"
        }
        for vv in self.vessels:
            if vv.target_point != "B":
                continue
            if vv.status not in outbound_statuses:
                continue
            if self.hours_to_dt(vv.next_event_time).date() == current_day:
                return True
        return False

    def select_sanjulian_transfer(self, t):
        if self.sanjulian_bbl <= 0:
            return None
        if self.sanjulian_transfer_target is not None:
            return None
        pending_same_day_arrival = self.same_day_daughter_arrival_pending(t)
        daughter_active_at_b = any(
            vv.assigned_mother == SANJULIAN_NAME and vv.status in {"BERTHING_B", "HOSE_CONNECT_B", "DISCHARGING"}
            for vv in self.vessels
        )
        if daughter_active_at_b:
            return None
        ...
        for mother_name in MOTHER_NAMES:
            if not self.mother_is_at_point_b(mother_name, t):
                continue
            if t < self.mother_berth_free_at[mother_name] - 1e-9:
                continue
            space = max(0.0, MOTHER_CAPACITY_BBL - self.mother_bbl[mother_name])
            if space <= 0:
                continue
            need_for_export = max(0.0, MOTHER_EXPORT_VOLUME - self.mother_bbl[mother_name])
            full_condition = self.sanjulian_bbl >= SANJULIAN_CAPACITY_BBL - 1e-6
            enough_to_complete = need_for_export > 0 and self.sanjulian_bbl >= min(space, need_for_export)
            idle_condition = not pending_same_day_arrival
            optimize_condition = self.sanjulian_bbl >= max(MIN_INCOMING_TRANSFER_BBL, 0.4 * SANJULIAN_CAPACITY_BBL)
            if not (full_condition or enough_to_complete or idle_condition or optimize_condition):
                continue
            transfer_vol = min(self.sanjulian_bbl, space)
            reason = []
            if enough_to_complete:
                transfer_vol = min(transfer_vol, need_for_export)
                reason.append("complete export target")
            if full_condition:
                reason.append("SanJulian full")
            if idle_condition:
                reason.append("mother idle with no same-day daughter arrival")
            if optimize_condition:
                reason.append("operational optimization")
            if transfer_vol > 0:
                candidates.append((
                    0 if enough_to_complete else 1,
                    self.mother_bbl[mother_name],
                    mother_name,
                    transfer_vol,
                    ", ".join(reason) or "operational optimization",
                ))

        if not candidates:
            return None
        candidates.sort(key=lambda x: (x[0], x[1], x[2]))
        _, _, mother_name, transfer_vol, reason = candidates[0]
        return mother_name, transfer_vol, reason

    def maybe_run_sanjulian_transfer(self, t):
        if self.sanjulian_transfer_target is None:
            selected = self.select_sanjulian_transfer(t)
            if not selected:
                return
            mother_name, transfer_vol, reason = selected
            duration = transfer_vol / SANJULIAN_TRANSFER_RATE_BPH if SANJULIAN_TRANSFER_RATE_BPH > 0 else 0.0
            self.sanjulian_transfer_target = mother_name
            self.sanjulian_transfer_volume = transfer_vol
            self.sanjulian_transfer_reason = reason
            self.sanjulian_transfer_end_time = t + duration
            self.sanjulian_berth_free_at = max(self.sanjulian_berth_free_at, t + duration)
            self.mother_berth_free_at[mother_name] = max(self.mother_berth_free_at[mother_name], t + duration)
            self.log_event(
                t,
                SANJULIAN_NAME,
                "SANJULIAN_TRANSFER_START",
                f"Transloading {transfer_vol:,.0f} bbl to {mother_name} @ {SANJULIAN_TRANSFER_RATE_BPH:,.0f} bph | reason: {reason}",
                mother=mother_name,
            )
            return

        if t < self.sanjulian_transfer_end_time - 1e-9:
            return

        mother_name = self.sanjulian_transfer_target
        transfer_vol = min(
            self.sanjulian_transfer_volume,
            self.sanjulian_bbl,
            max(0.0, MOTHER_CAPACITY_BBL - self.mother_bbl[mother_name]),
        )
        if transfer_vol > 0:
            self.mother_api[mother_name] = self.blend_api(
                self.mother_bbl[mother_name], self.mother_api.get(mother_name, 0.0),
                transfer_vol, self.sanjulian_api,
            )
            self.mother_bbl[mother_name] += transfer_vol
            self.sanjulian_bbl -= transfer_vol
            if self.sanjulian_bbl <= 0:
                self.sanjulian_bbl = 0.0
                self.sanjulian_api = 0.0
            self.export_ready[mother_name] = True
            if self.export_ready_since[mother_name] is None:
                self.export_ready_since[mother_name] = t
        self.log_event(
            t,
            SANJULIAN_NAME,
            "SANJULIAN_TRANSFER_COMPLETE",
            f"Transferred {transfer_vol:,.0f} bbl to {mother_name} | SanJulian remaining: {self.sanjulian_bbl:,.0f} bbl",
            mother=mother_name,
        )
        self.sanjulian_transfer_target = None
        self.sanjulian_transfer_end_time = None
    ...
    def projected_mother_stock(self, mother_name, horizon, exclude_vessel=None):
        ...
        projected = self.point_b_asset_bbl(mother_name)
        for vv in self.vessels:
    ...
    def log_event(self, t, vessel_name, event, detail="", voyage_num=None, mother=None):
        ...
        self.log.append({
            ...
            "Alkebulan_bbl": round(self.sanjulian_bbl),
        ...
        while t <= total_hours:
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    if not candidates:
                        mother_levels = f"{SANJULIAN_NAME}: {self.sanjulian_bbl:,.0f}/{SANJULIAN_CAPACITY_BBL:,} bbl"
                        ...
                        self.log_event(arrival, v.name, "WAITING_MOTHER_CAPACITY",
                                       f"No berth/capacity slot currently available on Point B assets ({mother_levels}); "
                        ...
                        if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0 and not POINT_B_DIRECT_TO_SANJULIAN:
                        ...
                        if start > arrival + 0.01:
                            ...
                            self.reserve_point_b_receiver_until(selected_mother, _discharge_end)
                            v.next_event_time = start + BERTHING_DELAY_HOURS
                        ...
                        if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0 and not POINT_B_DIRECT_TO_SANJULIAN:
                            ...
                            self.log_event(
                                ...
                                v.name,
                                "MOTHER_PRIORITY_ASSIGNMENT",
                                f"Day {day_key + 1} Point B assignment: "
                    ...
                    start, berth_t, selected_mother = selected
                    if selected_mother != v.assigned_mother and v.assigned_mother == SANJULIAN_NAME:
                        self.log_event(
                            decision_t,
                            v.name,
                            "MOTHER_PRIORITY_ASSIGNMENT",
                            f"Hourly Point B reassessment reallocated receiver {v.assigned_mother} -> {selected_mother}",
                            voyage_num=v.current_voyage,
                            mother=selected_mother,
                        )
                    v.assigned_mother = selected_mother
                    ...
                    if v.assigned_mother != SANJULIAN_NAME:
                        self.log_event(t, v.name, "WAITING_MOTHER_CAPACITY",
                                       "Blocked: no explicit SanJulian assignment at Point B",
                                       voyage_num=v.current_voyage)
                        v.next_event_time = t + 0.5
                        continue
                    if not self.is_point_b_receiver_available(v.assigned_mother, t):
                        ...
                        self.log_event(
                            ...
                            f"{v.assigned_mother} berth unavailable at Point B; reassessing at {self.hours_to_dt(_next_chk).strftime('%Y-%m-%d %H:%M')}",
                            voyage_num=v.current_voyage,
                    ...
                    if v.assigned_mother != SANJULIAN_NAME:
                        ...
                        continue
                    selected_mother = v.assigned_mother
                    if not self.is_point_b_receiver_available(selected_mother, t):
                        ...
                        self.log_event(
                            ...
                            v.name,
                            "WAITING_MOTHER_RETURN",
                            f"{selected_mother} berth unavailable at Point B; reassessing at {self.hours_to_dt(next_recheck).strftime('%Y-%m-%d %H:%M')}",
                            voyage_num=v.current_voyage,
                            mother=selected_mother,
                        )
                        continue
                    if self.sanjulian_bbl + v.cargo_bbl > SANJULIAN_CAPACITY_BBL:
                        ...
                        # Blend vessel cargo API into SanJulian floating storage.
                        ...
                        self.sanjulian_berth_free_at = max(self.sanjulian_berth_free_at, t + DISCHARGE_HOURS)
                        ...
                        self.log_event(t, v.name, "DISCHARGE_START",
                                       ...
                                       f"{selected_mother}: {self.sanjulian_bbl:,.0f} bbl "
                    ...
                    self.log_event(t, v.name, "CAST_OFF_COMPLETE_B",
                                   "Cast-off from SanJulian complete; returning to storage",
                                   voyage_num=v.current_voyage)
            ...
            self.maybe_run_sanjulian_transfer(t)
            ...
            self.timeline.append({
                ...
                "SanJulian_api": round(self.sanjulian_api, 2),
                "Alkebulan_api": round(self.sanjulian_api, 2),
...
print(f"    - {SANJULIAN_NAME:<9}: {sim.sanjulian_bbl:,.0f} bbl")
...
ax2.plot(timeline_df["Time"], timeline_df["SanJulian_bbl"],
            color="#c0392b", linewidth=1.4, alpha=0.9, label=f"{SANJULIAN_NAME} Volume")
...
ax2.set_title(
    f"Point B Assets ({MOTHER_PRIMARY_NAME} + {SANJULIAN_NAME} + {MOTHER_TERTIARY_NAME}) — Volume Level",
    fontsize=11,
)
