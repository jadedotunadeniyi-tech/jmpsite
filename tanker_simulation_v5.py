...
WESTMORE_PERMITTED_VESSELS = {"Sherlock", "Laphroaig", "Bagshot", "Rathbone", "Watson", "Woodstock"}
...
class Simulation:
    ...
    def return_allocation_candidate(self, cargo_bbl, vessel_name, point_restrict=None):
        ...
        if not eligible:
            ...
            fallback = max(
                ...
                key=lambda name: (
                    self.storage_bbl[name],
                    -abs(self.storage_bbl[name] - STORAGE_CRITICAL_THRESHOLD_BY_NAME[name]),
                    name,
                ),
            )
    ...
    def projected_storage_stock(self, storage_name, horizon):
        """Estimate storage stock at a future sim-hour by accumulating production
        and subtracting committed loading operations that complete within the horizon.
        Used by the spread-logic forecast to avoid dispatching a vessel to a
        Duke/Starturn berth that won't actually need help by the time it arrives."""
        rate     = self.effective_production_rate(storage_name)
        stock    = float(self.storage_bbl[storage_name])
        capacity = STORAGE_CAPACITY_BY_NAME[storage_name]
        dt       = max(0.0, horizon)
        # Production accumulates up to tank capacity
        projected = min(stock + rate * dt, capacity)
        # Subtract any loading committed at this storage that completes before horizon
        for vv in self.vessels:
            if vv.assigned_storage != storage_name:
                continue
            if vv.status not in {"BERTHING_A", "HOSE_CONNECT_A", "LOADING", "WAITING_DEAD_STOCK"}:
                continue
            cap = self.effective_load_cap(vv.name, storage_name)
            if vv.next_event_time <= horizon + 1e-6:
                projected -= cap
        return max(projected, 0.0)

    def vessel_eta_to_storage_point(self, v, storage_name, t):
        """Conservative lower-bound estimate of hours until vessel v could
        realistically *start berthing* at storage_name from its current position.
        This is intentionally optimistic (ignores tidal gates) to avoid
        over-blocking the spread — only catches the obvious cross-route cases
        where A/C vessels cannot teleport to D/E within the same cycle.

        Returns 0 if the vessel is already at (or en-route to) the target point."""
        target_point = STORAGE_POINT.get(storage_name, "A")
        current_point = v.target_point  # where the vessel currently is / heading

        # Same-area: vessel can reach the storage without a cross-route transit.
        # A↔C are both above the breakwater, so they count as same-area.
        ac_group = {"A", "C"}
        if current_point == target_point:
            return 0.0
        if current_point in ac_group and target_point in ac_group:
            return 0.0

        # Cross-route: vessel at A/C needs a full round-trip to reach D or E
        # (or vice-versa).  Approximate the minimum transit as the B→D/E return
        # leg — the vessel must at minimum complete a BIA discharge and then sail
        # back via the appropriate route.  We use fixed leg times rather than
        # simulating the full multi-leg schedule; daylight/tidal waits are omitted
        # so we remain an optimistic lower bound.
        if current_point in ac_group and target_point == "D":
            # A/C → B ≈ 7h sailing + 0.5h BW cross + B→BW (1.5h) + BW→CH (1h) + CH→D (3h)
            return SAIL_HOURS_A_TO_B + SAIL_HOURS_B_TO_BW + SAIL_HOURS_BW_TO_CH_IN + SAIL_HOURS_CH_TO_D
        if current_point in ac_group and target_point == "E":
            # A/C → B ≈ 7h + direct B→E ≈ 3h
            return SAIL_HOURS_A_TO_B + 3.0
        if current_point == "D" and target_point in ac_group:
            # D → B ≈ 6h + B→A ≈ 8h
            return (SAIL_HOURS_D_TO_CH + SAIL_HOURS_CH_TO_BW_OUT + SAIL_HOURS_CROSS_BW + SAIL_HOURS_BW_TO_B) + SAIL_HOURS_B_TO_A
        if current_point == "E" and target_point in ac_group:
            # E → B ≈ 3h + B→A ≈ 8h
            return 3.0 + SAIL_HOURS_B_TO_A
        if current_point == "D" and target_point == "E":
            # D → B → E roughly (same BIA discharge, different return)
            return (SAIL_HOURS_D_TO_CH + SAIL_HOURS_CH_TO_BW_OUT + SAIL_HOURS_CROSS_BW + SAIL_HOURS_BW_TO_B) + 3.0
        if current_point == "E" and target_point == "D":
            return 3.0 + SAIL_HOURS_B_TO_BW + SAIL_HOURS_BW_TO_CH_IN + SAIL_HOURS_CH_TO_D
        return 0.0
    ...
    def choose_hourly_storage_option(self, v, t):
        ...
        Prioritises storages closest to/above critical (borderline or unsafe),
        and spreads queued vessels to Duke/Starturn when permitted."""
        if v.name == self.point_f_active_loader or v.target_point == "F":
            return None

        candidates = [
            s for s in STORAGE_NAMES
            if s in STORAGE_NAMES and self.storage_allowed_for_vessel(s, v.name)
        ...
        # ── Spread logic: proactively route a vessel to Duke/Starturn when queuing ──
        #
        # POSITION-AWARE FORECASTING:
        # A vessel currently at Point A/C cannot physically appear at D/E within
        # the same cycle — the routes are entirely separate.  Before spreading a
        # vessel we:
        #   1. Estimate the vessel's ETA to the spread storage's point.
        #   2. Project that storage's stock at ETA (production accumulation minus
        #      committed loads).
        #   3. Only spread if the storage will genuinely need evacuation by the time
        #      the vessel arrives (unsafe OR within a forecast proximity band).
        #      If not, the vessel waits at its current point for back-to-back loading
        #      there, which is almost always more efficient for low-rate storages.
        #
        # THRESHOLD:  spread fires when projected stock at ETA is within
        # SPREAD_FORECAST_BAND bbls of critical (or already unsafe).
        # This is tighter than the production-bias proximity band so that
        # Duke/Starturn are not pre-loaded just because they happen to be slightly
        # elevated — only a real forecast need triggers cross-point dispatch.
        _SPREAD_FORECAST_BAND = 30_000   # bbls above critical at ETA to trigger spread
        ...
        if len(waiting_pool) >= 2:
            ...
            def spread_risk_key(storage_name):
                stock = self.storage_bbl[storage_name]
                crit = STORAGE_CRITICAL_THRESHOLD_BY_NAME[storage_name]
                unsafe = 0 if stock >= crit else 1
                return (unsafe, abs(stock - crit), -stock, storage_name)

            spread_storages = sorted(spread_storages, key=spread_risk_key)
            for spread_storage in spread_storages:
                # Skip if another vessel is already committed here
                active_or_reserved = any(
                    vv.assigned_storage == spread_storage
                    and vv.name != v.name
                    and vv.status in {"WAITING_BERTH_A", "BERTHING_A", "HOSE_CONNECT_A", "LOADING"}
                    for vv in self.vessels
                )
                if active_or_reserved:
                    continue

                # Position check: how long before THIS vessel can realistically
                # arrive at the spread storage's point?
                eta_hours = self.vessel_eta_to_storage_point(v, spread_storage, t)

                # Forecast the spread storage stock at that ETA
                proj_stock = self.projected_storage_stock(spread_storage, t + eta_hours)
                crit       = STORAGE_CRITICAL_THRESHOLD_BY_NAME[spread_storage]
                proj_gap   = proj_stock - crit   # positive = still above critical at ETA

                # Only spread if the storage will genuinely need help by arrival:
                # unsafe already, or projected to be within the forecast band.
                # If the storage will still be healthy at ETA, let this vessel
                # stay at its current point for back-to-back loading there.
                spread_needed = (proj_stock < crit) or (proj_gap <= _SPREAD_FORECAST_BAND)
                if not spread_needed:
                    continue

                # Among permitted vessels, pick the one best suited to this spread
                # storage (smallest position-penalty first, then load capacity, then name)
                queued = sorted(
                    [vv for vv in waiting_pool if self.storage_allowed_for_vessel(spread_storage, vv.name)],
                    key=lambda x: (
                        self.vessel_eta_to_storage_point(x, spread_storage, t),
                        -self.effective_load_cap(x.name, spread_storage),
                        x.name,
                    ),
                )
        ...
        # Rank by risk urgency: unsafe first, then closest to critical threshold.
        # A small production-rate bias is applied when a storage is unsafe or within
        # a proximity band of critical: high-production storages (Chapel/JasmineS/Westmore)
        # get their effective distance-to-critical tightened slightly, nudging them ahead
        # of Duke/Starturn at similar risk levels.  The bias is intentionally small so it
        # only tips the decision when storages are closely matched — it never overrides a
        # genuinely more urgent low-production storage.
        _max_rate  = max(STORAGE_PRODUCTION_RATE_BY_NAME.values())   # normalise to [0,1]
        _PROX_BAND  = 80_000   # bbls above critical where bias is active
        _BIAS_SCALE = 0.12     # max fractional gap reduction for highest-rate storage (12%)
        ...
        def risk_rank(storage_name):
            ...
            # Apply bias only when unsafe OR within proximity band of critical
            if unsafe or raw_gap <= _PROX_BAND:
                rate_norm    = STORAGE_PRODUCTION_RATE_BY_NAME.get(storage_name, _max_rate) / _max_rate
                effective_gap = raw_gap * (1.0 - _BIAS_SCALE * rate_norm)
            else:
        ...
        # Prefer options that can berth now; otherwise keep best risk-priority option.
