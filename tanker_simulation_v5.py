...
class Simulation:
    ...
    def choose_hourly_storage_option(self, v, t):
        """Choose best proactive storage option (Chapel/JasmineS/Westmore)
        for waiting/idle A/C daughters during hourly reassessment."""
        if v.name == self.point_f_active_loader or v.target_point == "F":
            return None

        preferred = [STORAGE_PRIMARY_NAME, STORAGE_SECONDARY_NAME, STORAGE_TERTIARY_NAME]
        candidates = [
        ...
        # Rank Chapel/Jasmine by current stock (highest first); keep Westmore as
        # explicit third-spread option for fleet utilization when 3+ vessels queue.
        aj = [s for s in candidates if s in (STORAGE_PRIMARY_NAME, STORAGE_SECONDARY_NAME)]
        aj_sorted = sorted(aj, key=lambda s: (-self.storage_bbl[s], s))
        ordered = aj_sorted + ([STORAGE_TERTIARY_NAME] if STORAGE_TERTIARY_NAME in candidates else [])

        waiting_pool = [
            vv for vv in self.vessels
            if vv.status in {"WAITING_BERTH_A", "IDLE_A", "WAITING_STOCK"}
            and vv.target_point in ("A", "C")
            and vv.name != self.point_f_active_loader
        ]

        # When 3+ daughters are queued, reserve one for Westmore (if permitted)
        # to keep all berths utilised instead of over-concentrating on Point A.
        if len(waiting_pool) >= 3 and STORAGE_TERTIARY_NAME in candidates:
            westmore_assigned = any(
                vv.assigned_storage == STORAGE_TERTIARY_NAME
                and vv.status in {"WAITING_BERTH_A", "BERTHING_A", "HOSE_CONNECT_A", "LOADING"}
                for vv in self.vessels
            )
            if not westmore_assigned:
                # Prefer smallest-capacity queued vessel for Westmore spread.
                queued = sorted(
                    [vv for vv in waiting_pool if self.storage_allowed_for_vessel(STORAGE_TERTIARY_NAME, vv.name)],
                    key=lambda x: (self.effective_load_cap(x.name, STORAGE_TERTIARY_NAME), x.name),
                )
                if queued and queued[0].name == v.name:
                    return STORAGE_TERTIARY_NAME

        # Prefer options that can berth now; else keep best stock-priority option.
        def rank(storage_name):
            ...
            return (0 if berth_now else 1, ord_idx, -self.storage_bbl[storage_name], storage_name)
                ...
                if v.status == "IDLE_A":
                    ...
                    if candidate_storages:
                        ...
                        self.log_event(t, v.name, "WAITING_STOCK",
                                       ...
                                       f"({storage_levels}; threshold guide {min_threshold:,.0f} bbl) — waiting",
                                       voyage_num=v.current_voyage)
