...
PGM_LOAD_RATE_BPH         = 440   # PGM (Point G) — pump rate; SantaMonica loads 80% less by volume
...
def storage_adjusted_load_cap(base_cap, storage_name, vessel_name=None):
    ...
    PGM (Point G) loads 80% below the vessel's normal capacity (SantaMonica only).
    ...
    if storage_name == STORAGE_SECONDARY_NAME:
        ...
        cap = int(round(base_cap * 0.20))   # 80% less than normal capacity (SantaMonica @ PGM)
...
class Simulation:
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        # mothers (Bryanston or GreenEagle) — never to SanJulian.
        ...
        for mother_name in MOTHER_NAMES:
            ...
            if _vessel_is_mto and mother_name == MOTHER_QUATERNARY_NAME:
                continue
            # Ibom-crude vessels (Bedford/Balham): SanJulian is never a valid target
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            for v in _ordered_vessels:
                ...
                # ── Mid-sim dormancy trigger (status-independent) ─────────────
                # Must fire BEFORE any status handler so the vessel is stopped
                # regardless of what it is currently doing (IDLE_A, LOADING,
                # SAILING, DISCHARGING, etc.).
                ...
                if _dorm_h is not None and t >= _dorm_h:
                    ...
                    v.dormancy_start_hour    = None   # one-shot
                    v.resumption_hold_logged = False
                    # Drop the vessel into IDLE_A so the hold fires below.
                    # Cargo is discarded — the vessel will be empty on resumption
                    # (it will load from resumption_storage at priority on wakeup).
                    v.status               = "IDLE_A"
                    v.cargo_bbl            = 0
