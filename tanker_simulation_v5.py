...
TIDE_ALLOWED_CROSSING_LEVELS_M = {round(1.7 + 0.1 * i, 1) for i in range(9)}
TIDE_MIN_CROSSING_M = min(TIDE_ALLOWED_CROSSING_LEVELS_M)
TIDE_MAX_CROSSING_M = max(TIDE_ALLOWED_CROSSING_LEVELS_M)
...
class Simulation:
    ...
    def tide_ok_at(self, hour):
        """True if tidal height is one of the allowed crossing levels (or no table loaded)."""
        if _TIDE_TABLE is None:
            return True
        h = self.tide_height_at(hour)
        return h is not None and round(h, 1) in TIDE_ALLOWED_CROSSING_LEVELS_M
    ...
    def tidal_period_labels(self, hour):
        wall_h = (hour + SIM_HOUR_OFFSET) % 24
        labels = []
        # Morning: 06:00-23:00 (user-defined), Afternoon: 12:00-16:00,
        # Evening: 16:00-18:00. Overlaps are intentionally allowed.
        if 6 <= wall_h < 23:
            labels.append("morning tide")
        if 12 <= wall_h < 16:
            labels.append("afternoon tide")
        if 16 <= wall_h < 18:
            labels.append("evening tide")
        return labels

    def tidal_period_label(self, hour):
        """Backward-compatible string label for existing log messages."""
        labels = self.tidal_period_labels(hour)
        return " / ".join(labels) if labels else "outside tidal window"
    ...
    def tidal_periods_available_for_day(self, hour):
        ...
        for slot in [day_start + 0.5 * i for i in range(48)]:
            ...
            if self.tide_ok_at(slot):
                for lbl in self.tidal_period_labels(slot):
                    if lbl not in labels:
                        labels.append(lbl)
        return " / ".join(labels) if labels else "no valid crossing period"
    ...
    def next_tidal_sail(self, current_hour):
        ...
        Return the earliest hour >= current_hour that satisfies BOTH:
          - daylight (DAYLIGHT_START <= (h+SIM_HOUR_OFFSET)%24 < DAYLIGHT_END)
                    - tide height in allowed set (1.7m to 2.5m in 0.1m steps) (skipped if no table)
