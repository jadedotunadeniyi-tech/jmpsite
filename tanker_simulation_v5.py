...
class Simulation:
    ...
    def _maybe_run_multiple_transient_op(self, t):
        ...
        Nomination (once per day at noon):
          When ≥2 shuttles are stuck at Point B and they cannot berth today
          (hard blockage or all berths occupied past daylight end), the largest-
          capacity-headroom waiting vessel is nominated as transient storage.
          The smallest waiting vessel transfers its cargo into it and is freed
          to return and reload.

        Transient capacity (ceiling, not target):
          The MTO_TRANSIENT_CAPACITY_BBL table sets the maximum the transient
          vessel may hold in total.  Transfer is clamped to available headroom
          so the cap is never exceeded.  The transient discharges as soon as
          any mother berth window opens — it does NOT wait to be "full".

        Multi-parcel accumulation (MTO_MAX_PARCELS_BEFORE_OFFLOAD):
          Controls how many additional shuttle cargoes can top up the transient
          on subsequent days while it is still waiting for a mother berth.
          Once the parcel count reaches the max, no further top-ups are accepted
          that day (the transient's opportunistic offload logic still runs
          independently every hourly tick via the WAITING_BERTH_B handler).
        """
        ...
        # ── Gate 3: vessels will be genuinely idle today ──────────────────────
        # MTO fires when: no mother has space (hard blockage) OR all berths
        # won't free before daylight end (18:00) — either way, the shuttles
        # cannot discharge today without MTO.
        _min_waiter_cargo = min(vv.cargo_bbl for vv in waiters)
        _all_mothers_to_check = list(MOTHER_NAMES)

        receiver_available = False
        for mn in _all_mothers_to_check:
            if not self.mother_is_at_point_b(mn, t):
                continue
            if self.mother_capacity_bbl(mn) - self.mother_bbl[mn] >= _min_waiter_cargo:
        ...
        if receiver_available:
            for mn in _all_mothers_to_check:
                ...
                if self.mother_capacity_bbl(mn) - self.mother_bbl[mn] < _min_waiter_cargo:
                    continue
                if self.mother_berth_free_at.get(mn, 0.0) <= _daylight_end_t:
        ...
        if existing_transient is not None:
            ...
            if _parcels_so_far >= MTO_MAX_PARCELS_BEFORE_OFFLOAD or _headroom <= 0:
                # Parcel limit or cap reached — no more top-ups today.
                # The WAITING_BERTH_B handler continues seeking an offload window.
                self.log_event(
                    t, existing_transient.name, "MTO_PARCEL_LIMIT_REACHED",
                    f"[MTO Day {day_key+1}] No further top-ups: "
                    f"{'parcel limit' if _parcels_so_far >= MTO_MAX_PARCELS_BEFORE_OFFLOAD else 'at capacity'} "
                    f"| on-board: {existing_transient.cargo_bbl:,.0f}/{_trn_cap:,.0f} bbl | "
            ...
            # ── Nominate a new transient vessel ───────────────────────────────
            # Pick the vessel with the most remaining headroom in the MTO cap
            # (i.e. largest_cap - current_cargo), so we choose the best
            # "container" available — not just the largest ship by name.
            def _headroom_score(vv):
                cap = MTO_TRANSIENT_CAPACITY_BBL.get(vv.name, vv.cargo_capacity)
                return max(0.0, cap - vv.cargo_bbl)

            waiters_scored = sorted(waiters, key=_headroom_score, reverse=True)
            ...
            if _headroom <= 0:
                return   # even the best candidate is already at cap

            remaining    = [vv for vv in waiters if vv is not transient_v]
            discharger_v = min(remaining, key=lambda vv: vv.cargo_bbl)
            transfer_bbl = min(discharger_v.cargo_bbl, _headroom)

            # Stamp nomination: transient flag drives priority in WAITING_BERTH_B
        ...
        # Discharger empties completely and returns to load
        ...
        # Transient: stays WAITING_BERTH_B and will opportunistically try every
        # hourly tick to find a mother berth via the priority handler above.
        ...
        _cap_label  = MTO_TRANSIENT_CAPACITY_BBL.get(
                          transient_v.name, transient_v.cargo_capacity)
        _remaining_headroom = max(0.0, _cap_label - transient_v.cargo_bbl)
        self.log_event(
            ...
            f"(cap {_cap_label:,.0f} bbl, {_remaining_headroom:,.0f} bbl headroom remaining) | "
            f"will discharge opportunistically at next mother berth window",
        ...
        self.log_event(
            ...
            f"freed to return and reload | cast-off "
        ...
        if transfer_hours > 0:
            self.log_event(
                ...
                f"Transfer complete | {transient_v.name}: {transient_v.cargo_bbl:,.0f} bbl",
                voyage_num=discharger_v.current_voyage,
            )
            ...
            for v in _ordered_vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    if _mto_since is not None:
                        # Find the earliest available primary mother (not SanJulian)
                        ...
                        if _mto_best_mother is not None:
                            ...
                            # No mother available yet — keep waiting with priority flag set
                            ...
                            self.log_event(
                                ...
                                f"[MTO] Transient holding — no primary mother with sufficient space; "
