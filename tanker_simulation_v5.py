...
DEAD_STOCK_FACTOR         = 1.00   # vessel loads as soon as stock ≥ 100% of cargo capacity
...
class DaughterVessel:
    def __init__(self, name, start_offset_hours=0, cargo_capacity=None):
        ...
        # to stamp VoyageCode with an "A" suffix (e.g. BNR-000A) so the
...
class Simulation:
    def __init__(self):
        ...
        _RETURNING_LEG1 = {
            ...
            "Bagshot",  "Watson",    "Berners",  "Woodstock",
        }
        ...
        for vv in self.vessels:
            if vv.name == "Bedford":
                ...
                # All 8 vessels are returning from BIA empty — they departed at
                # different times over the previous day so they arrive staggered.
                # Spread arrivals over ~22 hours (0h to 22h) in dispatch-priority
                # order so the first vessel to arrive gets first loading slot.
                # SAIL_HOURS_B_TO_A = 6h; we offset next_event_time so the vessel
                # fires SAILING_BA → IDLE_A at the right wall-clock time.
                _sorted_leg1 = sorted(_RETURNING_LEG1)   # alphabetical for determinism
                _idx = _sorted_leg1.index(vv.name)
                _n   = len(_sorted_leg1)
                # Spread: vessel 0 arrives at t=0 (08:00), vessel n-1 at t=22h
                _arrival_offset = _idx * (22.0 / max(_n - 1, 1))
                vv.status           = "SAILING_BA"
                vv.target_point     = "A"
                vv.cargo_bbl        = 0
                vv.next_event_time  = _arrival_offset   # arrives at loading point at this t
                vv._voyage_assigned = False
                vv.current_voyage   = 0
    ...
    def storage_locked_by_active_berth(self, storage_name, requesting_vessel=None):
        ...
        and finished documentation. CAST_OFF is excluded — a vessel casting off is
        physically departing, so the next vessel may begin its approach."""
        lock_statuses = {
            ...
            # CAST_OFF excluded: vessel is physically departing, berth is clearing
        }
        for vv in self.vessels:
    ...
    def mother_export_departure_eligible(self, mother_name):
        """Export may depart when the trigger is reached.

        Rules (in priority order):
          1. MUST sail: mother is physically full (no room for smallest daughter)
          2. MUST sail: been export_ready for ≥ EXPORT_SERIES_BUFFER_HOURS * 0.5
             (prevents indefinite deferral by inbound traffic)
          3. DEFER: ≥ EXPORT_DEFER_INBOUND_THRESHOLD daughters inbound AND
             mother has room for at least 2 more daughter cargoes (meaningful
             headroom). Avoids sailing with a half-full hull when traffic is high.
          4. SAIL: trigger reached, low daughter pressure
        """
        stock = self.mother_bbl[mother_name]
        cap   = self.mother_capacity_bbl(mother_name)
        remaining_capacity = max(0.0, cap - stock)

        # Rule 1 — physically full
        cannot_accommodate_next = remaining_capacity < MIN_INCOMING_TRANSFER_BBL
        if cannot_accommodate_next:
            return True

        # Must have reached export trigger
        reached_target = stock >= self.mother_export_trigger_bbl(mother_name)
        if not reached_target:
            return False

        # Rule 2 — safety valve: sail after waiting too long (prevents deadlock)
        ...
        # Rule 3 — defer only when daughters are inbound AND
        # mother has meaningful remaining space (≥ 2 daughter cargoes worth)
        _n_inbound = self._daughters_inbound_to_bia()
        two_cargo_space = MIN_INCOMING_TRANSFER_BBL * 2
        if (_n_inbound >= EXPORT_DEFER_INBOUND_THRESHOLD
                and remaining_capacity >= two_cargo_space):
            return False

        # Rule 4 — normal departure
        return True
    ...
    def _maybe_run_multiple_transient_op(self, t):
        ...
        # ── Gate 1: 08:00 or first tick >=12:00, once per calendar day ───────
        ...
        if not (_is_morning or _is_noon_or_later):
            return
        if day_key in self._mto_days_fired:
            return

        # ── Gate 2: >=2 shuttle vessels stranded at Point B ──────────────────
        ...
        if len(waiters) < 2:
            return
        # At 08:00 only fire if >=2 are already confirmed idle (hard-wait)
        _hard_waiters = [vv for vv in waiters if vv.status in _hard_wait]
        if _is_morning and not _is_noon_or_later and len(_hard_waiters) < 2:
            return

        # ── Gate 3: vessels genuinely cannot berth today ──────────────────────
        # "Cannot berth" means: no mother has space, OR all mothers with space
        # have their berths occupied past daylight end (18:00).
        # KEY FIX: if SanJulian is actively transloading to a primary mother
        # (BERTHING_B / HOSE_CONNECT_B / DISCHARGING at ~10k bph), that
        # mother's berth is blocked until sanjulian_next_t — which can be
        # 40+ hours away.  Use sanjulian_next_t as the effective free-time.
        ...
        if receiver_available:
            for mn in MOTHER_NAMES:
                ...
                # Override with SanJulian end-time if she is discharging here
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            for v in _ordered_vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    if candidate_storages:
                        ...
                        # the threshold is confirmed in HOSE_CONNECT_A.
                        v.status = "BERTHING_A"
                        start = t
                        self.storage_berth_free_at[selected_storage] = (
                            start + BERTHING_DELAY_HOURS + HOSE_CONNECTION_HOURS + load_hours
                        )
                        # Each storage has its own independent berth —
                        # Chapel and JasmineS can load simultaneously.
                        # Only update the serial gap for this specific storage.
                    ...
                    self.log_event(t, v.name, "LOADING_COMPLETE",
                                   f"Cargo: {v.cargo_bbl:,} bbl | Begin 1h documentation",
                                   voyage_num=v.current_voyage)
                    self.log_event(t, v.name, "DOCUMENTATION_START",
                                   "1 hour allocated for paperwork",
                                   voyage_num=v.current_voyage)
                    ...
                    if _mto_since is not None:
                        ...
                        if _mto_best_mother is not None:
                            ...
                            # No mother available yet — recheck at every half-step
                            # (TIME_STEP_HOURS) during daylight so the transient
                            # claims any berth the moment it frees up.
                    ...
                    # Clamp to available space — MTO transient vessels may carry
                    # more cargo than the mother has room for; discharge whatever
                    # fits and carry the residual forward.
                    _actual_discharge = min(v.cargo_bbl, _mother_space)
                    if _actual_discharge <= 0:
                        self.log_event(t, v.name, "WAITING_MOTHER_CAPACITY",
                        ...
                        # (e.g. BNR-000 → BNR-000A) so JMP can distinguish
                    ...
                    # If vessel still has residual cargo (partial discharge case —
                    # e.g. MTO transient that could only offload part of its volume),
                    # return to WAITING_BERTH_B with the MTO priority flag intact so
                    # it will immediately seek the next available berth.
                    if v.cargo_bbl > 0:
                        v.status = "WAITING_BERTH_B"
                        v.next_event_time = self.next_daylight_hourly_berth_check(t, point="B")
                        self.log_event(t, v.name, "WAITING_BERTH_B",
                                       f"Partial discharge complete — {v.cargo_bbl:,.0f} bbl residual; "
                                       f"seeking next available mother berth",
                                       voyage_num=v.current_voyage)
                        continue
            ...
            if active_export_mother is None and t >= self.next_export_allowed_at:
                ...
                # Mark export_ready for any primary mother whose stock is at or
                # above her export trigger — even if no CAST_OFF_B event set it.
                # This handles startup seeding (mothers pre-loaded above trigger)
                # and SanJulian transload top-ups that bypass the normal path.
                for _mn in MOTHER_NAMES:
                    if _mn == MOTHER_QUATERNARY_NAME:
                        continue
                    if (self.export_state[_mn] is None
                            and not self.export_ready[_mn]
                            and self.mother_is_at_point_b(_mn, t)
                            and self.mother_bbl[_mn] >= self.mother_export_trigger_bbl(_mn)):
                        self.export_ready[_mn] = True
                        if not self.export_ready_since.get(_mn):
            ...
            for mother_name in MOTHER_NAMES:
                ...
                if state == "DOC":
                    if t >= self.export_end_time[mother_name]:
                        ...
                        # Compute how long we have been waiting post-DOC
                        _doc_wait = t - (self.export_end_time[mother_name] - TIME_STEP_HOURS)
                        _max_doc_wait = 24.0   # never wait more than 24h for daughters
                        if daughter_active_here and _doc_wait < _max_doc_wait:
                            # Hold briefly while daughter is mid-discharge
