    ...
    def __init__(self):
        ...
        for vv in self.vessels:
            if vv.name == "Bedford":
                ...
                # Bagshot starts queued at Point B with cargo for SanJulian.
                # Balham holds the live berth first; Bagshot joins the discharge queue.
                ...
                self.log_event(0, vv.name, "WAITING_BERTH_B",
                               f"Startup queue at {SANJULIAN_NAME} — awaiting serial discharge berth "
                               f"({_bg_cap:,} bbl on board)",
                               voyage_num=vv.current_voyage,
                               mother=SANJULIAN_NAME)
            elif vv.name == "Watson" and vv.name not in _seeded_startups:
                # Watson starts queued at Point B with cargo for SanJulian.
                # Balham holds the live berth first; Watson joins the discharge queue.
        ...
        while t <= total_hours:
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    selected_mother = v.assigned_mother if v.assigned_mother in (MOTHER_NAMES + [SANJULIAN_NAME]) else "UNASSIGNED"
