...
class Simulation:
    ...
    def maybe_run_daily_preops_storage_reassessment(self, t):
        ...
        if day_key < 1:
            return
        if abs(wall_hour - 5.0) > 0.26:   # tolerance: catches any 0.5h step that straddles 05:00
            return
    ...
    def run(self):
        ...
        # Use strictly-less-than so the final recorded Day == SIMULATION_DAYS.
        # The inclusive bound (<=) produced an extra Day N+1 snapshot with no
        # events, making the last day appear blank on long runs.
        _loop_end = total_hours - TIME_STEP_HOURS + 1e-9
        t = 0.0
