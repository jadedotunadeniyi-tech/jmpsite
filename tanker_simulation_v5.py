...
PGM_LOAD_RATE_BPH         = 440   # PGM (Point G) — pump rate; SantaMonica loads 75% less by volume (~7k)
...
def storage_adjusted_load_cap(base_cap, storage_name, vessel_name=None):
    ...
    PGM (Point G) loads 75% below the vessel's normal capacity (SantaMonica only).
...
class Simulation:
    ...
    def _maybe_run_multiple_transient_op(self, t):
        ...
        if existing_transient is not None:
            ...
            if not _fits:
                # No discharger's full cargo fits in the transient's remaining headroom.
                # Abort rather than force a partial transfer that is operationally
                # misleading (e.g. Sherlock 85k clamped to 9k on a nearly-full transient).
                # The parcel-limit log already signals the operator that no top-up is possible.
                return
            discharger_v = min(_fits, key=lambda vv: vv.cargo_bbl)
            transfer_bbl = min(discharger_v.cargo_bbl, _headroom)
