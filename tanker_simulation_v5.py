...
MTSANBARTH_CAPACITY_BBL = 450_000  # MTSanBarth (intermediate floating storage)
...
PGM_LOAD_RATE_BPH         = 440   # PGM (Point G) — pump rate; SantaMonica loads ~7k bbl (25% of 28k capacity)
...
# Mother defaults: Bryanston 450k, GreenEagle 300k, MTSanBarth 450k — set by run_sim()
...
WESTMORE_LOAD_CAP_MULTIPLIER  = 0.90   # Point C: vessels load 10% below normal capacity
...
# ── Internal: MTSanBarth optimisation parameters ───────────────────────────────
# MTSanBarth sits permanently at BIA. It receives daughter discharges like a
# primary mother but never sails to the export terminal. Instead it transloads
# to primary mothers via MTSANBARTH_TRANSLOAD_RATE_BPH. Transloads trigger when:
#   T1  MTSanBarth reaches MTSANBARTH_CAPACITY_BBL (force-drain)
#   T2  MTSanBarth >= a primary mother's remaining export-volume requirement
#   T3  A primary mother is idle at BIA and no daughter is arriving same day
#   T4  MTSanBarth > MTSANBARTH_OPTIM_THRESHOLD_FRAC AND mother has headroom
MTSANBARTH_OPTIM_THRESHOLD_FRAC  = 0.25    # T4: trigger above 25% of capacity
...
# Dynamic MTSanBarth daughter accumulation: when ≥ this many daughters are
# inbound/waiting at BIA, raise mtsanbarth_daughters_min_threshold by 1 per
# extra daughter above the base so MTSanBarth loads more before draining.
...
#  Each row: (Name, Capacity bbl).  MTSanBarth capacity set in Section E above.
...
MOTHER_QUATERNARY_NAME = "MTSanBarth"   # intermediate — does not export
...
# MTSanBarth uses MTSANBARTH_CAPACITY_BBL (450,000) defined in Section E above.
...
def storage_adjusted_load_cap(base_cap, storage_name, vessel_name=None):
    ...
    Westmore (Point C) loads 10% below the vessel's normal capacity.
    ...
    if storage_name == STORAGE_SECONDARY_NAME:
        ...
        cap = int(round(base_cap * 0.25))   # 75% less than normal capacity (SantaMonica @ PGM) → ~7k
...
MOTHER_CAPACITY_BY_NAME = {
    ...
    MOTHER_QUATERNARY_NAME: MTSANBARTH_CAPACITY_BBL,    # MTSanBarth  — Section E
}
...
STATUS_CODES = {
    ...
    # MTSanBarth transload events — MTSanBarth behaves like a daughter vessel
    # when pumping to a primary mother (BERTHING_B → HOSE_CONNECT_B →
    # DISCHARGING → CAST_OFF_B), with matching log events on both sides.
    "SJ_TRANSLOAD_START"    : "MTSanBarth: starting transload cycle to primary mother",
    "SJ_TRANSLOAD_COMPLETE" : "MTSanBarth: transload cycle to primary mother complete",
    "SJ_TRANSLOAD_ABORT"    : "MTSanBarth: transload aborted — target left Point B",
    "SJ_FENDER_PREP"        : "MTSanBarth: fender preparation in progress",    # Mid-simulation mother unavailability window
...
class ThirdPartyVessel:
    ...
    Discharges to the earliest available PRIMARY mother (never MTSanBarth).
    Priority rules:
...
class Simulation:
    def __init__(self):
        ...
        # MTSanBarth NEVER joins the export_state / export_ready / export_sail
        ...
        #
        # mtsanbarth_status : None | "BERTHING_B" | "HOSE_CONNECT_B" |
        #                    "DISCHARGING" | "CAST_OFF_B"
        # mtsanbarth_target : name of the primary mother being loaded
        # mtsanbarth_amount : bbl committed to this transload cycle
        # mtsanbarth_next_t : sim-hour when the current phase completes
        self.mtsanbarth_status           = None
        self.mtsanbarth_target           = None
        self.mtsanbarth_amount           = 0.0
        self.mtsanbarth_next_t           = None
        # mtsanbarth_fender_ready_t: sim-hour when fender preparation completes.
        # Set to t + SANJULIAN_FENDER_PREP_HOURS after every cast-off from a
        # primary mother.  Until this time: daughters may not berth MTSanBarth,
        ...
        # reaches mtsanbarth_daughters_min_threshold.
        # Resets to 0 after each completed transload cycle.
        # T1 (near-capacity force-drain) bypasses this gate to prevent overflow.
        self.mtsanbarth_daughters_loaded        = 0
        # mtsanbarth_daughters_min_threshold: dynamically adjusted after startup
        # seeding.  Any vessels already in BERTHING_B/HOSE_CONNECT_B/DISCHARGING
        # at MTSanBarth from t=0 complete early and are counted by the normal
        ...
        self.mtsanbarth_daughters_min_threshold = 2
        # Legacy aliases kept for backward compat with app summary reads
        self.mtsanbarth_transload_state  = None   # set to dict when active
        self.mtsanbarth_transload_end_t  = None
        self.mtsanbarth_total_transloaded = 0.0
        # Remove MTSanBarth from export machinery — it will never export
        ...
        # ── Adjust MTSanBarth daughter-minimum threshold for pre-seeded vessels ──
        # Count vessels already in BERTHING_B / HOSE_CONNECT_B / DISCHARGING at
        # MTSanBarth from t=0.  These will complete early in the run and increment
        # mtsanbarth_daughters_loaded via the normal DISCHARGING handler.  We raise
        ...
        self.mtsanbarth_daughters_min_threshold = 2 + _sj_preseeded
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        Priority rule:
          - Primary mothers (Bryanston, GreenEagle) are preferred when they can
            berth within SANJULIAN_DELAY_THRESHOLD_HOURS of MTSanBarth.
          - MTSanBarth is included as a candidate when she can berth the vessel
          ...
          - EXCEPTION: MTO transient vessels (_is_mto_offload=True) must NEVER
            be assigned MTSanBarth. All consolidated MTO cargo must discharge to
        ...
        # Bedford/Balham carrying Ibom crude must only discharge to primary
        # mothers (Bryanston or GreenEagle) — never to MTSanBarth.
        _vessel_is_ibom = (v.name in {"Bedford", "Balham"}
                           and abs(self.vessel_api.get(v.name, 0.0) - IBOM_API) < 0.5)
        # Look-ahead: also include primary mothers returning from export within 24h
        # so vessels waiting at BIA reassign from MTSanBarth to an incoming primary.
        _lookahead_t = at_time + 24.0

        for mother_name in MOTHER_NAMES:
            # MTO transient vessels: MTSanBarth is never a valid target
            if _vessel_is_mto and mother_name == MOTHER_QUATERNARY_NAME:
                continue
            # Ibom-crude vessels (Bedford/Balham): MTSanBarth is never a valid target
            if _vessel_is_ibom and mother_name == MOTHER_QUATERNARY_NAME:
                continue
        ...
        # are already inbound/waiting, also offer MTSanBarth even if a primary could
        # take the vessel — this distributes load and reduces queue time.
        # Hard gate: MTSanBarth cannot receive a daughter while she is still
        # berthed at a primary mother (BERTHING_B / HOSE_CONNECT_B / DISCHARGING
        # / CAST_OFF_B). She must have fully cast off first.
        sj_occupied = self.mtsanbarth_status in {
    ...
    def mother_is_at_point_b(self, mother_name, t):
        ...
        MTSanBarth does not go on export voyages, but she CAN be marked
        ...
        # including MTSanBarth for dry-dock / maintenance periods)
        for _start_h, _end_h in self.mother_unavailability_windows.get(mother_name, []):
            if _start_h <= t < _end_h:
                return False
        if mother_name == MOTHER_QUATERNARY_NAME:
            # MTSanBarth has no export state machine — just return True once
    ...
    # ── MTSanBarth transload machinery ─────────────────────────────────────────

    def _sj_primary_mothers(self):
        """Return list of primary mother names (everything except MTSanBarth)."""
    ...
    def _sj_best_transload_target(self, t, require_full_space=True):
        ...
        When require_full_space=True (default, regulatory compliance):
          Only considers mothers that have space >= the ENTIRE MTSanBarth volume.
          This enforces the single-berthing-operation rule — no split discharges.
          MTSanBarth will wait at anchor until a mother can receive her full contents.
    ...
    def _daughters_inbound_to_bia(self):
        ...
        Used for MTSanBarth dynamic threshold and export departure look-ahead.
        """
        _at_or_inbound = {
    ...
    def _sj_transload_trigger_check(self, t):
        ...
        Priority rules:
          T1  MTSanBarth at ≥90% capacity — force drain immediately
          T2  MTSanBarth holds enough to complete a primary mother's export gap
          T3  A primary mother is idle (no daughter active or arriving) AND
              MTSanBarth holds any volume — trigger regardless of stock size.
              This fires even if MTSanBarth only holds a small amount.
          T4  Optimisation: MTSanBarth above 25% and a mother has ≥50k free space

        Hard gate: MTSanBarth cannot start a transload while any daughter vessel
        is still berthing, connecting, discharging, or casting off at MTSanBarth.
        She must wait until her own berth is fully clear first.
        """
        sj_vol = self.mother_bbl[MOTHER_QUATERNARY_NAME]
        sj_cap = MTSANBARTH_CAPACITY_BBL

        # Never transload if MTSanBarth is empty
        if sj_vol <= 0:
            return None, None, 0

        # Never start a new cycle while one is active
        if self.mtsanbarth_status is not None:
            return None, None, 0

        # Never start a new cycle during post-cast-off fender preparation
        if t < self.mtsanbarth_fender_ready_t:
            return None, None, 0

        # ── Hard gate: wait until no daughter is berthing/discharging to MTSanBarth ──
        daughter_on_mtsanbarth = any(
            vv.assigned_mother == MOTHER_QUATERNARY_NAME and
            vv.status in {"BERTHING_B", "HOSE_CONNECT_B", "DISCHARGING", "CAST_OFF_B"}
            for vv in self.vessels
        )
        if daughter_on_mtsanbarth:
            return None, None, 0
        ...
        # daughter beyond the base (capped at mtsanbarth_daughters_min_threshold + 3)
        # so MTSanBarth accumulates more cargo before draining.  This reduces the
        ...
        _effective_min_threshold = self.mtsanbarth_daughters_min_threshold + _dyn_extra
        ...
        # T1b — MTSanBarth holding ≥50% but gate not yet met AND daughters still
        ...
        # highest-urgency optimisation opportunity: drain MTSanBarth into the
        ...
        # MTSanBarth is holding meaningful cargo and a primary is near-empty.
        for mn in self._sj_primary_mothers():
            if not self.mother_is_at_point_b(mn, t):
                continue
        ...
        # cause of MTSanBarth holding volume for many days (all primaries cycling
        ...
        # T2 — MTSanBarth holds enough to complete a primary mother's export gap
        # Requires the target has space for the entire MTSanBarth volume (no partial).
        for mn in self._sj_primary_mothers():
        ...
        #       MTSanBarth transloads regardless of stock volume (even small amounts).
        for mn in self._sj_primary_mothers():
            if not _sj_eligible_strict(mn):
                continue
            space = self.mother_capacity_bbl(mn) - self.mother_bbl[mn]
        ...
        # T4 — Optimisation: any primary mother has space ≥ full MTSanBarth volume.
        ...
        if receiver_available:
            for mn in MOTHER_NAMES:
                ...
                if mn == MOTHER_QUATERNARY_NAME and self.mtsanbarth_status in {
                    ...
                    if self.mtsanbarth_next_t is not None:
                        berth_free_at = max(berth_free_at, self.mtsanbarth_next_t)
    ...
    def _run_mtsanbarth_transload(self, t):
        """Advance MTSanBarth's transload state machine each time-step.

        MTSanBarth behaves exactly like a daughter vessel when transloading:
        ...
        All events are logged with Vessel = "MTSanBarth" so the mother vessel
        ...
        if self.mtsanbarth_status is not None and self.mtsanbarth_next_t is not None:
            if t < self.mtsanbarth_next_t - 1e-6:
                return   # phase not yet complete — nothing to do this step

            target = self.mtsanbarth_target
            amount = self.mtsanbarth_amount

            # ── BERTHING_B → HOSE_CONNECT_B ──────────────────────────────────
            if self.mtsanbarth_status == "BERTHING_B":
            ...
            # ── HOSE_CONNECT_B → DISCHARGING: credit cargo to mother now ─────
                ...
                # MTSanBarth must discharge ALL contents to a single mother in
                ...
                if space < sj_vol:
                    self.log_event(t, _sj, "SJ_TRANSLOAD_ABORT",
                                   ...
                                   f"space but MTSanBarth holds {sj_vol:,.0f} bbl — "
                ...
                if sj_vol <= 0:
                    self.log_event(t, _sj, "SJ_TRANSLOAD_ABORT",
                                   f"Transload to {target} aborted — MTSanBarth is empty")
                    self._sj_reset()
                    return
                ...
                self.mtsanbarth_amount = actual  # store actual for DISCHARGE_COMPLETE log

                pump_hours = actual / MTSANBARTH_TRANSLOAD_RATE_BPH
                self.mtsanbarth_status = "DISCHARGING"
                self.mtsanbarth_next_t = t + pump_hours
                ...
                self.mtsanbarth_transload_state = {
                    ...
                    "end_t": self.mtsanbarth_next_t, "phase": "DISCHARGING",
                }
            ...
            # ── DISCHARGING → CAST_OFF_B ─────────────────────────────────────
            # MTSanBarth must empty ALL her volumes to the SAME target vessel in
            ...
            # permits.  If the target fills before MTSanBarth empties, the
            # operation aborts — MTSanBarth re-anchors and waits for a mother
            # with sufficient space before starting a new cycle.
            # PROHIBITED: redirecting residual to a different mother vessel.
            elif self.mtsanbarth_status == "DISCHARGING":
                ...
                if sj_remaining > 0:
                    ...
                    if cur_space > 0 and self.mother_is_at_point_b(target, t):
                        ...
                        if self.mother_bbl[_sj] <= 0:
                            self.mother_api[_sj] = 0.0
                        self.mtsanbarth_amount += residual
                        extra_pump_hours = residual / MTSANBARTH_TRANSLOAD_RATE_BPH
                        self.mtsanbarth_next_t = t + extra_pump_hours
                        self.mother_berth_free_at[target] = max(
                            self.mother_berth_free_at.get(target, 0.0),
                            t + extra_pump_hours + CAST_OFF_HOURS,
                        )
                        self.mtsanbarth_transload_state = {
                            "target": target, "amount": self.mtsanbarth_amount,
                            "end_t": self.mtsanbarth_next_t, "phase": "DISCHARGING",
                        }
                        self.log_event(t, _sj, "DISCHARGE_START",
                        ...
                        self.log_event(t, _sj, "SJ_TRANSLOAD_ABORT",
                                       ...
                                       f"MTSanBarth re-anchoring with {sj_remaining:,.0f} bbl remaining.",
                                       mother=target)
                        actual = self.mtsanbarth_amount
                        self.mtsanbarth_total_transloaded += actual
                        cast_off_t = self.next_cast_off_window(t)
                        wait_co    = cast_off_t - t
                        self.mtsanbarth_status = "CAST_OFF_B"
                        self.mtsanbarth_next_t = cast_off_t + CAST_OFF_HOURS
                        self.mtsanbarth_transload_state = {
                            "target": target, "amount": actual,
                            "end_t": self.mtsanbarth_next_t, "phase": "CAST_OFF_B",
                        }
                        if wait_co > 0:
                ...
                self.log_event(t, _sj, "DISCHARGE_COMPLETE",
                               f"{target}: {self.mother_bbl[target]:,.0f} bbl | "
                               f"MTSanBarth emptied — {actual:,.0f} bbl total | "
            ...
            # ── CAST_OFF_B → cycle complete ──────────────────────────────────
            elif self.mtsanbarth_status == "CAST_OFF_B":
                # Before completing, check if more barrels arrived on MTSanBarth
                ...
                if sj_remaining > 0:
                    ...
                    # remaining; a new cycle will start once MTSanBarth is free
                ...
                self.log_event(t, _sj, "SJ_TRANSLOAD_COMPLETE",
                               ...
                               f"{actual:,.0f} bbl | "
                               f"MTSanBarth now {self.mother_bbl[_sj]:,.0f} bbl | "
                               f"{target} now {self.mother_bbl[target]:,.0f} bbl",
                               mother=target)
                # ── Fender preparation after cast-off ───────────────────────
                # MTSanBarth requires SANJULIAN_FENDER_PREP_HOURS of fender
                ...
                self.mtsanbarth_fender_ready_t = fender_ready
                self.log_event(t, _sj, "SJ_FENDER_PREP",
                               ...
                               mother=target)
                self.mtsanbarth_transload_end_t = t
                self._sj_reset()
                return

        # ── Check whether a new transload cycle should start ──────────────────
        if self.mtsanbarth_status is None:
            ...
            # Also enforce that MTSanBarth's fender preparation is complete before she
            # can berth: add SANJULIAN_FENDER_PREP_HOURS to earliest possible start.
            _fender_earliest = max(t, self.mtsanbarth_fender_ready_t)
            ...
            self.mtsanbarth_status = "BERTHING_B"
            self.mtsanbarth_target = target
            self.mtsanbarth_amount = amount
            self.mtsanbarth_next_t = berth_start + BERTHING_DELAY_HOURS
            self.mtsanbarth_transload_state = {
                "target": target, "amount": amount,
                "end_t": self.mtsanbarth_next_t, "phase": "BERTHING_B",
            }
            self.mtsanbarth_transload_end_t = berth_start + _full_cycle
            self.log_event(t, _sj, "SJ_TRANSLOAD_START",
                           f"[{reason}] Transload cycle started → {target}: "
                           f"{amount:,.0f} bbl @ {MTSANBARTH_TRANSLOAD_RATE_BPH:,} bph | "
                           f"MTSanBarth: {self.mother_bbl[_sj]:,.0f} bbl | "
                           f"{target}: {self.mother_bbl[target]:,.0f} bbl",
                           mother=target)
            self.log_event(berth_start, _sj, "BERTHING_START_B",
                           f"Berthing at {target} ({BERTHING_DELAY_HOURS}h procedure) "
                           f"— MTSanBarth transload",
                           mother=target)

    def _sj_reset(self):
        """Clear all MTSanBarth transload state after a cycle ends or aborts."""
        self.mtsanbarth_status          = None
        self.mtsanbarth_target          = None
        self.mtsanbarth_amount          = 0.0
        self.mtsanbarth_next_t          = None
        self.mtsanbarth_transload_state = None
    ...
    def select_point_b_mother(self, v, decision_time, day_key, candidates):
        ...
        Primary mothers are generally preferred over MTSanBarth.  However, when a
        same-day primary is delayed by more than SANJULIAN_DELAY_THRESHOLD_HOURS
        beyond MTSanBarth's earliest available start, that primary is demoted so that
        MTSanBarth absorbs the incoming daughter immediately rather than leaving her
        ...
        can become fully occupied, and MTSanBarth must act as the active buffer.

        Grouping:
          group 0 — same-day primary available within SANJULIAN_DELAY_THRESHOLD_HOURS
                    of MTSanBarth's start (or MTSanBarth not in candidates)
          group 1 — MTSanBarth (beats delayed primaries; loses to prompt primaries)
        ...
        for start, berth_t, mother_name in candidates:
            ...
            ranked.append({
                ...
                "is_mtsanbarth": 1 if mother_name == MOTHER_QUATERNARY_NAME else 0,
            })

        # MTSanBarth's earliest start (None when she is not in candidates)
        sj_entry = next((r for r in ranked if r["is_mtsanbarth"] == 1), None)
        ...
        _g0_starts = [
            ...
            if r["is_mtsanbarth"] == 0 and r["same_day"]
    ...
    def log_event(self, t, vessel_name, event, detail="", voyage_num=None, mother=None):
        ...
        self.log.append({
            ...
            "MTSanBarth_bbl":  round(self.mother_bbl[MOTHER_QUATERNARY_NAME]),
            "MTSanBarth": self.mtsanbarth_status or "IDLE_B",
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            for v in _ordered_vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    # Lock berth through end of cast-off, not just end of pumping.
                    # A vessel physically alongside the mother cannot leave until cast-off
                    # completes — the berth must remain locked for that full duration.
                    # next_cast_off_window is not yet known precisely here, so use the
                    ...
                    if getattr(v, "_is_mto_offload", False):
                        ...
                        v.voyage_code = make_voyage_code(v.name, v.current_voyage)
                    # Track how many daughters have fully discharged to MTSanBarth
                    ...
                    if selected_mother == MOTHER_QUATERNARY_NAME:
                        self.mtsanbarth_daughters_loaded += 1
                    ...
                    if v.assigned_mother not in MOTHER_NAMES:
                        ...
                        continue
                    selected_mother = v.assigned_mother
                    # MTSanBarth never triggers the export state machine — it
                    # transloads to primary mothers instead (see _run_mtsanbarth_transload)
            ...
            if active_export_mother is None:
                for _fm, _fhours in EXPORT_FORCE_SCHEDULE.items():
                    if _fm == MOTHER_QUATERNARY_NAME:
                        continue   # MTSanBarth never exports
            ...
            if active_export_mother is None and t >= self.next_export_allowed_at:
                ...
                # In case a mother reached her export trigger via MTSanBarth
            ...
            for mother_name in MOTHER_NAMES:
                # MTSanBarth has no export state — handled by transload machinery
                if mother_name == MOTHER_QUATERNARY_NAME:
                    continue
            ...
            # 3b. Advance MTSanBarth transload state machine
            # MTSanBarth never exports — instead it pumps inventory to primary
            # mothers whenever one of the four trigger conditions is satisfied.
            self._run_mtsanbarth_transload(t)

            # 3c. Advance ZeeZee third-party discharge state machine
            # Runs after MTSanBarth so both compete for the same berth slots
            ...
            self.timeline.append({
                ...
                "MTSanBarth_bbl":  round(_m_bbl[MOTHER_QUATERNARY_NAME]),
                "MTSanBarth": self.mtsanbarth_status or "IDLE_B",
                ...
                "MTSanBarth_api":  round(_m_api.get(MOTHER_QUATERNARY_NAME, 0.0), 2),
                **vessel_statuses,
        ...
        # MTSanBarth final state — exposed so the app can surface them in KPIs
        self.final_mtsanbarth_bbl        = self.mother_bbl[MOTHER_QUATERNARY_NAME]
        self.final_mtsanbarth_api        = self.mother_api.get(MOTHER_QUATERNARY_NAME, 0.0)
        # mtsanbarth_total_transloaded already accumulated during the run
...
ax2.plot(timeline_df["Time"], timeline_df["MTSanBarth_bbl"],
