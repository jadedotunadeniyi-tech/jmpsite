...
#     unable to receive on the same day (last-resort receiver).
#   - Stores cargo up to SANJULIAN_CAPACITY_BBL.
#   - Transloads to primary mothers when any of four conditions fire
#     (see Simulation._run_sanjulian_transload).
#   - Excluded from MOTHER_NAMES so all export state-machines ignore it.
...
POINT_A_ONLY_VESSELS = {"Watson", "Woodstock"}  # not permitted at Point C (Westmore)
...
class Simulation:
    def __init__(self):
        ...
        # Berth timeline: earliest t when SanJulian can start a new reception.
        # Mirrors mother_berth_free_at but is separate so it never conflicts.
        self.sanjulian_berth_free_at    = 0.0
        # Active transload: None | {"mother": str, "end_t": float, "vol": float}
        # Only one transload runs at a time.
        self.sanjulian_transload_state  = None
        # Cumulative statistics (available in summary / CSV)
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        # ── SanJulian secondary fallback ─────────────────────────────────────
        # Only offered when no primary mother has a feasible slot right now.
        if not candidates:
    ...
    def select_point_b_mother(self, v, decision_time, day_key, candidates):
        ...
        for start, berth_t, mother_name in candidates:
            if mother_name == SANJULIAN_NAME:
                # SanJulian projected "stock" is its own volume; it doesn't export
    ...
    def _sanjulian_primary_mothers_available(self, cargo_bbl, at_time):
        """Return True if at least one primary mother can currently receive cargo_bbl."""
    ...
    def _sanjulian_candidate_slot(self, v, at_time):
        """Return earliest feasible berthing time for v to discharge to SanJulian,
        or None if SanJulian has no space or primary mothers are available.
        SanJulian is ONLY offered when no primary mother can accept the cargo."""
    ...
    def _run_sanjulian_transload(self, t):
        """Called every timestep.  Advances any active transload and, when none
        is running, checks four trigger conditions to start a new one.

        Trigger conditions (any one is sufficient):
          T1  SanJulian ≥ 90 % of capacity.
          T2  SanJulian holds enough volume to fill a mother to its export target.
        ...
        if self.sanjulian_transload_state is not None:
            ...
            if t >= state["end_t"]:
                # Transfer is complete — commit volumes
                mother  = state["mother"]
                vol_req = state["vol"]
                actual  = min(
                    vol_req,
                ...
                return   # transload in progress — nothing else to do this tick
        ...
        # Daughters still in transit that will arrive at BIA today
        _outbound = {
        ...
        for mother_name in MOTHER_NAMES:
            ...
            # T1: SanJulian at or above 90 % capacity → must offload urgently
            if sj_pct >= 0.90:
                trigger = True
                reason  = f"T1 — SanJulian at {sj_pct*100:.0f}% capacity"

            # T2: volume in SanJulian is enough to top up a mother to export volume
            elif (stock < MOTHER_EXPORT_VOLUME
                  and sj >= MOTHER_EXPORT_VOLUME - stock
                  and space >= MOTHER_EXPORT_VOLUME - stock):
                trigger = True
                reason  = (f"T2 — SanJulian can complete {mother_name}'s export target "
                           f"(need {MOTHER_EXPORT_VOLUME - stock:,.0f} bbl, have {sj:,.0f} bbl)")

            # T3: mother idle and no daughter arriving today → opportunistic
            elif (not daughters_arriving_today
                  and not any(
                      vv.assigned_mother == mother_name
                      and vv.status in {"BERTHING_B", "HOSE_CONNECT_B", "DISCHARGING", "WAITING_BERTH_B"}
                      for vv in self.vessels)):
                trigger = True
                reason  = f"T3 — {mother_name} idle, no daughter arrivals today"

            # T4: optimisation — meaningful SanJulian inventory and mother has room
            ...
            break   # only one concurrent transload
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    if v.assigned_mother not in MOTHER_NAMES and v.assigned_mother != SANJULIAN_NAME:
                        ...
                        v.next_event_time = t + 0.5
                        continue
                    # SanJulian is always physically at Point B (no export voyages)
                    ...
                    if v.assigned_mother not in MOTHER_NAMES and v.assigned_mother != SANJULIAN_NAME:
                        ...
                        continue
                    selected_mother = v.assigned_mother
                    # SanJulian never goes on export, so no mother_is_at_point_b check needed
                    ...
                    # Capacity check — uses SanJulian's own cap when applicable
                    if selected_mother == SANJULIAN_NAME:
                    ...
                    if _recv_stock + v.cargo_bbl > _recv_cap:
                        ...
                        if selected_mother == SANJULIAN_NAME:
                            # Credit cargo to SanJulian's inventory
                    ...
                    # Read receiver stock for log message
                    if selected_mother == SANJULIAN_NAME:
                        _recv_stock_now = self.sanjulian_bbl
                    else:
                        _recv_stock_now = self.mother_bbl[selected_mother]
                    ...
                    # SanJulian is a valid prior receiver; treat it the same as a mother
                    # for return-allocation purposes — the vessel simply departs BIA.
                    selected_mother = (v.assigned_mother
