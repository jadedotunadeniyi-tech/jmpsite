...
class Simulation:
    def __init__(self):
        ...
        self.last_daylight_reassessment_hour = None
    ...
    def trigger_idle_ac_reassessment(self, t):
        """Force immediate re-evaluation of idle A/C-target daughters.

        This does not bypass normal berth/stock/daylight constraints; it only
        wakes eligible IDLE_A vessels so the existing allocator can run now.
        """
        for vv in self.vessels:
            if vv.status != "IDLE_A":
                continue
            if vv.target_point not in {"A", "C"}:
                continue
            if vv.name == self.point_f_active_loader:
                continue
            if self.point_f_swap_pending_for == vv.name:
                continue
            if t < vv.next_event_time:
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            # Hourly daylight reassessment pulse for A/C berthing allocation.
            # This keeps idle daughter-to-storage matching active without
            # waiting for unrelated state transitions.
            if abs(t - round(t)) < 1e-9:
                wall_h = (t + SIM_HOUR_OFFSET) % 24
                if DAYLIGHT_START <= wall_h < DAYLIGHT_END:
                    hour_key = int(t)
                    if self.last_daylight_reassessment_hour != hour_key:
                        self.last_daylight_reassessment_hour = hour_key
                        self.trigger_idle_ac_reassessment(t)

            # 2. Advance each vessel's state machine.
            # During daylight, prioritize loading-allocation checks for IDLE_A
            # daughters by larger cargo capacity when competing at the same
            # location on the same day.
            due_vessels = [v for v in self.vessels if t >= v.next_event_time]
            wall_h_now = (t + SIM_HOUR_OFFSET) % 24
            daylight_now = DAYLIGHT_START <= wall_h_now < DAYLIGHT_END
            order_index = {id(v): i for i, v in enumerate(self.vessels)}
            if daylight_now:
                due_vessels.sort(
                    key=lambda vv: (
                        1 if vv.status == "IDLE_A" else 0,
                        vv.target_point if vv.status == "IDLE_A" else "",
                        -vv.cargo_capacity if vv.status == "IDLE_A" else 0,
                        order_index[id(vv)],
                    )
                )
            else:
                due_vessels.sort(key=lambda vv: order_index[id(vv)])

            for v in due_vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    self.log_event(arrival, v.name, "CROSSED_BREAKWATER_AC_IN",
                                   ...
                                   voyage_num=v.current_voyage)
                    # Reassess idle A/C allocation immediately when a daughter
                    # crosses inbound breakwater toward Point A/C.
                    self.trigger_idle_ac_reassessment(arrival)
                    v.status = "SAILING_BA"
