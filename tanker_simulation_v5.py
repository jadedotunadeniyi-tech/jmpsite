...
GREENEAGLE_CAPACITY_BBL = 570_000
...
# Per-vessel-day discharge point overrides for daughter vessels.
# Structure: {vessel_name: {day_number (int): mother_name (str)}}
# When a daughter arrives at Point B (or is reassessed while WAITING_BERTH_B)
# on a day that has an entry here, she is forced to discharge to the specified
# mother vessel — provided that mother is currently feasible.  If the forced
# mother is not yet available, the daughter waits for the next hourly
# reassessment (same behaviour as a manual nomination on startup day).
# This override applies ONLY to named daughter vessels; it does NOT affect the
# ZeeZee third-party discharge schedule (which is controlled separately via
# ZEEZEE_SCHEDULE and the ZeeZee Discharge Override Panel in the app).
# Cleared and repopulated by run_sim on every call; never persists across runs.
...
MOTHER_EXPORT_TRIGGER_BY_NAME = {
    ...
    MOTHER_TERTIARY_NAME:  MOTHER_EXPORT_TRIGGER,
}
                ...
                if v.status == "IDLE_A":
                    ...
                    if not candidates:
                        ...
                        if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0:
                            ...
                            # DAUGHTER_DISCHARGE_OVERRIDES: {vessel: {day: mother}}
                            # Forces this daughter to a specific mother on a given
                            # sim day.  ZeeZee is unaffected (she is handled by
                            # _run_zeezee / ZEEZEE_SCHEDULE entirely separately).
                            _ddo_mother = DAUGHTER_DISCHARGE_OVERRIDES.get(v.name, {}).get(day_key)
                            _candidate_mothers = {m for _, _, m in candidates}
                            if _ddo_mother and _ddo_mother in _candidate_mothers:
                                ...
                                self.log_event(
                                    ...
                                    f"Discharge point override: forced to {_ddo_mother} "
                                    f"on Day {day_key + 1} (user rule)",
                                    voyage_num=v.current_voyage, mother=_ddo_mother,
                                )
                            elif _ddo_mother and _ddo_mother not in _candidate_mothers:
                                # Forced mother not yet feasible — wait for hourly recheck
                                v.next_event_time = self.next_daylight_hourly_berth_check(arrival, point="B")
                                self.log_event(
                                    arrival, v.name, "WAITING_BERTH_B",
                                    f"Discharge point override targets {_ddo_mother} on Day {day_key + 1} "
                                    f"but mother not currently feasible; hourly reassessment at "
                    ...
                    if STARTUP_DAY_DISABLE_POINT_B_PRIORITY and day_key == 0:
                        ...
                        # ── Daughter discharge point override (reassessment) ──
                        # Honour any user-forced mother rule before auto-selection.
                        ...
                        if _ddo_mother and _ddo_mother in _candidate_mothers:
                            ...
                            # Forced mother still not feasible — keep waiting
                            next_recheck = self.next_daylight_hourly_berth_check(decision_t, point="B")
                            v.next_event_time = next_recheck
                            self.log_event(
                                decision_t, v.name, "WAITING_BERTH_B",
                                f"Discharge point override awaiting {_ddo_mother} on Day {day_key + 1}; "
...
ax2.axhline(MOTHER_EXPORT_TRIGGER_BY_NAME[MOTHER_PRIMARY_NAME], color="#e74c3c", linestyle="--", alpha=0.7,
          label=(f"{MOTHER_PRIMARY_NAME}/{MOTHER_TERTIARY_NAME} Export Trigger "
