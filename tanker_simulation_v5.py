...
VESSEL_NAMES           = [
    ...
    "Laphroaig",   # loads 2nd
    "Rathbone",    # loads 3rd
    "SantaMonica", # loads 4th
    "Bedford",     # loads 4th
    "Balham",      # loads 5th
    "Woodstock",   # loads 6th
    "Bagshot",     # loads 7th
    "Watson",      # loads 8th (Point A/C)
    "Berners",     # Point A only
]
VESSEL_CAPACITIES      = {
    "Rathbone" : 44_000,
    "SantaMonica": 28_000,
...
# ── SanJulian — intermediate floating storage (does NOT export) ───────────────
# SanJulian sits permanently at BIA, receives daughter discharges exactly like
# the primary mothers, but NEVER sails to the export terminal.
# Instead it transloads its inventory to primary mothers via a dedicated pump
# at SANJULIAN_TRANSLOAD_RATE_BPH.  Transloads are triggered when any of the
# four conditions below are satisfied (checked every TIME_STEP_HOURS):
#   T1  SanJulian reaches SANJULIAN_CAPACITY_BBL (force-drain to avoid overflow)
#   T2  SanJulian holds >= a primary mother's remaining export-volume requirement
#   T3  A primary mother is idle at Point B and no daughter is arriving same day
#   T4  Optimisation window: SanJulian > SANJULIAN_OPTIM_THRESHOLD_FRAC of
#       capacity AND a primary mother has >= SANJULIAN_OPTIM_MIN_SPACE_BBL free
SANJULIAN_CAPACITY_BBL          = 450_000                # SanJulian max capacity (bbl)
SANJULIAN_TRANSLOAD_RATE_BPH    = 12_000                 # pump rate to primary mothers
SANJULIAN_OPTIM_THRESHOLD_FRAC  = 0.25                  # T4: start optimising above 25%
SANJULIAN_OPTIM_MIN_SPACE_BBL   = 50_000                # T4: mother must have this headroom
SANJULIAN_FENDER_PREP_HOURS     = 1.5                   # fender prep: after cast-off AND before berthing
# Minimum hours a same-day primary must be delayed beyond SanJulian's available
# start before SanJulian is preferred.  When all same-day primaries are delayed
# by more than this threshold, daughters are routed to SanJulian immediately
# rather than queuing for hours at congested primaries.
...
# SantaMonica may only load from Chapel (Point A), Duke (Point D),
# and Starturn (Point E). JasmineS and Westmore are NOT permitted.
...
# Watson loads from Point A (Chapel, JasmineS) and Point C (Westmore).
WATSON_PERMITTED_STORAGES = (
    STORAGE_PRIMARY_NAME,    # Chapel   — Point A
    STORAGE_SECONDARY_NAME,  # JasmineS — Point A
    STORAGE_TERTIARY_NAME,   # Westmore — Point C
)
WESTMORE_PERMITTED_VESSELS = {"Sherlock", "Laphroaig", "Bagshot", "Rathbone", "Watson"}  # Watson permitted at Point C; Woodstock and SantaMonica not permitted at Point C
DUKE_PERMITTED_VESSELS = {"Woodstock", "Bagshot", "Rathbone", "SantaMonica"}
STARTURN_PERMITTED_VESSELS = {"Woodstock", "Rathbone", "SantaMonica"}
POINT_A_ONLY_VESSELS = {"Woodstock", "Berners"}  # not permitted away from Point A
...
# ── THIRD-PARTY VESSEL: ZeeZee ────────────────────────────────────────────────
#  ZeeZee is an externally-operated tanker that arrives at Point B fully loaded
#  once per calendar month to discharge to any available primary mother vessel.
#  She receives discharge priority over regular daughters on her arrival day,
#  unless mothers are operationally constrained (on export / offline / at cap).
#  If the only blocker is daughter-vessel congestion at Point B, ZeeZee's wait
#  must not exceed ZEEZEE_MAX_WAIT_HOURS (default 48 h = 2 days).
#
#  ZEEZEE_ARRIVAL_DAY   — calendar day-of-month she appears (1–28)
#  ZEEZEE_CARGO_BBL     — cargo she brings on each visit (bbl)
#  ZEEZEE_API           — API gravity of her cargo
#  ZEEZEE_MAX_WAIT_HOURS— max hours she waits when only daughters are blocking
# ─────────────────────────────────────────────────────────────────────────────
ZEEZEE_ENABLED         = True
ZEEZEE_ARRIVAL_DAY     = 15        # day-of-month she arrives (1–28)
ZEEZEE_CARGO_BBL       = 200_000   # cargo volume per visit (bbl)
ZEEZEE_API             = 32.0      # API gravity of ZeeZee's cargo
ZEEZEE_MAX_WAIT_HOURS  = 48.0      # max wait when blocked only by daughters
...
MOTHER_QUATERNARY_NAME = "SanJulian"
...
MOTHER_CAPACITY_BY_NAME = {
    MOTHER_PRIMARY_NAME: MOTHER_CAPACITY_BBL,
    MOTHER_SECONDARY_NAME: 570_000,
    MOTHER_TERTIARY_NAME: 570_000,
    MOTHER_QUATERNARY_NAME: SANJULIAN_CAPACITY_BBL,
}
...
class ThirdPartyVessel:
    ...
    Lifecycle per visit:
      WAITING_B  → arrives at BIA, awaiting a berth slot
      BERTHING_B → berth confirmed, 30-min berthing procedure
      HOSE_CONNECT_B → 2-h hose connection
      DISCHARGING → 12-h cargo transfer into mother vessel
      CAST_OFF_B  → departure (leaves fleet until next monthly arrival)

    Priority: ZeeZee is processed before regular daughters in the run() loop
    on her arrival day.  She bypasses the select_point_b_mother() scoring and
    takes the earliest available primary mother slot directly.
    """

    def __init__(self, cargo_bbl: float, api: float, arrival_t: float):
        self.name           = "ZeeZee"
        self.cargo_bbl      = cargo_bbl
        self.cargo_capacity = cargo_bbl
        self.api            = api
        self.status         = "WAITING_B"
        self.arrival_t      = arrival_t          # sim-hour she appeared at BIA
        self.assigned_mother = None
        self.next_event_time = arrival_t
        self.current_voyage  = 1
        # Deadline enforcement — set once priority is first delayed by daughters
        self.daughter_block_start: float | None = None

    def __repr__(self):
        return f"ZeeZee[{self.status}|cargo={self.cargo_bbl:,}bbl]"
...
class Simulation:
    def __init__(self):
        ...
        # zeezee: None when not visiting; ThirdPartyVessel instance while active
        # zeezee_months_visited: set of (year, month) already triggered so the
        #   monthly check fires exactly once per calendar month.
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        if sj_candidate is not None and not sj_occupied and not sj_fender_busy:
            ...
            if earliest_primary_start is None or sj_start < earliest_primary_start - 1e-6:
    ...
    def mother_is_at_point_b(self, mother_name, t):
        ...
        SanJulian never leaves Point B — it always returns True (no export sailing).
        """
        if mother_name == MOTHER_QUATERNARY_NAME:
            # SanJulian is permanently moored at BIA — it never goes on export
            return True
        ...
        if t < self.mother_seeded_away_until.get(mother_name, 0.0):
            return False
        # Mid-simulation scheduled unavailability windows
    ...
    def mother_export_departure_eligible(self, mother_name):
        ...
        remaining_capacity = max(0.0, self.mother_capacity_bbl(mother_name) - stock)
    ...
    def _sj_best_transload_target(self, t):
        ...
        for mn in self._sj_primary_mothers():
            ...
            to_trigger = max(0.0, MOTHER_EXPORT_TRIGGER - self.mother_bbl[mn])
    ...
    def _sj_transload_trigger_check(self, t):
        ...
        # ── Minimum-load gate ─────────────────────────────────────────────────
        # SanJulian must have received full discharges from at least 2 daughter
        # vessels (since her last offload cycle) before T2/T3/T4 triggers fire.
        # The threshold is raised at startup for any pre-seeded vessels so that
        # only genuinely fresh arrivals during this sim run count toward the gate.
        # T1 (near-capacity force-drain) bypasses this to prevent overflow.
        _min_daughters_met = (
            self.sanjulian_daughters_loaded >= self.sanjulian_daughters_min_threshold
        )
        ...
        # T2/T3/T4/T5 require at least 2 daughter discharges first
    ...
    def _run_sanjulian_transload(self, t):
        ...
        if self.sanjulian_status is not None and self.sanjulian_next_t is not None:
            ...
            # ── CAST_OFF_B → cycle complete ──────────────────────────────────
                ...
                if (self.mother_bbl[target] >= MOTHER_EXPORT_TRIGGER
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            #   Fires once per calendar month on ZEEZEE_ARRIVAL_DAY at 08:00.
            #   ZeeZee is injected only when no visit is already in progress.
            if ZEEZEE_ENABLED and self.zeezee is None:
                _zz_wall = (t + SIM_HOUR_OFFSET) % 24
                _zz_cal  = _SIM_EPOCH + timedelta(hours=t)
                _zz_ym   = (_zz_cal.year, _zz_cal.month)
                if (_zz_cal.day == ZEEZEE_ARRIVAL_DAY
                        and abs(_zz_wall) < TIME_STEP_HOURS * 0.5   # fires at 08:00 tick
                        and _zz_ym not in self.zeezee_months_visited):
                    self.zeezee_months_visited.add(_zz_ym)
                    self.zeezee = ThirdPartyVessel(
                        cargo_bbl  = ZEEZEE_CARGO_BBL,
                        api        = ZEEZEE_API,
                        arrival_t  = t,
                    )
                    self.vessel_api["ZeeZee"] = ZEEZEE_API
                    self.log_event(
                        t, "ZeeZee", "VESSEL_JOINED",
                        f"ZeeZee arrived at Point B — {ZEEZEE_CARGO_BBL:,} bbl "
                        f"@ {ZEEZEE_API}° API — awaiting discharge berth",
                    )

            # Step B: advance ZeeZee's state machine (runs before daughter loop).
            if ZEEZEE_ENABLED and self.zeezee is not None:
                _zz = self.zeezee
                if t >= _zz.next_event_time:

                    if _zz.status == "WAITING_B":
                        # ── Find earliest available primary mother ────────────
                        # ZeeZee always targets a PRIMARY mother (never SanJulian).
                        # She picks the mother with the earliest free berth that
                        # is at Point B and has capacity for her cargo.
                        _zz_best_start  = None
                        _zz_best_mother = None
                        _berthing_win   = self.next_berthing_window(t, point="B")
                        for _mn in MOTHER_NAMES:
                            ...
                            if _zz_best_start is None or _slot < _zz_best_start:
                                _zz_best_start  = _slot
                                _zz_best_mother = _mn

                        if _zz_best_mother is None:
                            # ── No primary available (operational constraint) ─
                            # Mothers are absent / at capacity — true operational
                            # block.  ZeeZee waits without counting against the
                            # daughter-congestion deadline.
                            _zz.daughter_block_start = None   # reset congestion clock
                            _next_chk = self.next_daylight_hourly_berth_check(t, point="B")
                            _zz.next_event_time = _next_chk
                            self.log_event(
                                t, "ZeeZee", "WAITING_MOTHER_CAPACITY",
                                "No primary mother available (operational constraint); "
                                f"reassessing at {self.hours_to_dt(_next_chk).strftime('%Y-%m-%d %H:%M')}",
                            )
                        else:
                            # ── Primary mother exists ─────────────────────────
                            # Check whether the delay is caused by daughter
                            # congestion (berth occupied by a queued daughter)
                            # vs the mother being genuinely unavailable.
                            _now_free = (self.mother_berth_free_at[_zz_best_mother]
                                         <= t + TIME_STEP_HOURS * 0.5)
                            if not _now_free:
                                # Berth is occupied — check if it is daughters causing the delay
                                _blocking_daughter = any(
                                    v.assigned_mother == _zz_best_mother
                                    and v.status in {"BERTHING_B","HOSE_CONNECT_B","DISCHARGING"}
                                    for v in self.vessels
                                )
                                if _blocking_daughter:
                                    # ── Daughter congestion — start / check deadline ─
                                    if _zz.daughter_block_start is None:
                                        _zz.daughter_block_start = t
                                        self.log_event(
                                            t, "ZeeZee", "WAITING_BERTH_B",
                                            f"Berth at {_zz_best_mother} occupied by daughter; "
                                            f"daughter-congestion clock started — deadline in "
                                            f"{ZEEZEE_MAX_WAIT_HOURS:.0f}h "
                                            f"({self.hours_to_dt(t + ZEEZEE_MAX_WAIT_HOURS).strftime('%Y-%m-%d %H:%M')})",
                                        )
                                    _waited = t - _zz.daughter_block_start
                                    if _waited >= ZEEZEE_MAX_WAIT_HOURS:
                                        # ── Deadline exceeded: evict next daughter ─
                                        # Find the daughter currently occupying the berth
                                        # and force-advance their state to vacate it.
                                        # We do this by marking mother berth free NOW
                                        # and logging the deadline breach.
                                        self.mother_berth_free_at[_zz_best_mother] = t
                                        _zz_best_start = self.next_berthing_window(t, point="B")
                                        self.log_event(
                                            t, "ZeeZee", "ZEEZEE_DEADLINE_OVERRIDE",
                                            f"2-day daughter-congestion deadline exceeded "
                                            f"({_waited:.1f}h waited); forcing berth clearance "
                                            f"at {_zz_best_mother} to honour ZeeZee priority",
                                        )
                                        _zz.daughter_block_start = None
                                    else:
                                        _next_chk = self.next_daylight_hourly_berth_check(t, point="B")
                                        _zz.next_event_time = _next_chk
                                        continue
                                else:
                                    # Mother berth reserved but NOT by a daughter
                                    # (e.g. SanJulian transload, fendering buffer).
                                    # Treat as operational constraint — no deadline.
                                    _zz.daughter_block_start = None
                                    _next_chk = self.next_daylight_hourly_berth_check(t, point="B")
                                    _zz.next_event_time = _next_chk
                                    self.log_event(
                                        t, "ZeeZee", "WAITING_BERTH_B",
                                        f"Berth at {_zz_best_mother} reserved (non-daughter constraint); "
                                        f"reassessing at {self.hours_to_dt(_next_chk).strftime('%Y-%m-%d %H:%M')}",
                                    )
                                    continue

                            # ── Berth is free: proceed to BERTHING_B ─────────
                            _zz.daughter_block_start = None
                            _zz.assigned_mother = _zz_best_mother
                            _discharge_end = (
                                _zz_best_start + BERTHING_DELAY_HOURS
                                + HOSE_CONNECTION_HOURS + DISCHARGE_HOURS
                            )
                            self.mother_berth_free_at[_zz_best_mother] = max(
                                self.mother_berth_free_at[_zz_best_mother], _discharge_end
                            )
                            _zz.status = "BERTHING_B"
                            _zz.next_event_time = _zz_best_start + BERTHING_DELAY_HOURS
                            self.log_event(
                                _zz_best_start, "ZeeZee", "BERTHING_START_B",
                                f"ZeeZee berthing at {_zz_best_mother} "
                                f"(priority discharge — 30 min procedure)",
                                mother=_zz_best_mother,
                            )

                    elif _zz.status == "BERTHING_B":
                        _mn = _zz.assigned_mother
                        if not self.mother_is_at_point_b(_mn, t):
                            # Mother left unexpectedly — requeue
                            ...
                            self.log_event(t, "ZeeZee", "HOSE_CONNECTION_START_B",
                                           f"Hose connection initiated at {_mn} (2 h)",
                                           mother=_mn)
                        ...
                        _mcap = self.mother_capacity_bbl(_mn)
                        if self.mother_bbl[_mn] + _zz.cargo_bbl > _mcap:
                            # Capacity issue (e.g. unexpected transfer) — wait
                            _zz.next_event_time = t + 6
                            self.log_event(t, "ZeeZee", "WAITING_MOTHER_CAPACITY",
                                           f"{_mn} lacks space for ZeeZee cargo; rechecking in 6 h")
                        else:
                            ...
                            self.mother_berth_free_at[_mn] = max(
                                self.mother_berth_free_at[_mn], t + DISCHARGE_HOURS
                            )
                            _zz.next_event_time = t + DISCHARGE_HOURS
                            self.log_event(
                                t, "ZeeZee", "DISCHARGE_START",
                                f"Discharging {_zz.cargo_bbl:,} bbl "
                                f"@ {_zz.api:.2f}° API | "
                        ...
                        self.log_event(t, "ZeeZee", "DISCHARGE_COMPLETE",
                                       f"{_mn}: {self.mother_bbl[_mn]:,.0f} bbl | "
                                       f"ZeeZee departure in {CAST_OFF_HOURS*60:.0f} min",
                                       mother=_mn)

                    elif _zz.status == "CAST_OFF_B":
                        self.log_event(t, "ZeeZee", "VESSEL_DEPARTED",
                                       "ZeeZee cast off and departed Point B — next visit next month")
                        self.zeezee = None   # visit complete; reset for next month
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    _forced_stor  = STORAGE_DISPATCH_OVERRIDES.get(v.name, {}).get(_dispatch_day)
                    if (_forced_stor
                        ...
                        # Clear stale assignment so pre-assigned block cannot intercept
            ...
            # Include ZeeZee in vessel_statuses when she is visiting.
            if self.zeezee is not None:
                _zz_snap = self.zeezee
                vessel_statuses_extra = {
                    "ZeeZee":            _zz_snap.status,
                    "ZeeZee_cargo_bbl":  round(_zz_snap.cargo_bbl),
                    "ZeeZee_api":        round(_zz_snap.api, 2),
                }
            else:
                vessel_statuses_extra = {}
