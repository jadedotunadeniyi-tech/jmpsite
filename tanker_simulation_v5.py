...
SANJULIAN_CAPACITY_BBL          = 440_000                # SanJulian max capacity (bbl)
...
STATUS_CODES = {
    ...
    "SJ_FENDER_PREP"        : "SanJulian: fender preparation in progress",
...
class Simulation:
    ...
    def point_b_candidate_slots(self, v, at_time):
        ...
        Priority rule:
          - Primary mothers (Bryanston, Alkebulan, GreenEagle) always get first
            consideration and will be chosen by select_point_b_mother when they
            can berth the vessel at least as quickly as SanJulian.
          - SanJulian is included as a candidate when she can berth the vessel
            NO LATER than the earliest available primary mother slot.  This means:
              * If no primary is available at all → SanJulian is the fallback.
              * If all primaries have their berths occupied and SanJulian is free
                sooner → SanJulian is offered so the vessel does not wait idle.
              * If a primary can berth immediately → SanJulian is NOT offered
                (primary wins and is_sanjulian sort key keeps it ranked last anyway).
          The is_sanjulian sort key in select_point_b_mother ensures that when a
          primary and SanJulian have equal start times, the primary is always chosen.
        """
        ...
        for mother_name in MOTHER_NAMES:
            if not self.mother_is_at_point_b(mother_name, at_time):
                continue
            _cap = (SANJULIAN_CAPACITY_BBL
                    if mother_name == MOTHER_QUATERNARY_NAME
                    else MOTHER_CAPACITY_BBL)
    ...
    def _sj_best_transload_target(self, t):
        ...
        for mn in self._sj_primary_mothers():
            if not self.mother_is_at_point_b(mn, t):
                continue          # skip mothers currently at export or seeded away
            # Skip mothers flagged for imminent export departure (DOC or export_ready)
            if self.export_state.get(mn) == "DOC":
                continue
            if self.export_ready.get(mn):
                continue
            space = MOTHER_CAPACITY_BBL - self.mother_bbl[mn]
            if space <= 0:
                continue
    ...
    def _sj_transload_trigger_check(self, t):
        ...
        # T1 — SanJulian at or near capacity: force drain immediately (no gate)
        if sj_vol >= sj_cap * 0.90:
            target, amount = self._sj_best_transload_target(t)
            if target:
                return "T1-NEAR_CAPACITY", target, amount

        # T2/T3/T4 require at least 2 daughter discharges first
        if not _min_daughters_met:
            return None, None, 0

        # Helper: returns True if a primary mother is eligible to receive a transload
        # (at Point B, not in export operations, not flagged for imminent export)
        def _sj_eligible(mn):
            if not self.mother_is_at_point_b(mn, t):
        ...
        # T4 — Optimisation: any primary mother has ≥50k free space.
        # Threshold lowered: SanJulian transloads whenever she holds any volume
        # and a primary mother has meaningful headroom, not just above 25%.
    ...
    def _run_sanjulian_transload(self, t):
        ...
        if self.sanjulian_status is not None and self.sanjulian_next_t is not None:
            ...
            # ── HOSE_CONNECT_B → DISCHARGING: credit cargo to mother now ─────
                ...
                space   = max(0.0, MOTHER_CAPACITY_BBL - self.mother_bbl[target])
            ...
            # remains, or switching to another primary mother if needed.
                ...
                if sj_remaining > 0:
                    ...
                    cur_space = max(0.0, MOTHER_CAPACITY_BBL - self.mother_bbl[target])
    ...
    def select_point_b_mother(self, v, decision_time, day_key, candidates):
        ...
        Primary mothers always rank before SanJulian (is_sanjulian sort key).

        Crucially: the old immediate/non-immediate pool split is removed.
        That split caused SanJulian (free right now) to beat a primary mother
        (free later the same day) because the primary was never in the
        'immediate' pool and therefore never compared against SanJulian.
        Now all candidates are ranked together — primary mothers beat SanJulian
        regardless of whether SanJulian is available sooner today.
        SanJulian only wins when no primary mother can berth this vessel at all.
        """
        ...
        # Sort all candidates together — no separate immediate pool.
        # Key priority:
        #   1. Prefer same-day primary over SanJulian:
        #      - is_sanjulian=0 AND same_day=True  → group 0 (best)
        #      - is_sanjulian=1 (SanJulian)         → group 1 (middle)
        #      - is_sanjulian=0 AND same_day=False  → group 2 (next-day primary)
        #      This means SanJulian beats a primary that can only berth tomorrow.
        #   2. start time (earlier is better within group)
        #   3. projected stock at next 08:00 (higher is better for export readiness)
        #   4. unused_today (spread load across mothers)
        #   5. name (stable tiebreak)
        def _sort_key(r):
            if r["is_sanjulian"] == 0 and r["same_day"]:
                group = 0   # same-day primary — always first
            elif r["is_sanjulian"] == 1:
                group = 1   # SanJulian — middle priority
            else:
                group = 2   # next-day primary — SanJulian beats this
            return (
                group,
                r["start"],
                -r["projected_8"],
    ...
    def run(self):
        ...
        while t <= total_hours:
            ...
            for v in self.vessels:
                ...
                if v.status == "IDLE_A":
                    ...
                    if not self.mother_is_at_point_b(selected_mother, t):
                        ...
                        continue
                    _mother_cap = (SANJULIAN_CAPACITY_BBL
            ...
            self.timeline.append({
                ...
                "SanJulian_api": round(self.mother_api.get(MOTHER_QUATERNARY_NAME, 0.0), 2),
                **vessel_statuses
            })
...
ax2.plot(timeline_df["Time"], timeline_df["SanJulian_bbl"],
            color="#e67e22", linewidth=1.4, alpha=0.9, label=f"{MOTHER_QUATERNARY_NAME} Volume")
