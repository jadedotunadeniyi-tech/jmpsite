...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        # ── Arriving BIA ──────────────────────────────────────────────────────
        def _vcode_badge(r):
            """Small monospace voyage-code badge rendered below the vessel chip."""
            vc = str(r.get("VoyageCode", "")).strip()
            if not vc:
                return ""
            return (f'<div style="margin-top:2px">'
                    f'<span style="background:rgba(0,0,0,0.35);border-radius:2px;'
                    f'padding:0 5px;font-size:9px;font-family:monospace;'
                    f'letter-spacing:0.04em;color:#fff">{vc}</span></div>')
