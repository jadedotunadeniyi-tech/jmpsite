...
Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  ...
  "extraction_notes": ""
        ...
        # ── PDF file uploader (server-side extraction via Google Gemini API) ────
        # API key is read from Streamlit Cloud → Settings → Secrets.
        # Add this once:  GOOGLE_API_KEY = "AIzaSy..."
        ...
        if not _pdf_api_key:
            st.info(
                "**One-time setup required** — add your Google AI Studio API key to "
                "Streamlit Cloud → Settings → Secrets:\n\n"
                "```toml\nGOOGLE_API_KEY = \"AIzaSy...\"\n```\n\n"
                "⚠️ Get the key from **aistudio.google.com → Get API key** (not Google Cloud Console). "
                "AI Studio keys work immediately with no extra setup.",
                icon="🔑",
            )
        else:
            ...
            if _stock_pdf is not None:
                ...
                if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                    ...
                    _pdf_emsg = ""
                    try:
                        # Use google-generativeai SDK — handles auth, model resolution,
                        # and PDF uploads correctly without raw URL construction.
                        # Pre-installed on Streamlit Cloud.
                        import google.generativeai as _genai
                        import io as _io

                        _genai.configure(api_key=_pdf_api_key)

                        # Try models in order — SDK raises exception if model unavailable
                        _gemini_models = [
                            "gemini-2.0-flash",
                        ...
                        for _model_name in _gemini_models:
                            try:
                                ...
                                _gemini_response = _gemini_model.generate_content(
                                    [_pdf_part, (
                                        _IMPORTER_SYSTEM_PROMPT
                                        + "\n\nExtract all startup data from this "
                                        "Daily Stock Update report exactly as specified."
                                    )]
                                )
                                break  # success — stop trying models
                            except Exception as _me:
                                _last_model_err = f"{_model_name}: {_me}"
                                continue

                        if _gemini_response is None:
                            raise ValueError(
                                f"All Gemini models failed. Last error: {_last_model_err}. "
                                "Check your GOOGLE_API_KEY in Streamlit Cloud → Settings → Secrets. "
                                "Make sure it was created at aistudio.google.com (not Google Cloud Console)."
                            )

                        _resp_text = _gemini_response.text.strip()
