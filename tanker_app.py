...
=============================================================
  ...
  Tab 2  "fleet"     — one row per vessel per daily 8am update
    ...
    vessel   → exact name: Sherlock, Laphroaig, Rathbone, SantaMonica,
               Bedford, Balham, Woodstock, Bagshot, Watson, Berners
...
VESSEL_COLORS = {
    ...
    "Berners"  : "#7f8c8d",   # steel gray    — Point A-only 63k
...
LOCATION_CATALOGUE = [
    ...
    {"display": "Sailing → SanJulian (A/C outbound)", "sim_value": "En Route SanBarth→BIA",
    ...
    {"display": "Approaching SanJulian (Fairway Buoy)", "sim_value": "Fairway Buoy",
     "field_zone": "Transit", "target_mother": "SanJulian", "target_storage": None,
     "statuses": [
        ("SAILING_BW_TO_FWY",   "🚢 Breakwater → Fairway Buoy (2h)"),
        ("SAILING_AB_LEG2",     "🚢 Fairway Buoy → BIA (2h)"),
    ...
    {"display": "Breakwater outbound → SanJulian", "sim_value": "Breakwater (outbound)",
     "field_zone": "Transit", "target_mother": "SanJulian", "target_storage": None,
     "statuses": [
        ("SAILING_CH_TO_BW_OUT", "🚢 Channel → Breakwater (1h, tidal)"),
        ("SAILING_CROSS_BW_OUT", "🚢 Crossing Breakwater outbound (0.5h, tidal)"),
    ...
    {"display": "Cawthorne Channel outbound → SanJulian", "sim_value": "Cawthorne Channel (outbound)",
     "field_zone": "Transit", "target_mother": "SanJulian", "target_storage": None,
     "statuses": [
        ("SAILING_D_CHANNEL",    "🚢 Point D → Cawthorne Channel (3h, tidal)"),
        ("SAILING_CH_TO_BW_OUT", "🚢 Channel → Breakwater (1h, tidal)"),
    ...
    {"display": "SanJulian (BIA)",     "sim_value": "SanJulian",
     "field_zone": "BIA",
     "statuses": [
        ("DISCHARGING",            "⬇️ Discharge — in progress"),
        ("HOSE_CONNECT_B",         "🔧 Hose connection underway"),
    ...
    {"display": "Cawthorne Channel → BIA via SanJulian", "sim_value": "Cawthorne Channel (outbound)",
    ...
    {"display": "Returning Duke from BIA via SanJulian", "sim_value": "En Route BIA→Storage",
     "field_zone": "Transit", "target_storage": "Duke", "target_mother": "SanJulian",
     "statuses": [
        ("SAILING_B_TO_BW_IN",  "🚢 BIA → clear breakwater (1.5h)"),
        ("SAILING_CROSS_BW_IN", "🚢 Crossing Breakwater inbound (0.5h, tidal)"),
...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn, pgm,
    ...
    if vessel_resumption_json and hasattr(mod, "set_vessel_resumption"):
        ...
        for _vr in _vr_list:
            try:
                ...
                _vr_date      = str(_vr.get("date", "")).strip()
                ...
                if _vr_indef or _vr_date == "indefinite":
                    ...
                    # Seed it directly on the vessel object after sim is created
                    # (set_vessel_resumption requires a real date, so we set the
                    # hour manually after instantiation via _resumption_seed_by_vessel).
    ...
    # Structure: {vessel_name: {day_number (int): mother_name (str)}}
    # ZeeZee is unaffected — her schedule is controlled by ZEEZEE_SCHEDULE above.
    _ddo_orig = {}
    if hasattr(mod, "DAUGHTER_DISCHARGE_OVERRIDES"):
        _ddo_orig = {k: dict(v) for k, v in mod.DAUGHTER_DISCHARGE_OVERRIDES.items()}
    ...
    if daughter_discharge_overrides_json and hasattr(mod, "DAUGHTER_DISCHARGE_OVERRIDES"):
        try:
            ...
            if isinstance(_raw_ddo, dict):
                for _vn, _day_map in _raw_ddo.items():
                    if isinstance(_day_map, dict):
                        mod.DAUGHTER_DISCHARGE_OVERRIDES[_vn] = {
                            int(_d): str(_m) for _d, _m in _day_map.items()
                        }
    ...
    if vessel_states_json:
        ...
        for v in sim.vessels:
            if v.name in _resumption_seed_by_vessel:
                ...
                if _seed_indef:
                    # Override resumption_hour with indefinite sentinel —
                    # vessel will never satisfy t >= resumption_hour so it
                    # sits idle for the entire run and never enters the JMP.
    ...
    for v in sim.vessels:
        ...
        elif v.name == "Bagshot" and "Bagshot" not in _vs_override and v.name not in _resumption_seed_names:
        ...
        # This is intentionally hardcoded and must NOT be changed by the
        # _POINT_B_START_VESSELS block or any other default-state logic.
        elif v.name == "Watson" and "Watson" not in _vs_override and v.name not in _resumption_seed_names:
...
def run_optimizer(base_params_json: str):
    ...
    # (threshold_frac, min_space_bbl) — determines how aggressively T4/T5 drain SJ.
    # (0.15, 25k) = aggressive: drain as soon as gate met and any mother has 25k space
    # (0.25, 50k) = conservative: current default, drain only with meaningful headroom
    ...
    def _score(S, log_df, tl_df):
        ...
        # SanJulian utilisation (5%) ensures the optimizer finds params that keep
        # SanJulian flowing and not holding volume for extended periods.
        composite = (
            crash_score * 0.27
            + overflow_score * 0.40
            + idle_score * 0.14
        ...
        if sj_high_frac > 0.30: bottlenecks.append(f"SanJulian holding volume too long ({sj_high_frac*100:.0f}% of time ≥70% full)")
    ...
    all_results = []
    try:
        for rank, (dsf, pft, expw, (bstart, bend), (sj_thr, sj_spc)) in enumerate(_grid, 1):
            try:
    ...
    if not all_results:
        ...
        _log, _tl, S = run_sim(
            ...
            pgm=base.get("pgm",10_000),
            bryanston=0, alkebulan=0, greeneagle=0, sanjulian=0,
...
def chart_mothers(tl_df, export_trigger, cap_by_name, export_trigger_by_name=None):
    ...
    # Export trigger lines — Bryanston uses the base trigger,
    # Alkebulan and GreenEagle have their own higher triggers.
    _trig_by = export_trigger_by_name or {}
    _bry_trig = int(_trig_by.get("Bryanston", export_trigger))
    _alk_trig = int(_trig_by.get("Alkebulan", export_trigger))
    ...
    if _alk_trig != _bry_trig:
        fig.add_hline(y=_alk_trig,
                      line=dict(color="#ff9955", dash="dash", width=1.5),
                      annotation_text=f"Alkebulan export trigger ({_alk_trig:,} bbl)",
    ...
    _cap_ag  = int(cap_by_name.get("Alkebulan", _cap_bry))
    ...
    fig.add_hline(y=_cap_ag, line=dict(color="#64748b", dash="dot"),
                  annotation_text=f"Alkebulan/GreenEagle capacity ({_cap_ag:,} bbl)",
                  annotation_font_color="#cbd5e1")
    fig.add_hline(y=_cap_sj, line=dict(color="#94a3b8", dash="dot"),
                  annotation_text=f"SanJulian capacity ({_cap_sj:,} bbl)",
                  annotation_font_color="#e2e8f0")
    fig.update_layout(height=320, margin=_MARGIN,
                      title="BIA Mother Vessels — Volume (SanJulian: transload-only, no export)",
                      **_DARK, legend=dict(bgcolor="#ffffff"))
    fig.update_yaxes(tickformat=",", **_GRID, title_text="bbl")
    fig.update_xaxes(**_GRID)
    return fig
...
def chart_gantt(tl_df, vessel_names, log_df=None):
    ...
    _CATS = {
        ...
        "Idle"         : ("#475569", "💤 Idle"),
        "Transloading" : ("#000000", "⇄ SanJulian Transloading"),
    }
    ...
    if _sj_has_col:
        ...
        if not sub_sj.empty:
            ...
            for (cat, _blk), grp in sub_sj.groupby(["cat", "blk"], sort=False):
                ...
                hover   = (
                    f"<b>SanJulian</b>  ·  <b>⇄ Transloading</b><br>"
                    f"{lbl}<br>"
    ...
    _all_tick_text    = [
        ...
        ["<span style=\"color:#ffffff;font-weight:700\">⇄ SanJulian</span>"]
        if _sj_has_col else []
    )
...
def render_mother_cards(gs_vols, manual_mother, mod):
    ...
    for i, (mn, mk) in enumerate([
        ("Bryanston","bryanston"), ("Alkebulan","alkebulan"),
...
def main():
    ...
    MOTHER_CAP_BY_NAME = getattr(mod, "MOTHER_CAPACITY_BY_NAME", {
        ...
        "Alkebulan": int(mod.MOTHER_CAPACITY_BBL),
    ...
    with h2:
        ...
        st.caption(
            ...
            "4 mother vessels (Bryanston, Alkebulan, GreenEagle, SanJulian) · "
    ...
    _mother_opts = list(getattr(mod, "MOTHER_NAMES", ["Bryanston", "Alkebulan", "GreenEagle", "SanJulian"]))
    ...
    with _ctrl_day1:
        ...
        with _day1_right:
            with st.container():
                st.markdown("#### ⏸️ Vessel Resumption Dates")
                st.caption(
                    "Hold an existing vessel idle until a chosen date, then force it to "
                    "take loading priority at a specific storage on that day."
                )
                st.caption(
                    "Validation seed: resumed vessels are forced dormant from Day 1 and "
                    "only wake at 08:00 on the selected date for the assigned storage."
                )
            ...
            _sim_end   = (
                _sim_start + __import__("datetime").timedelta(days=sim_days - 1)
            ...
            with _vr_c2:
                _vr_storage = st.selectbox(
                    "Priority storage", _storage_opts, key="vr_storage_sel",
                    label_visibility="collapsed"
                )

            # Indefinite toggle: vessel held idle for the whole simulation run
            ...
            with _vr_c3:
                if not _vr_indefinite:
                    _vr_date = st.date_input(
                        ...
                        key="vr_date_sel",
                        label_visibility="collapsed",
                        help="Date at 08:00 when the vessel wakes and loads from the priority storage.",
                    )
                else:
                    _vr_date = None
                    st.markdown(
            ...
            with _vr_c4:
                if st.button("➕ Set", key="vr_add_btn", use_container_width=True):
                    # Sentinel date for indefinite: far beyond any sim horizon
                    _vr_date_str = "indefinite" if _vr_indefinite else str(_vr_date)
                    _vr_dict[_vr_vessel] = {
                        "name":       _vr_vessel,
                        "date":       _vr_date_str,
                        "storage":    _vr_storage,
                        "indefinite": _vr_indefinite,
                    }
            ...
            if _vr_dict:
                ...
                for _vr_key, _vr_entry in list(_vr_dict.items()):
                    _ta, _tb, _tc, _td = st.columns([2, 2, 2, 1])
                    with _ta:
                    ...
                    with _td:
                        if st.button("✕", key=f"vr_rm_{_vr_key}", use_container_width=True):
                            ...
                            st.session_state["vessel_resumptions"] = _vr_dict
                            st.rerun()
                if st.button("🗑️ Clear all resumptions", key="vr_clear_all"):
                    st.session_state["vessel_resumptions"] = {}
                    st.rerun()
            else:
                st.info("No resumptions set.", icon="ℹ️")
    ...
    with _ctrl_inputs:
        ...
        # ── Mother Vessel Unavailability Windows ─────────────────────────────
        st.markdown("---")
        st.markdown("### 🚫 Mother Vessel Unavailability Windows")
        st.caption(
            "Schedule a period when a primary mother vessel is unavailable (e.g. dry-dock, "
            "maintenance, redeployment). During the window she cannot receive daughters or "
            "SanJulian transloads — traffic is automatically rerouted to available mothers. "
            "The vessel resumes normal operations when the window ends."
        )
        ...
        # All mothers — including SanJulian — can be marked unavailable for
        # dry-dock / maintenance / redeployment windows.  SanJulian does not
        # run export voyages, but she can still be taken out of service.
...
""")
    ...
    _context_chips.append(
        (f'Resumptions: {len(st.session_state.get("vessel_resumptions", {}))}' if st.session_state.get("vessel_resumptions") else 'No resumptions',
         'context-chip' if st.session_state.get("vessel_resumptions") else 'context-chip')
    )
    _context_chips.append(
        ('Resumption validation seed active' if st.session_state.get("vessel_resumptions") else 'No dormant resumption seed',
    ...
    if not gs_vols:
        with st.expander("✏️ Enter 08:00 storage & mother volumes",
            ...
            mv = st.columns(4)
            for j,mn in enumerate(["Bryanston","Alkebulan","GreenEagle","SanJulian"]):
                with mv[j]:
                    _mother_cap_ui = int(MOTHER_CAP_BY_NAME.get(mn, mod.MOTHER_CAPACITY_BBL))
                    manual_mother[mn.lower()] = st.number_input(
                        mn, 0, _mother_cap_ui, 0, step=10_000, key=f"mv_{mn}")
            ...
            _mother_api_defaults = {"Bryanston": 32.00, "Alkebulan": 32.00, "GreenEagle": 32.00, "SanJulian": 32.00}
            manual_mother_api = {}
            for _j2, _mn2 in enumerate(["Bryanston", "Alkebulan", "GreenEagle", "SanJulian"]):
    ...
    if missing_vessels:
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                with rc[3]:
                    _cargo_default = (
                        _cargo_cap if "Discharging" in _sel_stat or "Loading" in _sel_stat
                        else 0
                    )
                    cg = st.number_input(
                ...
                _bia_sim_val = lc   # e.g. "Bryanston", "Alkebulan", …
    ...
    st.markdown("---")
    st.markdown("**🚢 Force Export Departure**")
    st.caption(
        ...
        "Use this to control *when* a mother sails independently of her fill level."
    )
    ...
    if _forced_deps:
        for _fi, _fe in enumerate(_forced_deps):
            ...
            with _fec2:
                ...
                st.markdown(
                    f'<span style="font-size:11px;color:#1e40af">🚢 Force sail on {_fdate_disp}</span>',
                    unsafe_allow_html=True,
                )
            with _fec3:
    ...
    params = dict(
        ...
        pgm       = _p("pgm",       manual_storage, "PGM",      SCAP.get("PGM", 40_000)//2),
        ...
        sanjulian = _p("sanjulian", manual_mother, "sanjulian", 0),
        ...
        sanjulian_api  = gs_vols.get("sanjulian_api",  manual_mother_api.get("sanjulian",  0.0)),
    )
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    all_moth = sum(S.get(f"final_{n}",0) for n in ["Bryanston","Alkebulan","GreenEagle","SanJulian"])
    ...
    _all_moth_vol = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","Alkebulan","GreenEagle","SanJulian"])
    _blended_moth = (
        sum(S.get(f"final_{n}", 0) * _mapi.get(n, 0.0)
            for n in ["Bryanston","Alkebulan","GreenEagle","SanJulian"]) / _all_moth_vol
    ...
    _final_mothers = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","Alkebulan","GreenEagle","SanJulian"])
...
<div class="summary-card">
  ...
  <ul>
    ...
    <li><strong>Mother tankers</strong> — 4 vessels (Bryanston, Alkebulan, GreenEagle, SanJulian)
  ...
  <ul>
        ...
        with mc[i]:
            ...
            if mn == "SanJulian":
                ...
                if _sj_active_target == mn and _sj_active_status:
                    _phase_labels = {
                        "BERTHING_B":    "⇄ SanJulian berthing",
                        "HOSE_CONNECT_B":"⇄ SanJulian hose connecting",
                        "DISCHARGING":   "⇄ SanJulian discharging",
                        "CAST_OFF_B":    "⇄ SanJulian cast-off",
                    }
                    _ph = _phase_labels.get(_sj_active_status,
                                            f"⇄ SanJulian {_sj_active_status}")
                    _sj_badge = (
    ...
    with v2:
        ...
        for vn in vnames:
            ...
            _loaded_bbl = int(vl.loc[vl.Event=="LOADING_START", "Detail"].map(_extract_cargo_bbl).sum())
            _discharged_bbl = int(vl.loc[vl.Event=="DISCHARGE_START", "Detail"].map(_extract_cargo_bbl).sum())
    ...
    for vtab, vn in zip(vtabs, vnames):
        with vtab:
            ...
            vcap  = mod.VESSEL_CAPACITIES.get(vn,mod.DAUGHTER_CARGO_BBL)
            _vol_loaded = int(loads["Detail"].map(_extract_cargo_bbl).sum()) if not loads.empty else 0
            _vol_discharged = int(discs["Detail"].map(_extract_cargo_bbl).sum()) if not discs.empty else 0
            base  = VESSEL_COLORS.get(vn,"#aaa")
    ...
    extra  = [c for c in ["Chapel_bbl","JasmineS_bbl","Westmore_bbl",
                           ...
                           "Alkebulan_api","Bryanston_api","GreenEagle_api",
    ...
    _pt_ao          = set(getattr(mod, "POINT_A_ONLY_VESSELS",      {"Berners"}))
    ...
    # Forces a named daughter vessel to discharge to a specific mother vessel
    # on a particular simulation day.  ZeeZee is not listed here — her schedule
    # is managed separately in the ZeeZee Discharge Override Panel below.
    _ddo_state = st.session_state.get("daughter_discharge_overrides", {})
    _ddo_count  = sum(len(v) for v in _ddo_state.values())
    ...
    with st.expander(
        ...
        st.markdown(
            '<div style="font-size:12px;color:#64748b;margin-bottom:10px">'
            'Force any <b>daughter vessel</b> to discharge to a specific '
            '<b>mother vessel</b> on a given simulation day. '
            'If the nominated mother is not yet available, the daughter waits '
            'and retries each hour until she becomes feasible. '
        ...
        _ddo_c1, _ddo_c2, _ddo_c3, _ddo_c4 = st.columns([2, 2, 2, 1])
        with _ddo_c1:
            _ddo_vessel = st.selectbox(
                "Daughter vessel",
                options=_ddo_all_vessels,
                key="ddo_vessel",
                help="Daughter vessel to override.",
            )
        with _ddo_c2:
            _ddo_day = st.number_input(
                "Sim day (1-based)",
                min_value=1,
                max_value=max(_ddo_sim_days, 1),
                value=1,
                step=1,
                key="ddo_day",
                help="Simulation day on which the override applies (Day 1 = first day of run).",
            )
        with _ddo_c3:
            _ddo_mother = st.selectbox(
                ...
                key="ddo_mother",
                help="Mother vessel this daughter must discharge to on the selected day.",
            )
        with _ddo_c4:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("➕ Add", key="ddo_add_btn", use_container_width=True):
                _upd_ddo = {k: dict(v) for k, v in
                            st.session_state.get("daughter_discharge_overrides", {}).items()}
                if _ddo_vessel not in _upd_ddo:
                    _upd_ddo[_ddo_vessel] = {}
                # day key stored as int (0-based internally, displayed 1-based)
                _upd_ddo[_ddo_vessel][int(_ddo_day) - 1] = str(_ddo_mother)
        ...
        if _ddo_state:
            ...
            for _ddo_vn, _ddo_day_map in sorted(_ddo_state.items()):
                for _ddo_dk, _ddo_mn in sorted(_ddo_day_map.items(), key=lambda x: int(x[0])):
                    _ddo_display_day = int(_ddo_dk) + 1
                    _dc1, _dc2 = st.columns([6, 1])
                    with _dc1:
                        st.markdown(
                            f'<span style="background:#064e3b;color:#fff;border-radius:4px;'
                            f'padding:2px 10px;font-size:11px;font-weight:700">{_ddo_vn}</span> '
                            f'<span style="font-size:12px;color:#374151">'
                            f' Day <b>{_ddo_display_day}</b> → discharge to '
                            f'<b>{_ddo_mn}</b></span>',
                            unsafe_allow_html=True,
                        )
                    with _dc2:
                        if st.button("✖", key=f"ddo_del_{_ddo_vn}_{_ddo_dk}",
                                     use_container_width=True):
                            _rm_ddo = {k: dict(v) for k, v in
                                       st.session_state.get("daughter_discharge_overrides", {}).items()}
                            if _ddo_vn in _rm_ddo:
                                _rm_ddo[_ddo_vn].pop(str(_ddo_dk), None)
                                _rm_ddo[_ddo_vn].pop(int(_ddo_dk), None)
                                if not _rm_ddo[_ddo_vn]:
                                    del _rm_ddo[_ddo_vn]
    ...
    _mother_cols  = ["Bryanston_bbl","Alkebulan_bbl","GreenEagle_bbl","SanJulian_bbl"]
    _storage_names = ["Chapel","JasmineS","Westmore","Duke","Starturn","PGM"]
    _mother_names  = ["Bryanston","Alkebulan","GreenEagle","SanJulian"]
    ...
    for _day in range(1, _total_sim_days + 1):
        ...
        _mapi_cols = ["Bryanston_api","Alkebulan_api","GreenEagle_api","SanJulian_api"]
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    _html.append(
        ...
        '<th colspan="5" class="sec-hdr-cell">Discharging Plan</th>'
        '</tr>'
    )
    ...
    _html.append(
        ...
        # BIA arrival
        '<th>Vessel (ETA)</th>'
        # Discharge per mother (4) + ZeeZee third-party (1)
        '<th>Bryanston</th><th>Alkebulan</th><th>GreenEagle</th><th>SanJulian</th>'
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _lcell(storage):
            ...
            for r in entries:
                ...
                chips.append(_chip(r["Vessel"], _label))
            inner = "<br>".join(chips)
        ...
        def _zz_dcell():
            ...
            if not _zz_entries and not _zz_waiting:
                ...
                _zz_present = [
                    r for r in _d[_d["Vessel"] == "ZeeZee"].to_dict("records")
                ]
                if _zz_present:
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _fload(name):
            ...
            for r in entries:
                ...
                chips.append(_chip(r["Vessel"], _f_label))
            inner = "<br>".join(chips)
            return f"<td>{inner}</td>"
        ...
        # BIA arrivals — use vessel colour (_chip) to match on-screen renderer
        _ffwy = _fde["fairway"]
        _fbia_arr = "<br>".join(
            _chip(r["Vessel"], f"{r['Vessel']} ({r['Time'][11:16]})")
            for r in _ffwy
        ...
        def _fdcell(mother):
            ...
            if _at_exp:
                ...
                stk2 = f'<span style="font-size:9px;color:#bfdbfe;display:block;margin-bottom:2px">Stock: {_kkk(ms)}{_api_bit}</span>'
                inner = "<br>".join(
                    _chip(r["Vessel"], f"{r['Vessel']} | {r['Time'][11:16]} | {_kkk(_parse_cargo(r['Detail']))}")
                    for r in entries
                )
                return f'<td style="background:#1e3a5f">{_exp_label}{stk2}{inner}</td>'
            stk = f'<span style="font-size:9px;color:#94a3b8">Stock: {_kkk(ms)}{_api_bit}</span>'
...
<div style="font-size:9px;color:#94a3b8;margin-top:8px">Generated: {_dt.datetime.now().strftime("%Y-%m-%d %H:%M")} | Tanker Operations v5</div>
    ...
    with _dc2:
        ...
        st.download_button(
            f"📥 Download Full Plan as CSV ({_total_sim_days} days)",
            data=_jmp_csv,
    ...
    with d3:
        ...
        for mn in ["Bryanston","Alkebulan","GreenEagle","SanJulian"]:
