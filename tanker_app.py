...
Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  ...
  "extraction_notes": ""
        ...
        # ── API Key input ──────────────────────────────────────────────────────
        _api_key_col, _pdf_uploader_col = st.columns([2, 3])
        with _api_key_col:
            _anthropic_api_key = st.text_input(
                "Anthropic API Key",
                type="password",
                key="pdf_anthropic_api_key",
                placeholder="sk-ant-…",
                help=(
                    "Your Anthropic API key — used only for PDF extraction. "
                    "Get one at console.anthropic.com. "
                    "The key is never stored beyond this session."
                ),
            )

        with _pdf_uploader_col:
            _stock_pdf = st.file_uploader(
                "Daily Stock Update PDF",
                type=["pdf"],
                key="stock_report_pdf",
                help="Upload the Daily Stock Update PDF. The AI will extract all vessel volumes and positions.",
                disabled=not _anthropic_api_key,
            )
            if not _anthropic_api_key:
                st.caption("⬅️ Enter your Anthropic API key first to enable PDF upload.")

        # ── Status banner (shown below uploader row) ───────────────────────────
        ...
        # Confirmed-apply green banner (persists after rerun)
        if st.session_state.get("vp_confirmed") and st.session_state.get("_pdf_apply_summary"):
            _apply_sum = st.session_state["_pdf_apply_summary"]
            st.success(
                f"✅ Stock report applied — {_apply_sum}. "
                "Storage volumes, mother stocks and vessel positions are now loaded into the simulation. "
        ...
        if _stock_pdf is not None and _anthropic_api_key:
            _pdf_bytes = _stock_pdf.read()
            _pdf_b64   = binascii.b2a_base64(_pdf_bytes).decode().strip()
            _pdf_hash  = hashlib.md5(_pdf_bytes).hexdigest()

            # Only re-extract if it's a new file
            if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                with st.spinner("📄 Extracting data from stock report with AI…"):
                    try:
                        import urllib.request as _urlreq
                        _req_body = json.dumps({
                            "model": "claude-sonnet-4-20250514",
                            "max_tokens": 1500,
                            "system": _IMPORTER_SYSTEM_PROMPT,
                            "messages": [{
                                "role": "user",
                                "content": [
                        ...
                        _req = _urlreq.Request(
                            ...
                            headers={
                                ...
                                "x-api-key": _anthropic_api_key.strip(),
                                "anthropic-version": "2023-06-01",
                                "anthropic-beta": "pdfs-2024-09-25",
                            },
                            method="POST",
                        )
                        with _urlreq.urlopen(_req, timeout=90) as _resp:
                            _resp_data = json.loads(_resp.read())

                        # Check for API-level errors returned in the body
                        if "error" in _resp_data:
                            raise ValueError(_resp_data["error"].get("message", str(_resp_data["error"])))

                        _resp_text = "".join(
                            b.get("text", "") for b in _resp_data.get("content", [])
                        )
                        # Strip any accidental markdown fences
                        _resp_text = _resp_text.strip()
                        if _resp_text.startswith("```"):
                            _resp_text = "\n".join(_resp_text.split("\n")[1:])
                        if _resp_text.endswith("```"):
                            _resp_text = "\n".join(_resp_text.split("\n")[:-1])

                        _extracted = json.loads(_resp_text)
                        st.session_state["_pdf_extracted"]   = _extracted
                        st.session_state["_pdf_last_hash"]   = _pdf_hash
                        st.session_state["_pdf_import_status"] = {
                            "type": "ok",
                            "msg": (
                                f"✅ PDF read — report dated {_extracted.get('report_date', '?')}. "
                                "Review the data below and press **Confirm & Apply** when ready."
                            ),
                        }
                        st.rerun()
                    except Exception as _pdf_err:
                        _err_msg = str(_pdf_err)
                        # Give a friendlier message for common auth errors
                        if "401" in _err_msg or "Unauthorized" in _err_msg.lower():
                            _err_msg = "API key rejected (401 Unauthorized). Check your key at console.anthropic.com."
                        elif "403" in _err_msg:
                            _err_msg = "API key does not have permission for this model (403 Forbidden)."
                        st.session_state["_pdf_import_status"] = {
                            "type": "error",
                            "msg": f"Extraction failed: {_err_msg}",
                        }
                        st.rerun()
