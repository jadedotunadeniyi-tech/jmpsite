...
""")
        ...
        tide_file = st.file_uploader(
            ...
            help=(
                ...
                "Vessels cross the breakwater only if high tide > 1.6 m within daylight.\n"
        ...
        if tide_file is not None:
            ...
            st.caption("🌊 Breakwater crossing: high tide > 1.6 m · daylight only (06:00–18:00)")
        else:
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
        ...
        try:
            ...
            if not _tide_ok:
                ...
                for _day in range(_sim_days_t):
                    ...
                    if not _slots:
                        _daily_rows.append({
                            ...
                            "Afternoon Tide": "—",
                            "Evening Tide": "—",
                            "Crossing Windows (daylight high tide >1.6m)": "No tidal data for this day",
                            "Windows": 0,
                        })
                        continue
                    ...
                    # Crossing windows: consecutive slots where local high tide is
                    # above threshold and inside daylight.
                    _DSTART = 6
                    _DEND   = 18
                    _windows = []
                    _windows_by_bucket = {
                    ...
                    for _i, _h in enumerate(_sorted_slots):
                        ...
                        _prev_h = _slots[_sorted_slots[_i - 1]] if _i > 0 else None
                        _next_h = _slots[_sorted_slots[_i + 1]] if _i < (len(_sorted_slots) - 1) else None
                        _is_high_tide = (
                            _slots[_h] > _tide_min_m
                            and (_prev_h is None or _slots[_h] >= _prev_h)
                            and (_next_h is None or _slots[_h] >= _next_h)
                        )
                        _valid = _is_high_tide and (_DSTART <= _hod < _DEND)
                        if _valid and _win_start is None:
                            ...
                            _bucket = _tide_bucket(_win_start % 24)
                            if _bucket:
                    ...
                    _daily_rows.append({
                        ...
                        "Crossing Windows (daylight high tide >1.6m)": _win_str,
                ...
                st.markdown(
                    ...
                    f'🌊 <b>Breakwater crossing rule:</b> vessels may only depart when high tide > '
                    f'<b>{_tide_min_m} m</b> AND within daylight (<b>{_DSTART:02d}:00–{_DEND:02d}:00</b>). '
                    f'Windows are split into <b>Morning Tide</b>, <b>Afternoon Tide</b>, and <b>Evening Tide</b>. '
                ...
                if _sd_slots:
                    ...
                    _fig_t.add_hline(
                        ...
                        annotation_text=f"Min crossing {_tide_min_m} m",
                    ...
                    # Valid crossing zones (local high tide > threshold AND daylight) — green fill
                    _in_zone = False
                    _zone_x0 = None
                    for _i, (_hod, _hgt) in enumerate(zip(_sd_hods, _sd_heights)):
                        _prev_h = _sd_heights[_i - 1] if _i > 0 else None
                        _next_h = _sd_heights[_i + 1] if _i < (len(_sd_heights) - 1) else None
                        _is_high = (
                            _hgt > _tide_min_m
                            and (_prev_h is None or _hgt >= _prev_h)
                            and (_next_h is None or _hgt >= _next_h)
                        )
                        _ok = _is_high and _DSTART <= _hod < _DEND
                    ...
                    _cwin_txt = _sel_row["Crossing Windows (daylight high tide >1.6m)"]
                    st.caption(
    ...
    if not run_opt:
        ...
        _tide_html_text  = (
            "🌊 Tidal Constraint Active — breakwater high tide > 1.6 m · 2h from SanBarth · 4h from BIA"
            if _tide_bytes else
...
<div class="summary-card">
  ...
  <ul>
    ...
    SanBarth→BIA breakwater require high tide &gt;1.6 m during daylight. Departures must
