...
def main():
    ...
    with _ctrl_data:
        ...
        def _parse_stock_csv(file_bytes):
            ...
            # Discharge @ BIA cols: 18=GreenEagle  19=Bryanston  20=SanJulian
            # Positive = mother is RECEIVING; negative on SanJulian row = it is DISCHARGING
            _MOM_COLS = {18: "GreenEagle", 19: "Bryanston", 20: "SanJulian"}

            # Detect SanJulian (MT San Barth) discharge target from row 29
            # col 18 negative on SanJulian row = discharging to GreenEagle
            _sanjulian_target_mom = ""
            if _num(rows, 29, 18) < 0:
                _sanjulian_target_mom = "GreenEagle"
            elif _num(rows, 29, 19) < 0:
                _sanjulian_target_mom = "Bryanston"
            ...
            for _vname, _r in _DAUGHTER_ROWS.items():
                ...
                if _vname == _ibom_loader or (_vname == "Bedford" and "ibom" in _ibom_comment):
                    # Vessel loading at Ibom buoy — its own ROB shows 0 (mid-loading).
                    # Use the Ibom field stock as the vessel's current accumulated cargo.
                    _status = "PF_LOADING"
                    _stor   = "Ibom"
                    _cargo  = _ibom_stock  # e.g. Bedford = 10,982 bbl from Ibom row
                    ...
                    _status = "SAILING_D_CHANNEL"
                      ...
                      or "proceed to asaramatoru" in _comment
            ...
            # Apply SanJulian discharge target to mother_volumes entry so
            # the confirm step knows SanJulian is discharging to that mother
            _sj_dv = {
                "name":             "SanJulian",
                "cargo_bbl":        mother_volumes.get("SanJulian", 0),
                "status":           "DISCHARGING" if _sanjulian_target_mom else "IDLE_B",
                "assigned_storage": "",
                "target_mother":    _sanjulian_target_mom,
                "notes":            f"Discharging to {_sanjulian_target_mom}" if _sanjulian_target_mom else "",
            }
            ...
            return {
                ...
                "sanjulian_vessel":   _sj_dv,
                "ibom_loading_vessel": _ibom_loader,
                "extraction_notes":   (
                    f"Parsed directly from CSV. Ibom stock: {_ibom_stock:,} bbl. "
                    + (f"SanJulian discharging to {_sanjulian_target_mom}." if _sanjulian_target_mom else "")
                ),
            }
        ...
        if st.session_state.get("_pdf_extracted"):
            ...
            with st.expander("📝 Review & Adjust Extracted Data", expanded=True):
                ...
                _sj_dv = _ex.get("sanjulian_vessel", {})
                if _sj_dv:
                ...
                with _confirm_col:
                    if st.button(
                        ...
                        # Add SanJulian vessel state (discharge pairing from CSV)
                        _sj_dv = _ex.get("sanjulian_vessel")
                        if _sj_dv:
                            _sj_mom = _sj_dv.get("target_mother", "") or ""
                            _vp_states_pdf["SanJulian"] = {
                                "status":         "DISCHARGING" if _sj_mom else "IDLE_B",
                                "cargo_bbl":      int(_sj_dv.get("cargo_bbl", 0)),
                                "location":       _sj_mom if _sj_mom else "SanJulian",
                                "target_storage": None,
