...
LOCATION_CATALOGUE = [
    ...
    # ── SanJulian — intermediate floating storage at BIA ──────────────────────
    # SanJulian is NOT a primary mother and never exports. Daughters berth here
    # only when all primary mothers are unavailable (at export or full).
    # Seeding a vessel here on Day 1 initialises it alongside SanJulian
    # with full intermediate-discharge semantics.
    {"display": "SanJulian (BIA — Intermediate)", "sim_value": "SanJulian",
     "field_zone": "BIA",
     "statuses": [
        ("DISCHARGING",            "⬇️ Discharge — to SanJulian intermediate storage"),
        ("HOSE_CONNECT_B",         "🔧 Hose connection underway (SanJulian)"),
        ("BERTHING_B",             "🔗 Berthing in progress (SanJulian)"),
        ("WAITING_BERTH_B",        "⏳ Arrived — waiting for SanJulian berth"),
        ("WAITING_MOTHER_CAPACITY","⏳ Waiting — SanJulian at capacity"),
        ("CAST_OFF_B",             "↩️ Discharge complete — cast off from SanJulian"),
        ("IDLE_B",                 "🟢 Idle alongside SanJulian"),
        ("WAITING_RETURN_STOCK",   "⏳ Waiting — return storage assignment"),
        ("WAITING_CAST_OFF",       "⏳ Discharge complete — awaiting cast-off window"),
    ]},
...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
            ...
            if v.name in vs:
                ...
                if _loc and _loc in _sp_map:
                    ...
                    if _loc in {"B", "Fairway", "Fairway Buoy"}:
                        v.target_point = "B"
                    # SanJulian is intermediate floating storage permanently at BIA
                    elif _loc == "SanJulian":
                        v.target_point    = "B"
                        v.assigned_mother = "SanJulian"
                ...
                # Accept both primary MOTHER_NAMES and "SanJulian" as valid assignments
                _valid_mothers = set(getattr(mod, "MOTHER_NAMES", [])) | {"SanJulian"}
                if _tm and _tm in _valid_mothers:
                ...
                if v.status in {"HOSE_CONNECT_B", "DISCHARGING"}:
                    ...
                    _valid_mothers_now = _mother_names_now | {"SanJulian"}
                    _selected_m = v.assigned_mother if v.assigned_mother in _valid_mothers_now else None
                    ...
                    if _xfr > 0 and _full_cargo > 0:
                        ...
                        if _selected_m == "SanJulian":
                            # Credit to SanJulian intermediate inventory
                            _prev_sj_bbl = sim.sanjulian_bbl
                            _prev_sj_api = sim.sanjulian_api
                            sim.sanjulian_bbl += _xfr
                            sim.sanjulian_total_received += _xfr
                            if sim.sanjulian_bbl > 0:
                                sim.sanjulian_api = (
                                    (_prev_sj_bbl * _prev_sj_api + _xfr * _vessel_api_val)
                                    / sim.sanjulian_bbl
                                )
                        else:
                            # Credit already-transferred volume to primary mother
                    ...
                    # Reserve berth for the remaining window
                    _berth_end = v.next_event_time + (0 if v.status == "DISCHARGING" else _disch_h)
                    if _selected_m == "SanJulian":
                        sim.sanjulian_berth_free_at = max(
                            sim.sanjulian_berth_free_at, _berth_end)
                    else:
    ...
    summary = dict(
        ...
        **{f"final_{k}": float(v) for k, v in sim.mother_bbl.items()},
        final_SanJulian            = float(getattr(sim, "final_sanjulian_bbl",        0.0)),
        sanjulian_total_received   = float(getattr(sim, "sanjulian_total_received",   0.0)),
        sanjulian_total_transloaded= float(getattr(sim, "sanjulian_total_transloaded", 0.0)),
        sanjulian_api              = round(float(getattr(sim, "final_sanjulian_api",   0.0)), 2),
...
def chart_mothers(tl_df, export_trigger, cap):
    ...
    # SanJulian — dashed amber line, no fill (intermediate storage, not a mother)
    if "SanJulian_bbl" in tl_df.columns:
        fig.add_trace(go.Scatter(
            x=tl_df.Time, y=tl_df["SanJulian_bbl"],
            name="SanJulian (intermediate)",
            line=dict(color="#f39c12", width=2, dash="dash"),
        ))
        fig.add_hline(y=450_000,
                      line=dict(color="#f39c12", dash="dot", width=1),
                      annotation_text="SanJulian cap (450,000 bbl)",
    ...
    fig.add_hline(y=cap, line=dict(color="#7f1d1d", dash="dot"),
                  annotation_text=f"Mother capacity ({cap:,} bbl)",
                  annotation_font_color="#fca5a5")
    fig.update_layout(height=320, margin=_MARGIN,
                      title="BIA — Mother Vessels & SanJulian Intermediate Storage",
                      **_DARK, legend=dict(bgcolor="#ffffff"))
    fig.update_yaxes(tickformat=",", **_GRID, title_text="bbl")
    fig.update_xaxes(**_GRID)
    return fig
...
def chart_gantt(tl_df, vessel_names, log_df=None):
    ...
    # Day grid lines + alternating week shading built as a single shapes list
    # (add_vline/add_vrect per-day causes 300+ DOM mutations on long runs)
...
def render_mother_cards(gs_vols, manual_mother, mod):
    """Render one status card per primary mother + SanJulian intermediate storage."""
    # 3 primary mothers + SanJulian = 4 columns
    cols = st.columns(4)
    ...
    # SanJulian — intermediate floating storage card (amber, no export label)
    _sj_cap  = int(getattr(mod, "SANJULIAN_CAPACITY_BBL", 450_000))
    _sj_bbl  = int(gs_vols.get("sanjulian", 0) or 0)
    _sj_pct  = max(4, min(100, int(_sj_bbl / _sj_cap * 100))) if _sj_cap else 0
    _sj_col  = "#f39c12"
    with cols[3]:
        st.markdown(f"""
        <div class="vcard" style="border-left-color:{_sj_col}">
          <div class="vcard-name" style="color:{_sj_col}">⛴️ SanJulian</div>
          <div style="font-size:10px;color:{_sj_col};font-weight:700;margin-bottom:3px">
            INTERMEDIATE STORAGE · No Export
          </div>
          <div style="font-size:22px;font-weight:700;color:#f0f6fc;margin:4px 0">
            {_sj_bbl:,} <span style="font-size:12px;color:#484f58">bbl</span>
          </div>
          <div style="font-size:11px;color:#484f58;margin-bottom:5px">
            {_sj_pct}% of {_sj_cap:,} bbl capacity
          </div>
          <div class="vcard-bar-bg" style="height:8px;margin-bottom:6px">
            <div class="vcard-bar-fg" style="background:{_sj_col};width:{_sj_pct}%;height:8px"></div>
          </div>
          <span style="font-size:11px;color:{_sj_col}">⇄ Transloads to primary mothers</span>
        </div>
        """, unsafe_allow_html=True)
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    _sj_kpi  = S.get("final_SanJulian", 0.0)
    with k2[3]: kpi("Final All Storage", f"{all_stor:,.0f} bbl")
    with k2[4]: kpi("Final Mothers + SanJulian",
                    f"{all_moth + _sj_kpi:,.0f} bbl",
                    sub=f"Mothers {all_moth:,.0f} · SJ {_sj_kpi:,.0f}")
    ...
    _final_sanjulian  = S.get("final_SanJulian", 0.0)
    _sj_rcvd_narr     = S.get("sanjulian_total_received",    0.0)
    _sj_xld_narr      = S.get("sanjulian_total_transloaded", 0.0)
...
<div class="summary-card">
  ...
  across all five fields. Primary mother vessels retained <strong>{_final_mothers:,.0f} bbl</strong>{
  f" and SanJulian intermediate storage held <strong>{_final_sanjulian:,.0f} bbl</strong> (received {_sj_rcvd_narr:,.0f} bbl · transloaded {_sj_xld_narr:,.0f} bbl to mothers)" if _final_sanjulian > 0 or _sj_rcvd_narr > 0 else ""
  }.
...
<div class="summary-card">
  ...
  <ul>
    ...
    # SanJulian intermediate storage KPI
    with mc[3]:
        _sj_fv   = S.get("final_SanJulian", 0.0)
        _sj_rcvd = S.get("sanjulian_total_received",    0.0)
        _sj_xld  = S.get("sanjulian_total_transloaded", 0.0)
        _sj_api  = S.get("sanjulian_api", 0.0)
        _sj_cap_pct = int(_sj_fv / 450_000 * 100) if _sj_fv > 0 else 0
        _sj_api_txt = f" · API {_sj_api:.2f}°" if _sj_fv > 0 else ""
        kpi("⛴️ SanJulian (Intermediate)",
            f"{_sj_fv:,.0f} bbl",
            sub=f"{_sj_cap_pct}% full{_sj_api_txt} · rcvd {_sj_rcvd:,.0f} · xld {_sj_xld:,.0f}")
    ...
    extra  = [c for c in ["Chapel_bbl","JasmineS_bbl","Westmore_bbl",
                           "Duke_bbl","Starturn_bbl","Mother_bbl","SanJulian_bbl",
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        # ── Discharge plan cells — one column per mother/SanJulian ──────────────
        def _dcell(mother):
            ...
            # SanJulian uses its amber brand colour; primary mothers use MOTHER_COLORS
            _cell_color = "#f39c12" if mother == "SanJulian" else _mc.get(mother, "#94a3b8")
            _lbl_extra  = " ⛴️" if mother == "SanJulian" else ""
            if not entries:
            ...
            stk = f'<span style="font-size:9px;color:#64748b;display:block;margin-bottom:2px">Stock: {_kkk(ms)}{_lbl_extra}{_api_bit}</span>'
            inner = "<br>".join(
                _chip(r["Vessel"],
                      f"{r['Vessel']} | {r['Time'][11:16]} | {_kkk(_parse_cargo(r['Detail']))}",
                      bg=_cell_color if mother == "SanJulian" else None)
                for r in entries
            )
...
<div style="font-size:9px;color:#94a3b8;margin-top:8px">Generated: {_dt.datetime.now().strftime("%Y-%m-%d %H:%M")} | Tanker Operations v5</div>
    ...
    with d3:
        ...
        rows.append(["Final SanJulian (bbl)",                S.get("final_SanJulian", 0)])
        rows.append(["SanJulian Total Received (bbl)",       S.get("sanjulian_total_received", 0)])
        rows.append(["SanJulian Total Transloaded (bbl)",    S.get("sanjulian_total_transloaded", 0)])
        for rec in recs:
