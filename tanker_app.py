...
STATUS_LABELS = {
    ...
    "WAITING_RETURN_STOCK"    : "BIA — Waiting, return stock low",
...
LOCATION_CATALOGUE = [
    ...
    {"display": "Sailing → Bryanston (A/C outbound)", "sim_value": "En Route SanBarth→BIA",
     ...
     "statuses": [
        ...
        ("WAITING_RETURN_STOCK","⏳ Holding — return storage stock too low"),
    ]},
    ...
    {"display": "BIA — Fairway Buoy",  "sim_value": "Fairway",
     ...
     "statuses": [
        ...
        ("WAITING_MOTHER_RETURN",  "⏳ Waiting — mother vessel away at export"),
        ("WAITING_MOTHER_CAPACITY","⏳ Waiting — mother vessel full"),
        ("WAITING_RETURN_STOCK",   "⏳ Waiting — insufficient return stock"),
        ("WAITING_DAYLIGHT",       "🌙 Waiting — daylight window"),
...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    for v in sim.vessels:
        ...
        elif v.name == "Woodstock" and "Woodstock" not in _vs_override:
            # Woodstock starts loading at Duke (Point D) from 0 bbl already on board.
            # Timing, storage deduction and berth reservation are handled by the
            # generic LOADING partial-cargo block that runs after all defaults.
            _wk_cap    = getattr(mod, "VESSEL_CAPACITIES", {}).get("Woodstock",
                         getattr(mod, "DAUGHTER_CARGO_BBL", 85_000))
            _duke_rate = getattr(mod, "DUKE_LOAD_RATE_BPH", 3_500)
            v.status              = "LOADING"
            v.target_point        = "D"
            v.assigned_storage    = "Duke"
            v.cargo_bbl           = 0                          # loading from scratch
            v.assigned_load_hours = _wk_cap / _duke_rate      # full load duration (overwritten by universal pass)
            v._voyage_assigned    = True
            v.current_voyage      = 1
    ...
    # defaults (e.g. Woodstock starting at Duke).
...
def run_optimizer(base_params_json: str):
    ...
    def _score(S, log_df, tl_df):
        ...
        n_vessels = len(S.get("vessel_names", []))

        # ── Throughput (30%) ────────────────────────────────────────────
        total_loaded  = S.get("loaded",    0)
        total_exported= S.get("exported",  0)
        max_possible  = sim_days * 24 * 2500 * 0.9  # rough upper bound
        tp_score = min(100.0, 100.0 * total_loaded / max(1, max_possible))

        # ── Idle (25%) — fraction of vessel-hours NOT idle ──────────────
        idle_sts = {"IDLE_A","IDLE_B","WAITING_BERTH_A","WAITING_BERTH_B",
                    "WAITING_STOCK","WAITING_DEAD_STOCK","WAITING_RETURN_STOCK",
                    "WAITING_MOTHER_CAPACITY","WAITING_MOTHER_RETURN"}
        total_slots = 0; idle_slots = 0
        for vn in S.get("vessel_names", []):
            if vn in tl_df.columns:
                col = tl_df[vn]
                total_slots += len(col)
                idle_slots  += col.isin(idle_sts).sum()
        idle_frac = idle_slots / max(1, total_slots)
        idle_score = max(0.0, 100.0 * (1.0 - idle_frac * 2.5))

        # ── Overflow (20%) ───────────────────────────────────────────────
        spilled = S.get("spilled", 0)
        overflow_score = max(0.0, 100.0 - min(100.0, spilled / max(1, total_loaded) * 500))

        # ── Export (15%) ─────────────────────────────────────────────────
        n_exports = S.get("exports", 0)
        export_score = min(100.0, n_exports * 20.0)   # 5 exports = 100

        # ── Turnaround (10%) — avg cycle hours ───────────────────────────
        ...
        composite = (tp_score * 0.30 + idle_score * 0.25 + overflow_score * 0.20
                     + export_score * 0.15 + turnaround_score * 0.10)

        # ── Bottlenecks ──────────────────────────────────────────────────
        bottlenecks = []
        if idle_frac > 0.35:  bottlenecks.append("High vessel idle time")
    ...
    for r in all_results:
        ...
        rows.append({
            ...
            "Throughput": sc["throughput_score"], "Idle": sc["idle_score"],
            "Overflow": sc["overflow_score"], "Export": sc["export_score"],
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Startup Day Manual Point B Nominations", expanded=startup_day_disable_point_b_priority):
            st.caption("Assign vessels to Point B mothers for Day 1. Vessels without a nomination will wait on Day 1.")
            _nom_choices = ["(No nomination)"] + _mother_opts
            _nom_defaults = {
                "Sherlock": "Bryanston",
                "Laphroaig": "Alkebulan",
                "Watson": "GreenEagle",
            }
            for _vn in _nom_vessels:
                _default_label = _nom_defaults.get(_vn, "(No nomination)")
                _idx = _nom_choices.index(_default_label) if _default_label in _nom_choices else 0
                _sel = st.selectbox(
                    f"{_vn} -> Point B Mother",
                    _nom_choices,
                    index=_idx,
                    key=f"startup_nom_{_vn}",
                )
                if _sel != "(No nomination)":
                    startup_day_manual_nominations[_vn] = _sel

        enable_point_b_startup_seed = st.toggle(
            "Validation Seed: Use startup manual nominations (full-load at BIA)",
            value=False,
            help=(
                "When enabled, every vessel nominated in 'Startup Day Manual Point B "
                "Nominations' starts at t=0 in HOSE_CONNECT_B with full cargo and the "
                "selected mother assignment."
            ),
        ...
        if enable_point_b_startup_seed:
            ...
            if not point_b_startup_seed:
                st.warning(
                    "Validation seed enabled, but there are no startup-day manual Point B nominations yet.",
                    icon="⚠️",
                )
...
""")
        ...
        tide_file = st.file_uploader(
            ...
            help=(
                ...
                "Vessels cross the breakwater only if tide is 1.7-2.5 m (0.1 m steps) within daylight.\n"
        ...
        if tide_file is not None:
            ...
            st.caption("🌊 Breakwater crossing: tide 1.7-2.5 m (0.1 m steps) · daylight only (06:00-18:00)")
        else:
        ...
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                with rc[1]:
                    ...
                    # Woodstock to Duke (loading)
                    if _is_ibom_vessel:
                        try: _default_loc_i = _loc_opts.index("Ibom (Offshore Buoy)")
                        except ValueError: pass
                    elif vn == "Woodstock":
                        try: _default_loc_i = _loc_opts.index("Duke (Awoba)")
                        except ValueError: pass
                ...
                with rc[2]:
                    ...
                    # Woodstock defaults to "Loading" at Duke
                    _stat_default_i = 0
                    if vn == "Woodstock":
                        _load_lbl = "⛽ Loading cargo"
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    # ==========================================================================
    sec("🌊 Tidal Prediction — Breakwater Crossing Windows")


    if _tide_bytes is None:
        st.info("ℹ️ No tidal file uploaded — upload a tidal CSV in the sidebar to see daily crossing windows and activate the breakwater constraint.")
    else:
        # Parse tide bytes directly — no sim module dependency
        try:
            _tide_allowed = {round(1.7 + 0.1 * i, 1) for i in range(9)}
            _tide_min_m = min(_tide_allowed)
            _tide_max_m = max(_tide_allowed)
            _DSTART     = 6
            _DEND       = 18
            ...
            def _parse_tide_bytes(raw_bytes, epoch_dt):
                ...
                sk = sorted(raw_pts)
                full = {}
                for slot in [x * 0.5 for x in range(int(sk[-1] * 2) + 2)]:
                    if slot in raw_pts:
            ...
            if not _tide_ok:
                ...
                for _day in range(_sim_days_t):
                    ...
                    if not _slots:
                        _daily_rows.append({
                            ...
                            "Afternoon Tide": "—",
                            "Evening Tide": "—",
                            "Crossing Windows (daylight tide 1.7-2.5m)": "No tidal data for this day",
                            "Windows": 0,
                        })
                        continue
                    ...
                    _period_ranges = {
                        "Morning Tide": (6, 23),
                        "Afternoon Tide": (12, 16),
                        "Evening Tide": (16, 18),
                    }

                    def _window_overlap_periods(_start_h, _end_h):
                        """Return all tide periods overlapped by a window [start, end)."""
                        _labels = []
                        for _name, (_pstart, _pend) in _period_ranges.items():
                            if _start_h < _pend and _end_h > _pstart:
                                _labels.append(_name)
                        return _labels

                    # Crossing windows: consecutive slots where tide level is in
                    # allowed crossing set and inside daylight.
                    _DSTART = 6
                    _DEND   = 18
                    _windows = []
                    _windows_by_bucket = {
                    ...
                    _win_start = None
                    _sorted_slots = sorted(_slots)
                    for _i, _h in enumerate(_sorted_slots):
                        _hod   = _h % 24   # hour of day
                        _valid_tide = round(_slots[_h], 1) in _tide_allowed
                        _valid = _valid_tide and (_DSTART <= _hod < _DEND)
                        if _valid and _win_start is None:
                            _win_start = _h
                        elif not _valid and _win_start is not None:
                            _win_end = _h
                            _ws = (_sim_epoch + _dt.timedelta(hours=_win_start)).strftime("%H:%M")
                            _we = (_sim_epoch + _dt.timedelta(hours=_win_end)).strftime("%H:%M")
                            _wh = round(_win_end - _win_start, 1)
                            _wlabel = f"{_ws}–{_we} ({_wh}h)"
                            _windows.append(_wlabel)
                            _ws_hod = _win_start % 24
                            _we_hod = _win_end % 24
                            if _we_hod <= _ws_hod:
                                _we_hod += 24
                            for _bucket in _window_overlap_periods(_ws_hod, _we_hod):
                                _windows_by_bucket[_bucket].append(_wlabel)
                            _win_start = None
                    if _win_start is not None:
                        _win_end = min(_day_end, max(_slots) + 0.5)
                        _ws = (_sim_epoch + _dt.timedelta(hours=_win_start)).strftime("%H:%M")
                        _we = (_sim_epoch + _dt.timedelta(hours=_win_end)).strftime("%H:%M")
                        _wh = round(_win_end - _win_start, 1)
                        _wlabel = f"{_ws}–{_we} ({_wh}h)"
                    ...
                    _win_str = "  ·  ".join(_windows) if _windows else "❌ No valid crossing window"
                    _morning_str = "  ·  ".join(_windows_by_bucket["Morning Tide"]) if _windows_by_bucket["Morning Tide"] else "—"
                    _afternoon_str = "  ·  ".join(_windows_by_bucket["Afternoon Tide"]) if _windows_by_bucket["Afternoon Tide"] else "—"
                    _evening_str = "  ·  ".join(_windows_by_bucket["Evening Tide"]) if _windows_by_bucket["Evening Tide"] else "—"
                    ...
                    _daily_rows.append({
                        ...
                        "Low Time":  _trough_dt,
                        "Morning Tide": _morning_str,
                        "Afternoon Tide": _afternoon_str,
                        "Evening Tide": _evening_str,
                        "Crossing Windows (daylight tide 1.7-2.5m)": _win_str,
                ...
                # ── Metric strip ────────────────────────────────────────────
                _no_cross_days = (_tide_df["Windows"] == 0).sum()
                _avg_windows   = _tide_df["Windows"].mean()
                _total_windows = _tide_df["Windows"].sum()
                _tc1, _tc2, _tc3, _tc4 = st.columns(4)
                with _tc1: kpi("Sim Days Covered", f'{len(_daily_rows)}')
                with _tc2: kpi("Avg Windows/Day",  f'{_avg_windows:.1f}')
                with _tc3: kpi("Total Windows",    f'{int(_total_windows)}')
                with _tc4: kpi("Restricted Days",
                               f'{_no_cross_days}',
                               sub="days with no crossing window" if _no_cross_days else "✅ all days crossable")
                ...
                st.markdown(
                    ...
                    f'🌊 <b>Breakwater crossing rule:</b> vessels may only depart when tide is within '
                    f'<b>{_tide_min_m:.1f}-{_tide_max_m:.1f} m</b> in <b>0.1 m steps</b> '
                    f'AND within daylight (<b>{_DSTART:02d}:00–{_DEND:02d}:00</b>). '
                    f'Windows are split into <b>Morning Tide</b> (06:00-23:00), '
                    f'<b>Afternoon Tide</b> (12:00-15:59), and <b>Evening Tide</b> (16:00-17:59). '
                    f'Overlaps are allowed, so one window can appear in multiple period columns. '
                ...
                def _tide_row_color(row):
                    # Use the Windows count stored in a parallel list by index
                    idx = row.name
                    w = _daily_rows[idx]["Windows"] if idx < len(_daily_rows) else 1
                    if w == 0:
                ...
                _tide_display_df = _tide_df.drop(columns=["Windows"]).reset_index(drop=True)
                ...
                if _sd_slots:
                    ...
                    _sd_hods   = [h % 24 for h in _sd_hours]
                    ...
                    # Valid crossing zones (tide in allowed set AND daylight) — green fill
                    _in_zone = False
                    _zone_x0 = None
                    for _i, (_hod, _hgt) in enumerate(zip(_sd_hods, _sd_heights)):
                        _ok = round(_hgt, 1) in _tide_allowed and _DSTART <= _hod < _DEND
    ...
    if not run_opt:
        ...
        SCORE_DIMS = [
            ("Throughput",  best_sc["throughput_score"],  "#56d364", "30% weight"),
            ("Idle",        best_sc["idle_score"],         "#f1c40f", "25% weight"),
            ("Overflow",    best_sc["overflow_score"],     "#79c0ff", "20% weight"),
            ("Export",      best_sc["export_score"],       "#c084fc", "15% weight"),
            ("Turnaround",  best_sc["turnaround_score"],   "#fb923c", "10% weight"),
        ]
        ...
        with st.expander("📊 All scenarios ranked — click to compare"):
            ...
            display_cols = [
                "Rank", "Score", "Throughput", "Idle", "Overflow",
                "Export", "Turnaround", "Loaded (bbl)", "Spilled (bbl)",
            ...
            tbl_show["Throughput"]   = tbl_show["Throughput"].round(1)
            tbl_show["Idle"]         = tbl_show["Idle"].round(1)
            tbl_show["Overflow"]     = tbl_show["Overflow"].round(1)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Throughput":  st.column_config.NumberColumn("Throughput", format="%.1f"),
                    "Overflow":    st.column_config.NumberColumn("Overflow", format="%.1f"),
...
<div class="summary-card">
  ...
  <ul>
    <li><strong>Tidal schedule adherence</strong> — all crossings of the main
        SanBarth→BIA breakwater require tide levels 1.7-2.5 m (0.1 m steps) during daylight. Departures must
