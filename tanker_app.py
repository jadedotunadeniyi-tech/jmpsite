        ...
        if not _pdf_api_key:
            ...
            if _stock_pdf is not None:
                ...
                if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                    ...
                    _spin_slot.info("📄 Reading stock report with AI — this takes about 20 seconds…")
                    try:
                        ...
                        if _resp.status_code != 200:
                            _http_body = _resp.json() if _resp.content else {}
                            _http_msg  = _http_body.get("error", {}).get("message", _resp.text[:200])
                            if _resp.status_code == 401:
                                raise ValueError("API key rejected (401 Unauthorized). Check your key in Streamlit Cloud Secrets.")
                            elif _resp.status_code == 403:
                                raise ValueError("Permission denied (403). The key may not have API access.")
                            else:
                                raise ValueError(f"API error {_resp.status_code}: {_http_msg}")
