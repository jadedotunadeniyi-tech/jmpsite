...
def main():
    ...
    with h2:
        ...
        st.caption(
            ...
            "3 mother vessels (Bryanston, GreenEagle, SanJulian) · "
...
<div class="summary-card">
  ...
  <ul>
    ...
    <li><strong>Mother tankers</strong> — 3 vessels (Bryanston, GreenEagle, SanJulian)
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    _html.append(
        ...
        '<th>Vessel (ETA)</th>'
        '<th>Bryanston</th><th>GreenEagle</th><th>SanJulian</th>'
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        # ── Arriving BIA ──────────────────────────────────────────────────────
        def _eta_label(r):
            """Build vessel ETA label with +N day suffix when ETA crosses midnight."""
            _eta_time = r["Time"][11:16]          # "HH:MM"
            _eta_date_str = r["Time"][:10]         # "YYYY-MM-DD"
            try:
                import datetime as _dtimp
                _eta_date = _dtimp.date.fromisoformat(_eta_date_str)
                _days_ahead = (_eta_date - _date).days
                if _days_ahead == 1:
                    return f"{r['Vessel']} ({_eta_time} +1d)"
                elif _days_ahead >= 2:
                    return f"{r['Vessel']} ({_eta_time} +{_days_ahead}d)"
            except Exception:
                pass
            return f"{r['Vessel']} ({_eta_time})"
        ...
        # ── MTO column: show transient/discharger pair for this day ──────────
        def _mto_dcell():
            # ── FIX: slice log_df for THIS day (not the stale build-loop _d) ─
            _day_rows = log_df[log_df["Day"] == _day]
            _mto_nom = _day_rows[
            ...
            _transient  = _mto_nom[0]["Vessel"] if _mto_nom else "?"
            _discharger = _mto_dis[0]["Vessel"] if _mto_dis else "?"
            _tc  = VESSEL_COLORS.get(_transient,  "#334155")
            _dc2 = VESSEL_COLORS.get(_discharger, "#334155")
            _time = _mto_nom[0]["Time"][11:16] if _mto_nom else ""
            # Extract transient voyage code and on-board stock from Detail
            _trn_vcode = str(_mto_nom[0].get("VoyageCode","")).strip() if _mto_nom else ""
            _trn_detail = str(_mto_nom[0].get("Detail","")) if _mto_nom else ""
            _trn_onboard = ""
            try:
                import re as _re
                ...
                pass
            # Show all dischargers (multiple parcels scenario) with their voyage codes
            _all_dis_rows = _mto_dis if _mto_dis else []
            _dis_chips = "".join(
                ...
                + f'</div>'
                for r in _all_dis_rows
            ) if _all_dis_rows else (
                f'<span style="background:{_dc2};color:#fff;border-radius:3px;'
                f'padding:1px 5px;font-size:9px;font-weight:700">{_discharger}</span>'
            )
            _vcode_badge = (
            ...
            _onboard_badge = (
                f'<div style="font-size:8px;color:#166534;margin:1px 0">'
                f'📦 {_trn_onboard}</div>'
            ) if _trn_onboard else ""
            return (
                f'<td style="background:#ecfdf5;text-align:center">'
                f'<div style="font-size:9px;color:#166534;font-weight:700;margin-bottom:3px">🔄 MTO {_time}</div>'
                f'<span style="background:{_tc};color:#fff;border-radius:3px;'
                f'padding:1px 6px;font-size:10px;font-weight:700">{_transient}</span>'
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _feta_label(r):
            """Full-run ETA label with +N day suffix when ETA crosses midnight."""
            ...
            try:
                ...
                if _fdays_ahead == 1:
                    ...
                    return f"{r['Vessel']} ({_eta_time} +{_fdays_ahead}d)"
            except Exception:
                pass
            return f"{r['Vessel']} ({_eta_time})"
        ...
        def _mto_fdcell():
            ...
            if not _fmto_nom and not _fmto_dis:
                return f'<td style="text-align:center;background:#f0fdf4">{_idle()}</td>'
            _ftransient  = _fmto_nom[0]["Vessel"] if _fmto_nom else "?"
            _ftc  = VESSEL_COLORS.get(_ftransient, "#334155")
            _ftime = _fmto_nom[0]["Time"][11:16] if _fmto_nom else ""
            # Voyage code and stock on board
            _ftrn_vcode = str(_fmto_nom[0].get("VoyageCode","")).strip() if _fmto_nom else ""
            _ftrn_detail = str(_fmto_nom[0].get("Detail","")) if _fmto_nom else ""
            _ftrn_onboard = ""
            try:
                import re as _re2
                _fob = _re2.search(r"on-board:\s*([\d,]+)\s*bbl", _ftrn_detail)
                if _fob:
                    _ftrn_onboard = f"{int(_fob.group(1).replace(',','')) // 1000}k bbl"
            except Exception:
                pass
            # Show all dischargers (multi-parcel) with voyage codes
            _fdis_chips = "".join(
                f'<div style="margin:1px 0">'
                f'<span style="background:{VESSEL_COLORS.get(r["Vessel"],"#334155")};color:#fff;'
                f'border-radius:3px;padding:1px 5px;font-size:9px;font-weight:700;'
                ...
                f'<span style="background:#334155;color:#fff;border-radius:3px;'
                f'padding:1px 5px;font-size:9px;font-weight:700">?</span>'
            )
            _fvcode_badge = (
                f'<div style="background:rgba(0,0,0,0.3);border-radius:2px;'
                f'padding:0 5px;font-size:9px;font-family:monospace;color:#fff;'
            ...
            return (
                f'<td style="background:#ecfdf5;text-align:center">'
                f'<div style="font-size:9px;color:#166534;font-weight:700;margin-bottom:3px">🔄 MTO {_ftime}</div>'
                f'<span style="background:{_ftc};color:#fff;border-radius:3px;'
                f'padding:1px 6px;font-size:10px;font-weight:700">{_ftransient}</span>'
