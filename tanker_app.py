...
Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  ...
  "extraction_notes": ""
        ...
        _pdf_uploader_col, _pdf_status_col = st.columns([2, 3])
        ...
        with _pdf_status_col:
            if st.session_state.get("_pdf_import_status"):
                _imp_st = st.session_state["_pdf_import_status"]
                if _imp_st["type"] == "ok":
            ...
            if st.session_state.get("_pdf_extracted"):
                ...
                _ex_totals = (
                    f"Storage: {sum(_ex.get('storage_volumes', {}).values()):,} bbl · "
                    f"Mothers: {sum(_ex.get('mother_volumes', {}).values()):,} bbl · "
                    f"Daughters: {sum(v.get('cargo_bbl',0) for v in _ex.get('daughter_vessels',[]))  :,} bbl"
                )
        ...
        if _stock_pdf is not None:
            ...
            if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                with st.spinner("Extracting data from stock report…"):
                    try:
                        ...
                        st.session_state["_pdf_import_status"] = {
                            "type": "ok",
                            "msg": f"Extracted from report dated {_extracted.get('report_date','?')}. "
                                   "Review and adjust below, then press Confirm.",
                        }
                    except Exception as _pdf_err:
                        st.session_state["_pdf_import_status"] = {
                            "type": "error",
                            "msg": f"Extraction failed: {_pdf_err}",
                        }
                    ...
                    if st.button(
                        ...
                        st.session_state["_pdf_import_status"] = {
                            ...
                            "msg": (
                                ...
                                "Simulation start date set to report date. "
