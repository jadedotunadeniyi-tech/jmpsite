...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    # Snapshot the per-storage rate dict so it can be fully restored after the run.
    # Without this, re-runs with different rates corrupt the module-level dict.
    if hasattr(mod, "STORAGE_PRODUCTION_RATE_BY_NAME"):
        orig["_spr_snapshot"] = dict(mod.STORAGE_PRODUCTION_RATE_BY_NAME)
    ...
    for k, v in orig.items():
        if k not in ("_ibom_rate", "_spr_snapshot"):
            setattr(mod, k, v)
    _r = orig["_ibom_rate"]
    ...
    # Restore per-storage rate dict — must be done after setattr loop above
    if "_spr_snapshot" in orig and hasattr(mod, "STORAGE_PRODUCTION_RATE_BY_NAME"):
        mod.STORAGE_PRODUCTION_RATE_BY_NAME.clear()
        mod.STORAGE_PRODUCTION_RATE_BY_NAME.update(orig["_spr_snapshot"])
    ...
    with st.sidebar:
        ...
        with st.expander("Startup Day Manual Point B Nominations", expanded=startup_day_disable_point_b_priority):
            ...
            for _mn in _mother_opts:
                ...
                with _mcol2:
                    # Show export-days input only when Export Operation is selected
...
<div class="summary-card">
  ...
  <ul>
    ...
    _jmp_days = params["sim_days"]   # full simulation duration

    # Paginate to avoid rendering 365 rows in one HTML block (crashes browsers)
    _JMP_PAGE_SIZE = 30
    _jmp_total_pages = max(1, math.ceil(_jmp_days / _JMP_PAGE_SIZE))
    if _jmp_total_pages > 1:
        _jmp_page_col, _ = st.columns([2, 4])
        with _jmp_page_col:
            _jmp_page = st.selectbox(
                "📅 View period",
                options=list(range(1, _jmp_total_pages + 1)),
                format_func=lambda p: (
                    f"Days {(p-1)*_JMP_PAGE_SIZE + 1}–{min(p*_JMP_PAGE_SIZE, _jmp_days)}"
                    f"  ({(_jmp_start + _dt.timedelta(days=(p-1)*_JMP_PAGE_SIZE)).strftime('%-d %b')} → "
                    f"{(_jmp_start + _dt.timedelta(days=min(p*_JMP_PAGE_SIZE, _jmp_days) - 1)).strftime('%-d %b %Y')})"
                ),
                key="jmp_page",
            )
        _jmp_day_start = (_jmp_page - 1) * _JMP_PAGE_SIZE + 1
        _jmp_day_end   = min(_jmp_page * _JMP_PAGE_SIZE, _jmp_days)
    else:
        _jmp_page      = 1
        _jmp_day_start = 1
        _jmp_day_end   = _jmp_days
