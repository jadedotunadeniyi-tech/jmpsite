...
def run_optimizer(base_params_json: str):
    ...
    def _score(S, log_df, tl_df):
        ...
        if not tl_df.empty:
            for sn in storage_names:
                ...
                if crit > 0:
                    risk_fracs.append(float((s_col > crit).mean()))
                borderline_fracs.append(float((s_col >= 0.90 * cap).mean()))
        ...
        spill_penalty = min(70.0, (spilled / max(1.0, total_loaded + 1.0)) * 600.0)
        risk_penalty = min(20.0, risk_avg * 100.0 * 0.40)
        borderline_penalty = min(20.0, borderline_avg * 100.0 * 0.35)
        overflow_score = max(0.0, 100.0 - spill_penalty - risk_penalty - borderline_penalty)
        ...
        # Composite objective prioritises stock crash-down + safety + utilisation.
        composite = (
            crash_score * 0.45
            + overflow_score * 0.30
            + idle_score * 0.20
    ...
    for r in all_results:
        ...
        rows.append({
            ...
            "Storage Safety": sc["overflow_score"], "Export": sc["export_score"],
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    if not run_opt:
        ...
        SCORE_DIMS = [
            ("Stock Drawdown", best_sc["throughput_score"], "#56d364", "45% weight"),
            ("Fleet Utilisation", best_sc["idle_score"],    "#f1c40f", "20% weight"),
            ("Storage Safety", best_sc["overflow_score"],   "#79c0ff", "30% weight"),
            ("Export Flow", best_sc["export_score"],        "#c084fc", "3% weight"),
            ("Turnaround", best_sc["turnaround_score"],     "#fb923c", "2% weight"),
        ]
