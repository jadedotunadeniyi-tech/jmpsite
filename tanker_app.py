...
def chart_mothers(tl_df, export_trigger, cap):
    ...
    if "SanJulian_bbl" in tl_df.columns:
        ...
        # SanJulian capacity reference line
        _sj_cap = 450_000
        fig.add_hline(y=_sj_cap,
                      line=dict(color="#f39c12", dash="dot", width=1),
                      annotation_text=f"SanJulian cap ({_sj_cap:,} bbl)",
...
def render_mother_cards(gs_vols, manual_mother, mod):
    """Render one status card per mother vessel + SanJulian intermediate storage."""
    # Three primary mothers + SanJulian = 4 columns
    cols = st.columns(4)
    ...
    # SanJulian — intermediate floating storage (no export)
    ...
    _sj_col  = "#f39c12"   # amber — distinct from primary mother palette
    with cols[3]:
        st.markdown(f"""
        <div class="vcard" style="border-left-color:{_sj_col}">
          <div class="vcard-name" style="color:{_sj_col}">⛴️ SanJulian</div>
          <div style="font-size:10px;color:#f39c12;font-weight:700;margin-bottom:3px">
          ...
          <span style="font-size:11px;color:#f39c12">⇄ Transloads to mothers</span>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        # Daughter options per mother: all vessels + special Export Operation token
        _EXPORT_TOKEN  = "Export Operation"
        _daughter_opts = _nom_vessels + [_EXPORT_TOKEN]
        # Default daughter assignments per mother (the three known Day-1 pairings)
        _mother_defaults = {
            "Bryanston":  ["Sherlock"],
            "Alkebulan":  ["Laphroaig"],
            "GreenEagle": ["Watson"],
        }
        startup_day_manual_nominations = {}   # {vessel → mother}  — sim-facing format
        mother_export_seed             = {}   # {mother → days}    — export delay seed
        with st.expander("Startup Day Manual Point B Nominations", expanded=startup_day_disable_point_b_priority):
            st.caption(
                "For each mother, select which daughters discharge to her on Day 1. "
                "Choose **Export Operation** to mark the mother as away at export "
            ...
            for _mn in _mother_opts:
                ...
                # Show export-days input directly below when Export Operation is selected
                if _EXPORT_TOKEN in _sel_daughters:
                    _exp_col, _ = st.columns([2, 3])
                    with _exp_col:
                ...
                # Build vessel→mother nominations (skip the Export token itself)
        ...
        enable_point_b_startup_seed = st.toggle(
            ...
            help=(
                ...
                "Mothers marked 'Export Operation' will be blocked for the given "
                "number of days before accepting new daughters."
            ),
        ...
        # ── Custom vessel injection ──────────────────────────────────────────
        # Collect the available storage names from the sim module so the UI
        # stays in sync if names ever change.
        _cv_storage_opts = [
            getattr(mod, "STORAGE_PRIMARY_NAME",    "Chapel"),
            getattr(mod, "STORAGE_SECONDARY_NAME",  "JasmineS"),
            getattr(mod, "STORAGE_TERTIARY_NAME",   "Westmore"),
        ...
        with st.expander("🚢 Add Custom Daughter Vessel(s)", expanded=False):
            st.caption(
        ...
        # ── Vessel Resumption Dates ──────────────────────────────────────────
        with st.expander("⏸️ Vessel Resumption Dates", expanded=False):
            st.caption(
                "Hold a vessel idle until a chosen date, then force it to load "
                "with priority at a specific storage on that day."
            )
            ...
            # ── Input form — stacked for sidebar readability ─────────────────
            ...
            if st.button("➕ Set resumption", key="vr_add_btn", use_container_width=True, type="primary"):
                _vr_dict[_vr_vessel] = {
                    "name":    _vr_vessel,
                    "date":    str(_vr_date),
                    "storage": _vr_storage,
            ...
            # ── Active resumptions table ─────────────────────────────────────
            if _vr_dict:
                st.markdown("---")
                st.markdown("**Active resumptions**")
                for _vr_key, _vr_entry in list(_vr_dict.items()):
                    _ta, _tb = st.columns([5, 1])
                    with _ta:
                        st.markdown(
                            f"**{_vr_entry['name']}** — {_vr_entry['date']}  \n"
                            f"↳ Priority load at **{_vr_entry['storage']}**"
                        )
                    with _tb:
                        if st.button("✕", key=f"vr_rm_{_vr_key}", use_container_width=True):
                            ...
                            st.session_state["vessel_resumptions"] = _vr_dict
                            st.rerun()
                st.button("🗑️ Clear all", key="vr_clear_all",
                          on_click=lambda: st.session_state.update({"vessel_resumptions": {}}))
            else:
                st.info("No resumptions set.", icon="ℹ️")

        # Serialise resumptions for run_sim
...
""")
        ...
        st.markdown("### 🗑️ Cache")
        st.caption(
            "The simulation caches results so repeated runs with the same "
            "parameters are instant. Clear the cache to force a completely "
            "fresh run — useful if you suspect stale results."
        )
        if st.button("🗑️ Clear simulation cache", use_container_width=True):
            st.cache_data.clear()
            st.cache_resource.clear()
            # Also wipe the version sentinel so the auto-clear guard resets
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    _sj_final = S.get("final_SanJulian", 0.0)
    with k2[3]: kpi("Final All Storage", f"{all_stor:,.0f} bbl")
    with k2[4]: kpi("Final All Mothers + SanJulian",
                    f"{all_moth + _sj_final:,.0f} bbl",
                    sub=f"Mothers {all_moth:,.0f} · SanJulian {_sj_final:,.0f}")
...
<div class="summary-card">
  ...
  <ul>
    ...
    with mc[3]:
        ...
        _sj_cap_kpi = int(getattr(mod, "SANJULIAN_CAPACITY_BBL", 450_000))
        _sj_pct_kpi = f"{_sj_fv / _sj_cap_kpi * 100:.0f}%" if _sj_cap_kpi else "—"
        _sj_api_txt = f" · API {_sj_api:.2f}°" if _sj_fv > 0 else ""
        kpi(
            "⛴️ SanJulian (Intermediate)",
            f"{_sj_fv:,.0f} bbl",
            sub=f"{_sj_pct_kpi} full{_sj_api_txt} · rcvd {_sj_rcvd:,.0f} · xld {_sj_xld:,.0f}",
        )
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        if not _t.empty:
            ...
            # SanJulian intermediate storage stock (separate from primary mothers)
            _ev[_day]["m_stocks"]["SanJulian"] = int(_f["SanJulian_bbl"]) if "SanJulian_bbl" in _f.index else 0
            _ev[_day]["stock_apis"] = {
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    _html.append(
        ...
        # BIA arrival
        '<th>Vessel (ETA)</th>'
        # Discharge per mother + SanJulian
        '<th>Bryanston</th><th>Alkebulan</th><th>GreenEagle</th>'
        '<th style="background:#92400e;color:#fef3c7">⛴️ SanJulian</th>'
        '</tr>'
    )
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        # ── SanJulian intermediate storage cell ───────────────────────────────
        _sj_ms   = _mstocks.get("SanJulian", 0)
        _sj_entries = [r for r in _de["discharge"] if _parse_mother(r["Detail"]) == "SanJulian"]
        if not _sj_entries:
            _sj_stk  = f'<span style="font-size:9px;color:#94a3b8">Stock: {_kkk(_sj_ms)}</span>'
            _sj_cell = f'<td style="text-align:center;background:#fef9c3">{_idle()}<br>{_sj_stk}</td>'
        else:
            _sj_stk  = f'<span style="font-size:9px;color:#92400e;display:block;margin-bottom:2px">Stock: {_kkk(_sj_ms)}</span>'
            _sj_inner = "<br>".join(
                _chip(r["Vessel"],
                      f"{r['Vessel']} | {r['Time'][11:16]} | {_kkk(_parse_cargo(r['Detail']))}",
                      bg="#f39c12")
                for r in _sj_entries
            )
            _sj_cell = f'<td style="background:#fef9c3">{_sj_stk}{_sj_inner}</td>'

        _html.append(
            f"<tr>{_date_cell}{_stock_cells}{_load_cells}"
            f"<td>{_ret_vessel}</td><td style='text-align:center'>{_ret_eta}</td>"
            f"<td>{_bia_arr}</td>"
            f"{_disch_cells}{_sj_cell}</tr>"
        )
