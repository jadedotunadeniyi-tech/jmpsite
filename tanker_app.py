        ...
        if multiple_transient_operation:
            ...
            with _mto_col2:
                st.markdown(
                    ...
                    'Balham / Bedford / Berners / Bagshot: <b>125,000</b><br>'
        ...
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                _is_returning_vessel = (vn in (
                    ...
                    "Bagshot", "Watson", "Berners", "SantaMonica",
                ))
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _mto_dcell():
            ...
            # Show all dischargers (multiple parcels scenario)
            _all_discharged = [r["Vessel"] for r in _mto_dis]
            _dis_chips = "".join(
                f'<span style="background:{VESSEL_COLORS.get(vn,"#334155")};color:#fff;'
                f'border-radius:3px;padding:1px 5px;font-size:9px;font-weight:700;'
                f'display:inline-block;margin:1px">{vn}</span>'
                for vn in _all_discharged
            ) if _all_discharged else (
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _mto_fdcell():
            ...
            # Show all dischargers (multi-parcel)
            _fall_discharged = [r["Vessel"] for r in _fmto_dis]
            _fdis_chips = "".join(
                f'<span style="background:{VESSEL_COLORS.get(vn,"#334155")};color:#fff;'
                f'border-radius:3px;padding:1px 5px;font-size:9px;font-weight:700;'
                f'display:inline-block;margin:1px">{vn}</span>'
                for vn in _fall_discharged
            ) if _fall_discharged else (
