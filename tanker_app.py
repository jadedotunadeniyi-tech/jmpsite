                ...
                if _vr_indef or _vr_date == "indefinite":
                    ...
                    if _vr_start and _vr_start not in ("", "indefinite"):
                        try:
                            ...
                            _epoch   = getattr(mod, "SIM_EPOCH", None)
                            _sd_obj  = _dti2.date.fromisoformat(_vr_start)
                            if _epoch:
                                _dormancy_start_h = (_sd_obj - _epoch).days * 24.0
                        except Exception:
                            _dormancy_start_h = 0.0
        ...
        for v in sim.vessels:
            if v.name in _resumption_seed_by_vessel:
                ...
                if _seed_indef:
                    ...
                    # Dormancy starts mid-sim — vessel runs normally until
                    # dormancy_start_hour, then holds idle until resumption_hour.
                    # Store the dormancy window on the vessel so the run loop
                    # can enforce it when t crosses dormancy_start_hour.
                    v.dormancy_start_hour    = _dorm_start_h
                    v.resumption_storage     = _seed_storage
                    v.resumption_priority    = False
                    # next_event_time stays as-is — vessel starts normally
...
def main():
    ...
    with _ctrl_inputs:
        ...
        with _avail_tab2:
            st.caption(
                "Select a vessel, choose its dormancy window (start → end), then pick the "
                "priority storage it loads from on resumption at 08:00. "
            ...
            with _vr_c3:
                if not _vr_indefinite:
                    # Date range: (dormancy_start, resumption_date)
                    _vr_range = st.date_input(
                        "Dormancy window (start → resumption date)",
                        ...
                        label_visibility="collapsed",
                        help=(
                            "Select the dormancy window. The vessel goes idle at the **start date** "
                            "and resumes loading at 08:00 on the **end date**."
                        ),
                    )
            ...
            if _vr_dict:
                ...
                with _col_h2: st.caption("Dormant from")
