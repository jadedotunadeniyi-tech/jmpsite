        ...
        if not _pdf_api_key:
            ...
            if _stock_pdf is not None:
                ...
                if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                    with st.spinner("📄 Reading stock report with AI — takes ~20 seconds…"):
                        try:
                            ...
                            _emsg = str(_pdf_err)
                            if "401" in _emsg or "unauthorized" in _emsg.lower():
                                _emsg = "API key rejected (401). Check the key in secrets.toml."
                            elif "403" in _emsg:
                                _emsg = "Permission denied (403). Check the key has API access."
                            st.session_state["_pdf_import_status"] = {
                                "type": "error",
