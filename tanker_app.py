...
Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  ...
  "extraction_notes": ""
        ...
        if not _pdf_api_key:
            st.info(
                "**One-time setup required** — add your Google API key to "
                    ...
                    try:
                        ...
                        for _model in _gemini_models:
                            _gemini_url = (
                                ...
                                f"{_model}:generateContent?key={_pdf_api_key}"
                            )
                            ...
                            if _resp.status_code == 200:
                                ...
                                break
                            _last_err = _resp.text[:300]

                        if _resp_data is None or _resp.status_code != 200:
                            _http_body = {}
                            try:
                                _http_body = _resp.json()
                            except Exception:
                                pass
                            _http_msg = _http_body.get("error", {}).get("message", _last_err)
                            if _resp.status_code in (400, 401, 403):
                                raise ValueError(
                                    f"API key error ({_resp.status_code}): {_http_msg}. "
                                    "Check your GOOGLE_API_KEY in Streamlit Cloud → Settings → Secrets."
                                )
                            else:
                                raise ValueError(f"Gemini API error {_resp.status_code}: {_http_msg}")

                        # Check for blocked / empty response
                        _candidates = _resp_data.get("candidates", [])
                        if not _candidates:
                            _feedback = _resp_data.get("promptFeedback", {})
                            raise ValueError(
                                f"Gemini returned no candidates. Feedback: {_feedback}"
                            )
