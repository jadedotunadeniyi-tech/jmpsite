        ...
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                if _is_bia_loc:
                    ...
                    _bia_cols = st.columns([3, 2, 2])
                    with _bia_cols[0]:
                        # Export operation toggle — lives here for any BIA vessel
                        _bia_is_export = st.checkbox(
                            "⛴ Export operation",
                            key=f"vbia_export_{vn}",
                            help=(
                                "Tick if this mother vessel is currently away at export. "
                                "The vessel will return with zero cargo after the specified days. "
                                "Only primary mothers (not SanJulian) should be marked for export."
                            ),
                        )
                    if _bia_is_export:
                        _bia_exp_cols = st.columns([3, 2, 2])
                        with _bia_exp_cols[0]:
                            _bia_export_mother = st.selectbox(
                                "Mother at export",
                                options=_primary_mothers,
                                key=f"vbia_expmother_{vn}",
                                label_visibility="visible",
                                help="Which primary mother is currently at export?"
                            )
                        with _bia_exp_cols[1]:
                            _bia_export_days = st.number_input(
                                "Export days",
                                min_value=1, max_value=30, value=3, step=1,
                                key=f"vbia_expdays_{vn}",
                                label_visibility="visible",
                                help="Days until this mother returns from export (vessel returns with 0 cargo).",
                            )
                        with _bia_exp_cols[2]:
                            st.markdown(
                                '<div style="font-size:10px;color:#b45309;padding-top:28px">'
                                '↩ Returns empty</div>',
                                unsafe_allow_html=True,
                            )
                        # Record the export seed for later
                        if _bia_export_mother:
                            mother_export_seed[_bia_export_mother] = int(_bia_export_days)
                    else:
                ...
                manual_states[vn] = {
                    ...
                    "is_export":             _bia_is_export,
                    "export_mother":         _bia_export_mother,
                    "export_days":           _bia_export_days,
                }
...
<div class="summary-card">
  ...
  <ul>
    ...
    def _allowed_ae_storages(vessel):
        ...
        Covers all five storages (Chapel, JasmineS, Westmore, Duke, Starturn).
        """
        result = []
