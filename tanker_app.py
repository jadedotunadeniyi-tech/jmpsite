...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    if vessel_states_json:
        ...
        for v in sim.vessels:
            if v.name in vs:
                d = vs[v.name]
                # Reset any sticky startup storage/mother assignments from a prior
                # manual override so the new Day-1 positioning fully controls placement.
    ...
    # This covers both manual overrides (vessel_states_json) and any vessel
    # seeded in LOADING status via the manual position form.
    ...
    for _v in sim.vessels:
        ...
        # emitted the same t=0 LOADING_START for this vessel.
...
2025-06-01 08:00 | Woodstock | IDLE_A | SanBarth | 0 |
```
