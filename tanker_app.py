...
Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  ...
  "extraction_notes": ""
        ...
        # ── PDF file uploader (server-side extraction via st.secrets) ──────────
        # API key is read from .streamlit/secrets.toml — set it once, never enter it again.
        # To configure: create .streamlit/secrets.toml next to your app with:
        #   ANTHROPIC_API_KEY = "sk-ant-..."

        # Resolve API key from secrets (preferred) or env var fallback
        _pdf_api_key = (
        ...
        if not _pdf_api_key:
            st.info(
                "**One-time setup required** — add your Anthropic API key to "
                "`.streamlit/secrets.toml` next to the app file:\n\n"
                "```toml\nANTHROPIC_API_KEY = \"sk-ant-...\"\n```\n\n"
                "Once added, restart the app and the PDF upload will work automatically "
                "with no key entry ever again.",
                icon="🔑",
            )
        else:
            _stock_pdf = st.file_uploader(
                ...
                help="Upload the Daily Stock Update PDF — all 3 pages. Data is extracted automatically.",
            )

            if _stock_pdf is not None:
                ...
                if st.session_state.get("_pdf_last_hash") != _pdf_hash:
                    ...
                    with _pdf_placeholder.container():
                        st.info("📄 Reading stock report with AI — this takes about 20 seconds…")
                    ...
                    try:
                            ...
                            json={
                                ...
                                "messages": [{
                                    ...
                                    "content": [
                                        ...
                                        {"type": "text",
                                         "text": "Extract all startup data from this Daily Stock Update report exactly as specified."}
                                    ]
                                }]
                            },
                            timeout=120,
                        ...
                        if _resp.status_code != 200:
                            _http_body = {}
                            try:
                                _http_body = _resp.json()
                            except Exception:
                                pass
                            _http_msg = _http_body.get("error", {}).get("message", _resp.text[:300])
                            if _resp.status_code == 401:
                                raise ValueError("API key rejected (401). Check the key in Streamlit Cloud → Settings → Secrets.")
                            elif _resp.status_code == 403:
                                raise ValueError("Permission denied (403). The key may not have API access enabled.")
                            else:
                                raise ValueError(f"API returned {_resp.status_code}: {_http_msg}")
                        ...
                        _extracted = json.loads(_resp_text.strip())
                        ...
                        _pdf_emsg = str(_pdf_err)
                        st.session_state["_pdf_import_status"] = {
                            "type": "error",
                            "msg": f"Extraction failed: {_pdf_emsg}",
                        }

                    # Replace spinner with result inline — no st.rerun() so file uploader stays intact
                    _pdf_placeholder.empty()
                    if _pdf_ok:
                        st.success(st.session_state["_pdf_import_status"]["msg"], icon="✅")
                    else:
                        st.error(st.session_state["_pdf_import_status"]["msg"], icon="❌")
