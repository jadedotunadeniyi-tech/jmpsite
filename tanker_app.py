...
LOCATION_CATALOGUE = [
    ...
    # These entries allow startup-day position entry for MTO operations.
    # "MTO — Watson (BIA)" etc. — one entry per shuttle that can act as receiver.
    {"display": "MTO — Watson (BIA)",      "sim_value": "Watson",
    ...
    {"display": "MTO — Sherlock (BIA)",    "sim_value": "Sherlock",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_receiver": True,
     "statuses": [
        ("PF_SWAP",          "📦 MTO receiver — accumulating cargo from discharger(s)"),
    ...
    {"display": "MTO — Laphroaig (BIA)",   "sim_value": "Laphroaig",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_receiver": True,
     "statuses": [
        ("PF_SWAP",          "📦 MTO receiver — accumulating cargo from discharger(s)"),
    ...
    {"display": "MTO — Rathbone (BIA)",    "sim_value": "Rathbone",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_receiver": True,
     "statuses": [
        ("PF_SWAP",          "📦 MTO receiver — accumulating cargo from discharger(s)"),
    ...
    {"display": "MTO — Discharging to Sherlock",  "sim_value": "Sherlock",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_discharger": True, "mto_target_vessel": "Sherlock",
     "statuses": [
        ("DISCHARGING",      "⬇️ MTO discharge — pumping cargo to Sherlock"),
        ("HOSE_CONNECT_B",   "🔧 MTO discharge — hose connection to Sherlock"),
        ("BERTHING_B",       "🔗 MTO discharge — berthing alongside Sherlock"),
        ("WAITING_BERTH_B",  "⏳ MTO discharge — waiting to berth alongside Sherlock"),
        ("CAST_OFF_B",       "↩️ MTO discharge — complete, casting off from Sherlock"),
    ]},
    {"display": "MTO — Discharging to Laphroaig", "sim_value": "Laphroaig",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_discharger": True, "mto_target_vessel": "Laphroaig",
     "statuses": [
        ("DISCHARGING",      "⬇️ MTO discharge — pumping cargo to Laphroaig"),
        ("HOSE_CONNECT_B",   "🔧 MTO discharge — hose connection to Laphroaig"),
        ("BERTHING_B",       "🔗 MTO discharge — berthing alongside Laphroaig"),
        ("WAITING_BERTH_B",  "⏳ MTO discharge — waiting to berth alongside Laphroaig"),
        ("CAST_OFF_B",       "↩️ MTO discharge — complete, casting off from Laphroaig"),
    ]},
    {"display": "MTO — Discharging to Rathbone",  "sim_value": "Rathbone",
     "field_zone": "BIA", "target_mother": None, "target_storage": None,
     "mto_discharger": True, "mto_target_vessel": "Rathbone",
     "statuses": [
        ("DISCHARGING",      "⬇️ MTO discharge — pumping cargo to Rathbone"),
        ("HOSE_CONNECT_B",   "🔧 MTO discharge — hose connection to Rathbone"),
    ...
    with _ctrl_data:
        ...
        if st.session_state.get("_pdf_extracted"):
            ...
            with st.expander("📝 Review & Adjust Extracted Data", expanded=True):
                ...
                _STORAGE_EDIT_OPTS = ["", "Chapel", "JasmineS", "Westmore", "Duke", "Starturn", "PGM", "Ibom"]
                _MTO_VESSEL_OPTS   = ["", "Watson", "Sherlock", "Laphroaig", "Rathbone", "Balham", "Bagshot", "SantaMonica", "Woodstock"]
                _MOTHER_EDIT_OPTS  = ["", "Bryanston", "GreenEagle", "MTSanBarth"]
                ...
                for _dvi, _dv in enumerate(_ex.get("daughter_vessels", [])):
                    ...
                    with _dc[4]:
                        _is_mto_r = _dv.get("is_mto_receiver", False)
                        _mto_tv_cur = _dv.get("mto_target_vessel", "") or ""
                        if _is_mto_r:
                            # MTO receiver — show badge, no mother dropdown
                            st.markdown(
                                '<div style="padding-top:28px;font-size:11px;color:#7c3aed;font-weight:600">📦 MTO Receiver</div>',
                                unsafe_allow_html=True)
                        elif _mto_tv_cur:
                            # MTO discharger — show target vessel picker
                            _mtv_idx = _MTO_VESSEL_OPTS.index(_mto_tv_cur) if _mto_tv_cur in _MTO_VESSEL_OPTS else 0
                            _dv["mto_target_vessel"] = st.selectbox(
                                "MTO target", _MTO_VESSEL_OPTS, index=_mtv_idx,
                                key=f"pdf_dv_mtotgt_{_dvi}",
                                label_visibility="collapsed",
                                help="The shuttle vessel receiving this MTO cargo.",
                            )
                            _dv["target_mother"] = ""  # clear mother
                        else:
        ...
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                if _is_bia_loc:
                    ...
                    if _is_mto_receiver:
                        # MTO RECEIVER — show info only, no mother selection needed
                        st.info(
                            f"**MTO Receiver** — this vessel accumulates cargo from "
                            f"discharger(s) at BIA. Set the discharger vessel(s) "
                            f"to **'MTO — Discharging to {vn}'** in their location row.",
                            icon="📦",
                        )

                    elif _is_mto_discharger:
                        # MTO DISCHARGER — target is a shuttle vessel, not a mother
                        st.info(
                            f"**MTO Discharging → {_mto_target_vessel}** — "
                            f"cargo will transfer to {_mto_target_vessel} at BIA. "
                            f"Set {_mto_target_vessel}'s location to "
                            f"'MTO — {_mto_target_vessel} (BIA)' as the receiver.",
                            icon="⬇️",
                        )
