...
def main():
    ...
    with _ctrl_day1:
        ...
        with _day1_right:
            with st.container():
                st.info(
                    "⏸️ **Vessel dormancy / resumption** settings have moved to the "
    ...
    with _ctrl_inputs:
        ...
        st.caption(
            ...
            "both **mother vessels** (Bryanston, GreenEagle, SanJulian) and **shuttle daughters**. "
            "During a mother's window she cannot receive daughters or SanJulian transloads — "
            "traffic is automatically rerouted. A shuttle held dormant wakes at 08:00 on the "
            "resumption date and takes loading priority at the assigned storage."
        )

        # ── TAB 1: Mother Vessel Unavailability Windows ───────────────────────
        _avail_tab1, _avail_tab2 = st.tabs(["🛢️ Mother Vessel Unavailability", "⏸️ Shuttle Vessel Dormancy"])
        ...
        # ── TAB 2: Shuttle Vessel Dormancy / Resumption ───────────────────────
        with _avail_tab2:
            st.caption(
                "Hold a shuttle vessel idle from Day 1 until a chosen resumption date, then "
                "force her to take loading priority at a specific storage on that date at 08:00. "
