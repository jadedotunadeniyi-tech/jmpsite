    ...
    with _ctrl_day1:
        ...
        if multiple_transient_operation:
            st.info(
                ...
                "The simulation will nominate one temporary storage vessel per day at "
                "Point B when congestion is detected at noon. "
                "MTO events appear in the event log as **MTO_TRANSIENT_NOMINATED**, "
                "**MTO_DISCHARGE_TO_TRANSIENT**, and **MTO_TRANSIENT_PRIORITY_BERTH**.",
                icon="🔄",
            )
...
""")
    ...
    _context_chips.append(
        ('🔄 MTO: ON' if multiple_transient_operation else 'MTO: off',
    ...
    if not gs_vols:
        with st.expander("✏️ Enter 08:00 storage & mother volumes",
            ...
            _mother_api_defaults = {"Bryanston": 32.00, "GreenEagle": 32.00, "SanJulian": 32.00}
            manual_mother_api = {}
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _mto_dcell():
            _mto_nom = [
                r for r in _d[_d["Event"] == "MTO_TRANSIENT_NOMINATED"].to_dict("records")
            ]
            _mto_dis = [
                r for r in _d[_d["Event"] == "MTO_DISCHARGE_TO_TRANSIENT"].to_dict("records")
            ]
            ...
            return (
                ...
                f'padding:1px 6px;font-size:10px;font-weight:700">{_discharger}</span>'
                f'</td>'
            )
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _mto_fdcell():
            ...
            _fdischarger = _fmto_dis[0]["Vessel"] if _fmto_dis else "?"
            _ftc  = VESSEL_COLORS.get(_ftransient,  "#334155")
            _fdc2 = VESSEL_COLORS.get(_fdischarger, "#334155")
            ...
            return (
                ...
                f'<div style="font-size:9px;color:#64748b;margin:2px 0">receives from</div>'
                f'<span style="background:{_fdc2};color:#fff;border-radius:3px;'
                f'padding:1px 6px;font-size:10px;font-weight:700">{_fdischarger}</span>'
                f'</td>'
            )
