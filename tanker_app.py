...
MOTHER_COLORS = {
    ...
    "SanJulian" : "#ff5555",  # vivid red     — boosted from #c0392b
...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    sim.sanjulian_bbl = min(alkebulan, getattr(mod, "SANJULIAN_CAPACITY_BBL", mod.MOTHER_CAPACITY_BBL))
    ...
    if alkebulan  > 0 and alkebulan_api  > 0:
        sim.sanjulian_api = float(alkebulan_api)
    ...
    # Balham   : BERTHING_B at SanJulian, 85k bbl ready to discharge
    ...
    for v in sim.vessels:
        if v.name == "Bedford" and "Bedford" not in _vs_override and v.name not in _resumption_seed_names:
            ...
            sim.vessel_api[v.name] = getattr(mod, "IBOM_API", 32.0)
            # Reserve SanJulian's berth for the full discharge window so no
            # other vessel attempts to berth there at t=0
            sim.sanjulian_berth_free_at = max(getattr(sim, "sanjulian_berth_free_at", 0.0), _BALHAM_END)
...
def chart_mothers(tl_df, export_trigger, cap, sanjulian_cap=None):
    fills = {"Bryanston" :"rgba(26,188,156,0.12)",
             "SanJulian" :"rgba(255,85,85,0.12)",
             "GreenEagle":"rgba(192,132,252,0.12)"}
    fig = go.Figure()
    for name, col in [("Bryanston","Bryanston_bbl"),
                       ("SanJulian","SanJulian_bbl" if "SanJulian_bbl" in tl_df.columns else "Alkebulan_bbl"),
    ...
    fig.add_hline(y=export_trigger,
                  ...
                  annotation_text=f"Export trigger for mothers ({export_trigger:,} bbl)",
    ...
    if sanjulian_cap:
        fig.add_hline(y=sanjulian_cap, line=dict(color="#dc2626", dash="dot"),
                      annotation_text=f"SanJulian capacity ({sanjulian_cap:,} bbl)",
                      annotation_font_color="#dc2626")
    fig.update_layout(height=300, margin=_MARGIN, title="Point B Assets — Volume",
                      **_DARK, legend=dict(bgcolor="#ffffff"))
    fig.update_yaxes(tickformat=",", **_GRID, title_text="bbl")
    fig.update_xaxes(**_GRID)
    return fig
...
def render_mother_cards(gs_vols, manual_mother, mod):
    """Render Point B asset cards: two export mothers plus SanJulian storage."""
    cols = st.columns(3)
    _assets = [
        ("Bryanston", "bryanston", int(mod.MOTHER_CAPACITY_BBL), True),
        ("SanJulian", "alkebulan", int(getattr(mod, "SANJULIAN_CAPACITY_BBL", mod.MOTHER_CAPACITY_BBL)), False),
        ("GreenEagle", "greeneagle", int(mod.MOTHER_CAPACITY_BBL), True),
    ]
    for i, (mn, mk, cap, is_export_mother) in enumerate(_assets):
        ...
        if is_export_mother:
            ...
            flag = '<span style="color:#f59e0b;font-size:11px">Intermediate floating storage at Point B</span>'
...
def main():
    ...
    with h2:
        ...
        st.caption(
            ...
            "2 mother vessels (Bryanston, GreenEagle) + SanJulian floating storage · "
    ...
    _mother_opts = ["Bryanston", "SanJulian", "GreenEagle"]
...
""")
    ...
    # Always show storage + Point B asset volume entry if not fully from Sheets
    if not gs_vols:
        with st.expander("✏️ Enter 08:00 storage & Point B asset volumes",
                          expanded=not use_gs):
            ...
            st.markdown("**Point B Asset Volumes at 08:00 (bbl)**")
            mv = st.columns(3)
            for j,(mn, mk, mcap) in enumerate([
                ("Bryanston", "bryanston", MOTHER_CAP),
                ("SanJulian", "alkebulan", int(getattr(mod, "SANJULIAN_CAPACITY_BBL", MOTHER_CAP))),
                ("GreenEagle", "greeneagle", MOTHER_CAP),
            ]):
                with mv[j]:
                    manual_mother[mk] = st.number_input(
                        mn, 0, mcap, 0, step=10_000, key=f"mv_{mn}")

            st.markdown("**Point B Asset Stock API Gravity at 08:00 (°API)**")
            st.caption("Set the API gravity of existing stock on each Point B asset. Ignored when stock is zero.")
            _mapi_ui_cols = st.columns(3)
            _mother_api_defaults = {"Bryanston": 32.00, "SanJulian": 32.00, "GreenEagle": 32.00}
            manual_mother_api = {}
            for _j2, (_mn2, _mk2) in enumerate([("Bryanston", "bryanston"), ("SanJulian", "alkebulan"), ("GreenEagle", "greeneagle")]):
                with _mapi_ui_cols[_j2]:
                    _stock_vol = manual_mother.get(_mk2, 0)
                    ...
                    manual_mother_api[_mk2] = _api_val
                    if _stock_vol > 0:
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    all_moth = sum(S.get(f"final_{n}",0) for n in ["Bryanston","SanJulian","GreenEagle"])
    with k2[3]: kpi("Final All Storage", f"{all_stor:,.0f} bbl")
    with k2[4]: kpi("Final Point B Assets", f"{all_moth:,.0f} bbl")
    ...
    _all_moth_vol = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","SanJulian","GreenEagle"])
    _blended_moth = (
        sum(S.get(f"final_{n}", 0) * _mapi.get(n, 0.0)
            for n in ["Bryanston","SanJulian","GreenEagle"]) / _all_moth_vol
        if _all_moth_vol > 0 else 0.0
    )
    with k3[3]: kpi("Point B Blended API", f"{_blended_moth:.2f}°", sub="all Point B assets combined")
    ...
    _final_mothers = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","SanJulian","GreenEagle"])
...
<div class="summary-card">
  ...
  <ul>
    ...
    <li><strong>Point B assets</strong> — Bryanston and GreenEagle export tankers plus SanJulian floating storage
...
<div class="summary-card">
  ...
  <ul>
    ...
    # ==========================================================================
    # ── SECTION 5: POINT B ASSET FORECAST ────────────────────────────────────
    # ==========================================================================
    sec("🛢️ Point B Assets Forecast")
    st.plotly_chart(
        chart_mothers(tl_df, EXPORT_TRIG, MOTHER_CAP, getattr(mod, "SANJULIAN_CAPACITY_BBL", None)),
        width='stretch'
    )
    _m_api = S.get("mother_api", {})
    mc = st.columns(3)
    for i,(mn,mk,api_key,cap) in enumerate([
        ("Bryanston", "bryanston", "Bryanston", MOTHER_CAP),
        ("SanJulian", "alkebulan", "SanJulian", int(getattr(mod, "SANJULIAN_CAPACITY_BBL", MOTHER_CAP))),
        ("GreenEagle", "greeneagle", "GreenEagle", MOTHER_CAP),
    ]):
        with mc[i]:
            fv    = S.get(f"final_{mn}", S.get("final_Alkebulan", 0) if mn == "SanJulian" else 0)
            start = params.get(mk, 0)
            d     = fv - start
            col_s = "#56d364" if d >= 0 else "#f85149"
            _mapi = _m_api.get(api_key, _m_api.get("Alkebulan", 0.0) if mn == "SanJulian" else 0.0)
            _api_txt = f" · <b>{(fv / cap * 100):.0f}% full · API {_mapi:.2f}°</b>" if fv > 0 and cap > 0 else ""
            kpi(mn, f"{fv:,.0f} bbl",
    ...
    all_ents = vnames + ["Chapel","JasmineS","Westmore","Duke","Starturn",
                         "Bryanston","SanJulian","GreenEagle"]
    ...
    extra  = [c for c in ["Chapel_bbl","JasmineS_bbl","Westmore_bbl",
                           ...
                           "SanJulian_api" if "SanJulian_api" in filt.columns else "Alkebulan_api","Bryanston_api","GreenEagle_api"] if c in filt.columns]
    _filt_show = filt[show_c+extra].rename(columns={
        ...
        "Alkebulan_api": "SanJulian API°", "SanJulian_api": "SanJulian API°", "Bryanston_api": "Bryanston API°",
    ...
    _mother_cols  = ["Bryanston_bbl", "SanJulian_bbl" if "SanJulian_bbl" in tl_df.columns else "Alkebulan_bbl", "GreenEagle_bbl"]
    _storage_names = ["Chapel","JasmineS","Westmore","Duke","Starturn"]
    _mother_names  = ["Bryanston","SanJulian","GreenEagle"]
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        _mapi_cols = ["Bryanston_api", "SanJulian_api" if "SanJulian_api" in _t.columns else "Alkebulan_api", "GreenEagle_api"]
        if not _t.empty:
            _f = _t.iloc[0]
            _ev[_day]["stocks"] = {
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    _html.append(
        ...
        '<th>Bryanston</th><th>SanJulian</th><th>GreenEagle</th>'
        '</tr>'
    )
...
<div style="font-size:9px;color:#94a3b8;margin-top:8px">Generated: {_dt.datetime.now().strftime("%Y-%m-%d %H:%M")} | Tanker Operations v5</div>
    ...
    with d3:
        ...
        for mn in ["Bryanston","SanJulian","GreenEagle"]:
