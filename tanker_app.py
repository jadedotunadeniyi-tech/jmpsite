...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn,
    ...
    if vessel_resumption_json and hasattr(mod, "set_vessel_resumption"):
        ...
        for _vr in _vr_list:
            try:
                ...
                if (_vr_name not in _known_vessels_vr
                        or _vr_storage not in _valid_storages_vr
                        or not _vr_date):
                    continue
    ...
    if storage_overrides_json and hasattr(mod, "STORAGE_DISPATCH_OVERRIDES"):
        try:
            ...
            # Normalise: keys are vessel names, values are {str(day): storage}
            for _vn, _day_map in _raw_sdo.items():
                if isinstance(_day_map, dict):
                    mod.STORAGE_DISPATCH_OVERRIDES[_vn] = {
                        int(_d): _s for _d, _s in _day_map.items()
                    }
    ...
    if vessel_states_json:
        ...
        for v in sim.vessels:
            if v.name in _resumption_seed_by_vessel:
                _seed_storage = _resumption_seed_by_vessel[v.name]["storage"]
                v.status = "IDLE_A"
                v.cargo_bbl = 0
    ...
    for _exp_mn, _exp_days in _mother_export.items():
        ...
        sim.mother_available_at[_exp_mn] = max(
            sim.mother_available_at.get(_exp_mn, 0.0), _exp_h
        )
        sim.next_mother_berthing_start_at = max(
...
def chart_mothers(tl_df, export_trigger, cap_by_name):
    ...
    # Export trigger applies only to primary mothers (Bryanston / Alkebulan / GreenEagle)
    fig.add_hline(y=export_trigger,
                  line=dict(color="#ff5555", dash="dash", width=1.5),
                  annotation_text=f"Primary mother export trigger ({export_trigger:,} bbl)",
...
def main():
    ...
    with _ctrl_day1:
        startup_day_disable_point_b_priority = st.toggle(
            ...
            value=True,
            help=(
                "Day 1 only: Point B assignment is manual via nominations below. Day 2 onward: strict automatic prioritization resumes."
            ),
            key="startup_day_disable_point_b_priority",
        )
        with st.expander("Startup Day Manual Point B Nominations", expanded=True):
            st.caption(
                "For each mother, select which daughters discharge to her on Day 1. Choose Export Operation to mark the mother as away at export for a custom number of days."
            )
            _mcols = st.columns(len(_mother_opts))
            for _idx, _mn in enumerate(_mother_opts):
                with _mcols[_idx]:
                    _defaults_for_m = _mother_defaults.get(_mn, [])
                    _sel_daughters = st.multiselect(
                        f"🛢️ {_mn}",
                        options=_daughter_opts,
                        default=[d for d in _defaults_for_m if d in _daughter_opts],
                        key=f"startup_nom_mother_{_mn}",
                        placeholder="Select daughters or Export…",
                    )
                    _selected_names = [d for d in _sel_daughters if d != _EXPORT_TOKEN]
                    _selected_by_mother[_mn] = list(_selected_names)
                    if _EXPORT_TOKEN in _sel_daughters:
                        _exp_days = st.number_input(
                            ...
                            help=f"Number of days {_mn} is away at export from t=0.",
                        )
                        mother_export_seed[_mn] = int(_exp_days)
                    _chips_html = "".join(
                        f'<span class="startup-chip">{_vn}</span>' for _vn in _selected_names
                    )
                    _export_html = (
                        f'<span class="startup-chip startup-chip-export">Export seeded: {mother_export_seed[_mn]} day(s)</span>'
                        if _mn in mother_export_seed else ""
                    )
                    _summary_body = _chips_html or '<span class="startup-muted">No daughters selected</span>'
                    _mother_color = MOTHER_COLORS.get(_mn, "#3b82f6")
                    st.markdown(
                        f'''<div class="startup-card" style="border-left-color:{_mother_color}">
                        <div class="startup-card-head">
                          <div class="startup-card-title">{_mn} selection</div>
                          <div class="startup-card-meta">{len(_selected_names)} daughter(s)</div>
                        </div>
                        <div>{_summary_body}</div>
                        <div style="margin-top:4px">{_export_html}</div>
                        </div>''',
                        unsafe_allow_html=True,
                    )
            ...
            _selection_counts = {}
            for _picked_vessels in _selected_by_mother.values():
                for _vn in _picked_vessels:
                    _selection_counts[_vn] = _selection_counts.get(_vn, 0) + 1
            _duplicate_noms = sorted(_vn for _vn, _count in _selection_counts.items() if _count > 1)
            _unassigned_vessels = sorted(_vn for _vn in _nom_vessels if _vn not in startup_day_manual_nominations)
            _seeded_total = len(startup_day_manual_nominations)
            _export_total = len(mother_export_seed)
            st.markdown(
                f'''<div class="startup-overall">
                <div class="startup-overall-title">Current Day 1 selection</div>
                <span class="startup-chip">Assigned daughters: {_seeded_total}</span>
                <span class="startup-chip">Export seeds: {_export_total}</span>
                <span class="startup-chip">Unassigned daughters: {len(_unassigned_vessels)}</span>
                </div>''',
                unsafe_allow_html=True,
            )
            if _duplicate_noms:
                st.warning(
                    "Selected on multiple mothers: " + ", ".join(_duplicate_noms) + ". The simulator keeps only the last mother selection for each duplicated daughter.",
                    icon="⚠️",
                )
            if _unassigned_vessels:
                st.info("Unassigned daughters: " + ", ".join(_unassigned_vessels), icon="ℹ️")
            else:
                st.success("All daughters have a visible Day 1 assignment.", icon="✅")

        enable_point_b_startup_seed = st.toggle(
            "Validation Seed: Use startup manual nominations (full-load at BIA)",
            value=False,
            help=(
                "When enabled, every daughter nominated above starts at t=0 in HOSE_CONNECT_B with full cargo assigned to the selected mother. Mothers marked 'Export Operation' will be blocked for the given number of days before accepting new daughters."
            ),
        ...
        with _day1_right:
            ...
            # ── Add row ──────────────────────────────────────────────────────
            _vr_c1, _vr_c2, _vr_c3, _vr_c4 = st.columns([2, 2, 2, 1])
    ...
    with _ctrl_inputs:
        ...
        _primary_mothers = [m for m in _mother_opts if m != "SanJulian"]
...
<div class="summary-card">
  ...
  <ul>
    ...
    st.plotly_chart(chart_mothers(tl_df, EXPORT_TRIG, MOTHER_CAP_BY_NAME), width='stretch')
    ...
    _pt_a_storages = [s for s, pt in _sp_map_jmp.items() if pt in ("A", "E")]
    # Build per-vessel permitted storage list (respecting POINT_A_ONLY etc.)
    _pt_ae_vessels = sorted(
        v for v in (S.get("vessel_names", []) or vnames)
        if v != "SanJulian"
        and any(_sp_map_jmp.get(s) in ("A", "E") for s in _pt_a_storages)
    )
    # Vessel → list of storages it is allowed to load from (Point A/E only)
    _pt_ao  = set(getattr(mod, "POINT_A_ONLY_VESSELS",    {"Woodstock", "Berners"}))
    _st_perm = {
        "Westmore": set(getattr(mod, "WESTMORE_PERMITTED_VESSELS", set())),
        "Duke":     set(getattr(mod, "DUKE_PERMITTED_VESSELS",     set())),
        "Starturn": set(getattr(mod, "STARTURN_PERMITTED_VESSELS", set())),
    }
    def _allowed_ae_storages(vessel):
        """Storages at Point A or E that this vessel is permitted to load."""
        result = []
        for _s, _pt in _sp_map_jmp.items():
            if _pt not in ("A", "E"):
                continue
            if vessel in _pt_ao and _pt != "A":
                continue  # Point-A-only vessel
            if _s in _st_perm and vessel not in _st_perm[_s]:
                continue  # vessel not permitted at this specific storage
            result.append(_s)
    ...
    with st.expander(
        ...
        st.markdown(
            ...
            'Force a Point A/E vessel to a specific loading storage on a given day. '
            'The simulation re-runs with the override applied so all downstream '
            'timings update accurately. Only storages that the vessel is permitted '
            'to load from are shown.</div>',
            unsafe_allow_html=True,
        )

        # ── Add a new override ────────────────────────────────────────────────
        _ov_c1, _ov_c2, _ov_c3, _ov_c4 = st.columns([2, 1, 2, 1])
        with _ov_c1:
            _ov_vessel = st.selectbox(
                "Vessel", options=_pt_ae_vessels,
                key="ov_vessel",
                help="Only Point A/E vessels are available for reallocation.",
            )
        with _ov_c2:
            _ov_day = st.number_input(
                ...
                value=1, step=1,
                key="ov_day",
                help="Simulation day number (Day 1 = first day of the run).",
            )
        ...
        with _ov_c3:
            _ov_storage = st.selectbox(
                ...
                "Reassign to storage", options=[], key="ov_storage",
            )
        with _ov_c4:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("➕ Add", key="ov_add_btn", use_container_width=True):
                if _ov_vessel and _ov_storage:
                    ...
                    _upd.setdefault(_ov_vessel, {})[str(int(_ov_day))] = _ov_storage
                    st.session_state["jmp_storage_overrides"] = _upd
                    # Bust the JMP HTML cache so it rebuilds with the new override
        ...
        if _existing_overrides:
            ...
            for _ov_v, _ov_days in sorted(_existing_overrides.items()):
                for _ov_d, _ov_s in sorted(_ov_days.items(), key=lambda x: int(x[0])):
                    ...
                    _ov_rows.append((_ov_v, _ov_d, _ov_s, _sim_date))
            for _r_idx, (_r_v, _r_d, _r_s, _r_date) in enumerate(_ov_rows):
                ...
                with _rc1:
                    st.markdown(
                        ...
                        f'<span style="font-size:12px;color:#374151"> Day {_r_d} ({_r_date})'
                        f' → <b>{_r_s}</b></span>',
                        unsafe_allow_html=True,
                    )
                with _rc2:
...
<style>
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    _html.append(
        ...
        '<th colspan="6" class="sec-hdr-cell">Opening Stock (bbl)</th>'
    ...
    _html.append(
        '<tr>'
        # Stock
        '<th>Chapel</th><th>JasmineS</th><th>Westmore</th><th>Duke</th><th>Starturn</th><th>Ibom</th>'
        ...
        # BIA arrival
        '<th>Vessel (ETA)</th>'
        # Discharge per mother — now 4 columns
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _lcell(storage):
            ...
            for r in entries:
                _is_forced = (
                    str(_day) in _active_overrides.get(r["Vessel"], {})
                    and _active_overrides[r["Vessel"]][str(_day)] == storage
                )
                _label = (
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _fload(name):
            ...
            for r in entries:
                _f_forced = (
                    str(_fd) in _factive_ov.get(r["Vessel"], {})
                    and _factive_ov[r["Vessel"]][str(_fd)] == name
                )
                _f_label = (
                    f"{r['Vessel']} | {r['Time'][11:16]} | {_kkk(_parse_cargo(r['Detail']))}"
                    + (" 🔒" if _f_forced else "")
