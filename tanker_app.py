        ...
        for _vr in _vr_list:
            try:
                ...
                if _vr_indef or _vr_date == "indefinite":
                    ...
                    # For mid-sim dormancy (_dorm_start_h > 0), we do NOT call
        ...
        for v in sim.vessels:
            if v.name in _resumption_seed_by_vessel:
                ...
                if _seed_indef:
                    ...
                    # Store both the dormancy window and the resumption hour on the
                    # vessel so the run loop can enforce it when t crosses
                    # dormancy_start_hour.
                    ...
                    # Compute and store the resumption sim-hour so the trigger
                    # can activate it at dormancy_start_hour.
                    try:
                        _r_epoch = getattr(mod, "_SIM_EPOCH", None)
                        _r_date  = _dt.date.fromisoformat(_vr_date)
                        if _r_epoch:
                    ...
                    # dormant when t reaches dormancy_start_hour.
            if v.name in vs:
                d = vs[v.name]
                if d.get("status"):
                    v.status = d["status"]
