...
=============================================================
  ...
  Tab 1  "volumes"   — one row per daily 8am update
    Columns: timestamp | chapel_bbl | jasmines_bbl | westmore_bbl |
             ...
             greeneagle_bbl | sanjulian_bbl | production_bph | sim_days
...
VESSEL_COLORS = {
    ...
    "SanJulian": "#000000",   # deep black    — distinct from Bedford amber
...
LOCATION_CATALOGUE = [
    ...
    {"display": "Sailing → MT SanBarth (A/C outbound)", "sim_value": "En Route SanBarth→BIA",
     "field_zone": "Transit", "target_mother": "SanJulian", "target_storage": None,
     "statuses": [
        ("SAILING_AB",          "🚢 Leg 1: Point A/C → Breakwater (1.5h)"),
        ("SAILING_CROSS_BW_AC", "🚢 Leg 2: Crossing Breakwater outbound (0.5h)"),
        ("SAILING_BW_TO_FWY",   "🚢 Leg 3: Breakwater → Fairway Buoy (2h)"),
    ...
    {"display": "MT SanBarth (BIA)",     "sim_value": "SanJulian",
     "field_zone": "BIA",
     "statuses": [
        ("DISCHARGING",            "⬇️ Discharge — in progress"),
        ("HOSE_CONNECT_B",         "🔧 Hose connection underway"),
    ...
    {"display": "Cawthorne Channel → BIA via MT SanBarth", "sim_value": "Cawthorne Channel (outbound)",
     "field_zone": "Transit", "target_storage": "Duke", "target_mother": "SanJulian",
     "statuses": [
        ("SAILING_D_CHANNEL",    "🚢 Point D → Cawthorne Channel (3h, tidal)"),
        ("SAILING_CH_TO_BW_OUT", "🚢 Channel → Breakwater (1h, tidal)"),
        ("SAILING_CROSS_BW_OUT", "🚢 Crossing Breakwater outbound (0.5h, tidal)"),
...
def run_sim(sim_days, chapel, jasmines, westmore, duke, starturn, pgm,
    ...
    mother_unavailability_json: JSON list of {mother, start_date, end_date} —
        ...
        SanJulian transloads are rerouted to other mothers during each window.
    """
    ...
    if vessel_resumption_json and hasattr(mod, "set_vessel_resumption"):
        ...
        for _vr in _vr_list:
            try:
                ...
                _vr_date      = str(_vr.get("date", "")).strip()         # resumption (end) date
    ...
    _OPT_KEYS = [
        ...
        ("SANJULIAN_OPTIM_THRESHOLD_FRAC", "sj_optim_threshold"),
    ...
    sim.mother_bbl["SanJulian"]  = min(sanjulian,  int(_mother_caps.get("SanJulian", _default_cap)))
    ...
    if sanjulian  > 0 and sanjulian_api  > 0:
        sim.mother_api["SanJulian"]  = float(sanjulian_api)
    ...
    for v in sim.vessels:
        ...
        # SanJulian starts with no daughter alongside at t=0 (berth is free)
    ...
    for _exp_mn, _exp_days in _mother_export.items():
        ...
        # blocks daughters AND SanJulian transloads from targeting this mother.
    ...
    if mother_export_force_json:
        ...
        if isinstance(_raw_force, list):
            ...
            _sj_name        = getattr(mod, "MOTHER_QUATERNARY_NAME", "SanJulian")
    ...
    summary = dict(
        ...
        vessel_names    = [v.name for v in sim.vessels] + ["SanJulian"],
        ...
        **{f"final_{k}": float(v) for k, v in sim.mother_bbl.items()},
        ...
        # SanJulian-specific metrics
        sanjulian_total_transloaded = float(getattr(sim, "sanjulian_total_transloaded", 0.0)),
        final_sanjulian_bbl         = float(getattr(sim, "final_sanjulian_bbl",
                                            sim.mother_bbl.get("SanJulian", 0.0))),
        final_sanjulian_api         = round(float(getattr(sim, "final_sanjulian_api",
                                            sim.mother_api.get("SanJulian", 0.0))), 2),
    )
...
def run_optimizer(base_params_json: str):
    ...
    # SanJulian optimisation: two targeted combinations.
    ...
    _orig_sj_thr = getattr(mod, "SANJULIAN_OPTIM_THRESHOLD_FRAC", 0.25)
    ...
    def _score(S, log_df, tl_df):
        ...
        # ── SanJulian utilisation score ──────────────────────────────────
        # Reward scenarios where SanJulian transloads frequently (short dwell
        ...
        # Average SanJulian volume (as % of capacity) over the sim — lower is better
        sj_cap_val = getattr(mod, "SANJULIAN_CAPACITY_BBL", 440_000)
        if "SanJulian_bbl" in tl_df.columns and not tl_df.empty:
            sj_avg_pct = float(tl_df["SanJulian_bbl"].mean()) / max(1, sj_cap_val)
            # Time SanJulian spent ≥70% full (holding too long)
            sj_high_frac = float((tl_df["SanJulian_bbl"] >= 0.70 * sj_cap_val).mean())
        else:
            sj_avg_pct   = 0.0
            sj_high_frac = 0.0
        ...
        for rank, (dsf, pft, expw, (bstart, bend), (sj_thr, sj_spc), mto_parcels) in enumerate(_grid, 1):
            ...
            try:
                ...
                _log, _tl, S = run_sim(
                    ...
                    sanjulian           = base.get("sanjulian", 450_000),
    ...
    if not all_results:
        ...
        _log, _tl, S = run_sim(
            ...
            greeneagle=base.get("greeneagle",300_000), sanjulian=base.get("sanjulian",450_000),
...
def chart_mothers(tl_df, export_trigger, cap_by_name, export_trigger_by_name=None):
    fills = {"Bryanston" :"rgba(26,188,156,0.12)",
             ...
             "SanJulian" :"rgba(243,156,18,0.12)"}
    fig = go.Figure()
    for name, col in [("Bryanston","Bryanston_bbl"),
                       ("GreenEagle","GreenEagle_bbl"),
                       ("SanJulian","SanJulian_bbl")]:
        if col in tl_df.columns:
            _dash = "dash" if name == "SanJulian" else "solid"
            fig.add_trace(go.Scatter(x=tl_df.Time, y=tl_df[col], name=name,
    ...
    _cap_sj  = int(cap_by_name.get("SanJulian", _cap_bry))
...
def chart_gantt(tl_df, vessel_names, log_df=None):
    ...
    SanJulian is rendered as an additional row below the daughter vessels,
    ...
    # SanJulian transload statuses — share the "At BIA" category so the bar
    ...
    def _sj_cat(st):
        """Category for SanJulian status column."""
    ...
    # SanJulian occupies y=0, daughter vessels above it
    _sj_has_col = "SanJulian" in tl_df.columns
    _sj_offset  = 1 if _sj_has_col else 0

    vessels_ordered = list(reversed(vessel_names))   # top vessel = first in list
    # y positions: daughter vessels start at _sj_offset so SanJulian sits at bottom
    ...
    # ── SanJulian row — transload activity at y=0 ────────────────────────────
    if _sj_has_col:
        _sj_col   = "#000000"  # deep black — matches MOTHER_COLORS["SanJulian"]
        _sj_idle  = "#2d3748"  # dark slate for idle periods
        sub_sj = tl_df[["Day", "Time", "SanJulian"]].dropna(subset=["SanJulian"]).copy()
        if not sub_sj.empty:
            sub_sj["xf"]  = (sub_sj["Day"] - 1) + sub_sj["Time"].apply(
                lambda d: (d.hour + d.minute / 60) / 24
            )
            sub_sj["cat"] = sub_sj["SanJulian"].apply(_sj_cat)
            sub_sj["blk"] = (sub_sj["cat"] != sub_sj["cat"].shift()).cumsum()

            for (cat, _blk), grp in sub_sj.groupby(["cat", "blk"], sort=False):
                if cat == "Idle":
                    continue   # don't clutter chart with SanJulian idle spans
                x0  = float(grp["xf"].iloc[0])
                x1  = float(grp["xf"].iloc[-1]) + (1.0 / SLOTS_PER_DAY)
                dur = x1 - x0
                rep_st  = grp["SanJulian"].mode().iloc[0]
                lbl     = STATUS_LABELS.get(rep_st, rep_st)
                d0, d1  = int(grp["Day"].iloc[0]), int(grp["Day"].iloc[-1])
                day_lbl = f"Day {d0}" if d0 == d1 else f"Day {d0}–{d1}"
    ...
    # All rows: daughter vessels + SanJulian at bottom
    _all_rows_ordered = vessels_ordered + (["SanJulian"] if _sj_has_col else [])
...
def render_mother_cards(gs_vols, manual_mother, mod):
    ...
    for i, (mn, mk) in enumerate([
        ...
        ("GreenEagle","greeneagle"), ("SanJulian","sanjulian")
    ]):
        ...
        _sj_cap = getattr(mod, "SANJULIAN_CAPACITY_BBL", cap)
        _sj_rate = getattr(mod, "SANJULIAN_TRANSLOAD_RATE_BPH", 12_000)

        if mn == "SanJulian":
            # SanJulian never exports — show transload status instead
...
def main():
    ...
    MOTHER_CAP_BY_NAME = getattr(mod, "MOTHER_CAPACITY_BY_NAME", {
        ...
        "GreenEagle": int(mod.MOTHER_CAPACITY_BBL),
        "SanJulian": int(getattr(mod, "SANJULIAN_CAPACITY_BBL", mod.MOTHER_CAPACITY_BBL)),
    })
    ...
    _mother_opts = list(getattr(mod, "MOTHER_NAMES", ["Bryanston", "GreenEagle", "SanJulian"]))
    ...
    _ctrl_day1, _ctrl_inputs, _ctrl_data, _ctrl_help = st.tabs([
        "Day 1 Startup",
        "Scenario Inputs",
        "Data & Constraints",
        "Reference & Help",
    ...
    with _ctrl_inputs:
        ...
        st.caption(
            ...
            "During a mother's window she cannot receive daughters or SanJulian transloads — "
    ...
    with _ctrl_data:
        ...
        # ======================================================================
        st.markdown("### 📄 Load from Daily Stock Report (PDF)")
        st.caption(
            "Upload the Daily Stock Update PDF to automatically extract storage volumes, "
            "mother vessel stocks, and daughter vessel positions. "
            "Review and adjust the extracted data before confirming — it will pre-populate "
            "all startup fields for the simulation."
        )

        _IMPORTER_SYSTEM_PROMPT = """You are extracting data from a Daily Stock Update PDF for a tanker simulation.

The document has 3 pages:
Page 1 (Production): Shows storage vessel stock levels with columns: Vessel name, Capacity, Prev Prediction (TOV), Variance, Actual/Opening stock, production sources (volumes), Scheduled Loading (volumes out to shuttle vessels), Next day prediction. Vessel names: MT WESTMORE (=Westmore), MT JASMINE S (=JasmineS), MT CHAPEL (=Chapel), MT DUKE (=Duke), IBOM (stock of Ibom offshore field — often 0 or small), ASARAMATORU (ignore — not a storage vessel for the sim), Barge Starturn (=Starturn). The comment column next to IBOM indicates which daughter vessel is currently loading at the offshore buoy (e.g. "Bedford is loading").

Page 2 (Loading Operations): Shows shuttle/daughter vessel status with columns: Shuttle Vessel, Capacity, Predicted ROB, Variance, Actual ROB@ 07:00hrs (TOV), Comment, then coloured columns showing which storage they loaded from, then coloured columns showing which mother vessel they are discharging to (Discharge @ BIA). Mother vessels at BIA: MT Bryanston (=Bryanston), MT Green Eagle (=GreenEagle), MT San Barth (=SanJulian — this is the sim internal name). Also shows Predicted ROB for next day.

Page 3 (Summary): Confirms total volumes and has "Today's Vessel Schedule Summary" table: Loading Plan (daughter vessel, storage, status: "Loading" or "To load"), Transit to BIA (vessel, mother allocation, ETA), Discharging Plan (daughter, mother, status), Return to Load (daughter, storage, ETA).

NAME MAPPINGS (use these exact names in output):
Storage: "MT CHAPEL"→"Chapel", "MT JASMINE S"/"Jasmine S"→"JasmineS", "MT WESTMORE"→"Westmore", "MT DUKE"→"Duke", "Barge Starturn"→"Starturn"
Mother: "MT BRYANSTON"/"Bryanston"→"Bryanston", "MT GREEN EAGLE"/"Green Eagle"→"GreenEagle", "MT SAN BARTH"/"Sanbarth"→"SanJulian"
Daughter: "MT SHERLOCK"→"Sherlock", "MT LAPHROAIG"→"Laphroaig", "MT WATSON"→"Watson", "MT BEDFORD"→"Bedford", "MT BALHAM"→"Balham", "MT WOODSTOCK"→"Woodstock", "MT BAGSHOT"→"Bagshot", "MT RATHBONE"→"Rathbone", "MT SANTA MONICA"→"SantaMonica"

STATUS CODE MAPPING for daughter vessels:
- "Loading" / "Cargo loading is still ongoing" at a storage → "LOADING"
- "To load" / "Scheduled to load" → "IDLE_A"
- "Cargo discharge is still ongoing" at a mother vessel → "DISCHARGING"
- "Scheduled to discharge" → "WAITING_BERTH_B"
- Currently alongside / moored at mother → "DISCHARGING" or "IDLE_B"
- "En route inbound to BIA" / "sail inbound" / "sailing to BIA" → "SAILING_AB_LEG2"
- "Weighing anchor" / "outbound from storage" → "SAILING_AB"
- "Returning" / "sail outbound from BIA" → "SAILING_BA"
- "En route Awoba/Duke" via Cawthorne → "SAILING_D_CHANNEL"
- "Holding at fairway buoy" → "WAITING_FAIRWAY"
- "At BIA" / "Drop anchor at BIA" → "WAITING_RETURN_STOCK"
- Currently loading at Ibom offshore buoy (from IBOM row comment on page 1) → "PF_LOADING"

NOTE: The vessel loading from Ibom (Bedford or Balham) is identified from the IBOM row comments on page 1. That vessel gets status "PF_LOADING" and assigned_storage "Ibom". Its Actual ROB on page 2 will often show 0 (still loading).

For daughter vessels in page 2: the "Actual ROB @ 07:00hrs (TOV)" column IS the cargo on board in bbls.
For storage vessels in page 1: the "Actual" column (which is the opening stock for the current day) IS the stock in bbls to use.
For mother vessels in page 2: their "Actual ROB" column shows their current stock.

Respond ONLY with valid JSON, no preamble, no markdown fences:
{
  "report_date": "YYYY-MM-DD",
  "storage_volumes": {"Chapel": 0, "JasmineS": 0, "Westmore": 0, "Duke": 0, "Starturn": 0},
  "mother_volumes": {"Bryanston": 0, "GreenEagle": 0, "SanJulian": 0},
  "daughter_vessels": [
    {"name": "Sherlock", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Laphroaig", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Watson", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Bedford", "cargo_bbl": 0, "status": "PF_LOADING", "assigned_storage": "Ibom", "target_mother": "", "notes": ""},
    {"name": "Balham", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Woodstock", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Bagshot", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "Rathbone", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""},
    {"name": "SantaMonica", "cargo_bbl": 0, "status": "IDLE_A", "assigned_storage": "", "target_mother": "", "notes": ""}
  ],
  "ibom_loading_vessel": "Bedford",
  "extraction_notes": ""
}"""

        # ── PDF upload + JSON paste (no API key required) ───────────────────────
        # The extraction runs entirely inside a small HTML component (browser-side)
        # which has automatic access to the Anthropic API — no key needed.
        # After extraction the component displays the JSON; paste it into the box below.

        import streamlit.components.v1 as _components

        _EXTRACT_SYSTEM = """You are extracting data from a Daily Stock Update PDF for a tanker simulation.

The document has 3 pages:
Page 1 (Production): Storage vessel stock levels. Vessel names: MT WESTMORE→Westmore, MT JASMINE S→JasmineS, MT CHAPEL→Chapel, MT DUKE→Duke, Barge Starturn→Starturn, ASARAMATORU (ignore). The IBOM row comment indicates which daughter vessel is currently loading at the offshore buoy (e.g. "Bedford is loading").
Page 2 (Loading Operations): Daughter vessel actual ROB (cargo bbls at 07:00), which storage loaded from (coloured column), which mother discharging to at BIA. Mothers: MT Bryanston→Bryanston, MT Green Eagle→GreenEagle, MT San Barth→SanJulian.
Page 3 (Summary): Today's Vessel Schedule Summary with Loading Plan, Transit to BIA, Discharging Plan, Return to Load tables.

NAME MAPPINGS (output these exact keys):
Daughter: MT SHERLOCK→Sherlock, MT LAPHROAIG→Laphroaig, MT WATSON→Watson, MT BEDFORD→Bedford, MT BALHAM→Balham, MT WOODSTOCK→Woodstock, MT BAGSHOT→Bagshot, MT RATHBONE→Rathbone, MT SANTA MONICA→SantaMonica
STATUS CODES: Loading/ongoing at storage→LOADING, To load/nominated→IDLE_A, Discharge ongoing at mother→DISCHARGING, Scheduled to discharge→WAITING_BERTH_B, En route inbound BIA→SAILING_AB_LEG2, Outbound from storage→SAILING_AB, Returning/outbound BIA→SAILING_BA, Cawthorne Channel route→SAILING_D_CHANNEL, At fairway buoy→WAITING_FAIRWAY, At BIA anchor→WAITING_RETURN_STOCK, Ibom buoy loading→PF_LOADING
NOTE: Ibom loading vessel is from IBOM row comment on page 1. Its page 2 ROB may show 0.
Respond ONLY with JSON (no markdown, no fences):
{"report_date":"YYYY-MM-DD","storage_volumes":{"Chapel":0,"JasmineS":0,"Westmore":0,"Duke":0,"Starturn":0},"mother_volumes":{"Bryanston":0,"GreenEagle":0,"SanJulian":0},"daughter_vessels":[{"name":"Sherlock","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Laphroaig","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Watson","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Bedford","cargo_bbl":0,"status":"PF_LOADING","assigned_storage":"Ibom","target_mother":"","notes":""},{"name":"Balham","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Woodstock","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Bagshot","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"Rathbone","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""},{"name":"SantaMonica","cargo_bbl":0,"status":"IDLE_A","assigned_storage":"","target_mother":"","notes":""}],"ibom_loading_vessel":"Bedford","extraction_notes":""}"""

        _EXTRACT_HTML = f"""
<!DOCTYPE html><html><head><meta charset="utf-8">
<style>
  *{{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}}
  body{{background:#f8fafc;padding:16px;font-size:13px}}
  .card{{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:16px;margin-bottom:12px}}
  .title{{font-weight:700;font-size:15px;color:#1e293b;margin-bottom:4px}}
  .sub{{font-size:12px;color:#64748b;margin-bottom:12px}}
  .drop{{border:2px dashed #cbd5e1;border-radius:8px;padding:24px;text-align:center;cursor:pointer;transition:.15s;background:#f8fafc}}
  .drop:hover,.drop.over{{border-color:#3b82f6;background:#eff6ff}}
  .drop p{{color:#64748b;margin-top:6px;font-size:12px}}
  .drop .icon{{font-size:28px}}
  .btn{{width:100%;padding:9px;border-radius:7px;border:none;background:#2563eb;color:#fff;font-size:13px;font-weight:600;cursor:pointer;margin-top:8px}}
  .btn:disabled{{background:#94a3b8;cursor:not-allowed}}
  .btn.secondary{{background:#f1f5f9;color:#475569;border:1px solid #cbd5e1}}
  .status{{padding:10px 14px;border-radius:7px;font-size:12px;margin:8px 0;display:flex;align-items:center;gap:8px}}
  ...
  textarea{{width:100%;height:140px;font-family:monospace;font-size:11px;border:1px solid #e2e8f0;border-radius:6px;padding:8px;resize:vertical;color:#334155;background:#f8fafc}}
  ...
  .pill{{display:inline-block;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:600;background:#dbeafe;color:#1e40af;margin-right:4px}}
  input[type=file]{{display:none}}
</style></head><body>

<div class="card">
  <div class="title">📄 Stock Report Extractor</div>
  <div class="sub">Upload the Daily Stock Update PDF — data is extracted automatically, no API key needed.</div>
  ...
  <div class="drop" id="dropzone" onclick="document.getElementById('fileInput').click()"
    ...
    <div class="icon">📄</div>
    <div style="font-weight:600;margin-top:8px;color:#334155">Drop PDF here or click to browse</div>
    <p>Daily Stock Update PDF · all 3 pages</p>
  </div>
  <input type="file" id="fileInput" accept=".pdf" onchange="handleFile(this.files[0])">

  <div id="status" style="display:none"></div>
  <div id="fileInfo" style="display:none;font-size:11px;color:#64748b;margin-top:6px"></div>
  <button class="btn" id="extractBtn" onclick="doExtract()" disabled>⚡ Extract Data</button>
</div>

<div class="card" id="resultCard" style="display:none">
  <div class="title">✅ Extracted JSON</div>
  <div class="sub">Copy everything below and paste it into the box in the app.</div>
  <textarea id="resultJson" readonly onclick="this.select()"></textarea>
  <button class="btn secondary" style="margin-top:6px" onclick="copyJSON()">📋 Copy to clipboard</button>
  <div id="copyOk" style="display:none;color:#166534;font-size:11px;margin-top:4px">✓ Copied!</div>
</div>

<script>
const SYSTEM = {json.dumps(_EXTRACT_SYSTEM)};
let pdfB64 = null;
...
function handleFile(file) {{
  if (!file || !file.name.toLowerCase().endsWith('.pdf')) {{
    showStatus('Only PDF files are supported.', 'err'); return;
  }}
  ...
  reader.onload = e => {{
    ...
    showStatus('PDF loaded — press Extract Data.', 'ok');
  }};
...
async function doExtract() {{
  ...
  showStatus('<span class="spin"></span> Extracting with AI — takes ~20 seconds…', 'loading');
  ...
  try {{
    const resp = await fetch('https://api.anthropic.com/v1/messages', {{
      ...
      body: JSON.stringify({{
        ...
        messages: [{{role:'user', content:[
          ...
          {{type:'text', text:'Extract all startup data from this Daily Stock Update report exactly as specified.'}}
        ]}}]
      }})
    }});
    ...
    document.getElementById('resultJson').value = JSON.stringify(parsed, null, 2);
    document.getElementById('resultCard').style.display = 'block';
    showStatus('✅ Extraction complete — copy the JSON and paste it into the app below.', 'ok');
  }} catch(e) {{
...
function copyJSON() {{
  const ta = document.getElementById('resultJson');
  ta.select();
...
function showStatus(msg, type) {{
  ...
  el.style.display = 'flex';
        ...
        _components.html(_EXTRACT_HTML, height=520, scrolling=False)

        st.markdown("**Paste extracted JSON here:**")
        _json_paste = st.text_area(
            "Paste JSON",
            value=st.session_state.get("_pdf_json_paste", ""),
            height=120,
            key="pdf_json_paste_area",
            placeholder='{"report_date": "2026-05-04", "storage_volumes": {...}, ...}',
            label_visibility="collapsed",
            help="Paste the JSON produced by the extractor above.",
        )

        _parse_col, _clear_col2 = st.columns([3, 1])
        with _parse_col:
            if st.button("📥 Load extracted data", key="pdf_parse_btn",
                         type="primary", use_container_width=True,
                         disabled=not _json_paste.strip()):
                try:
                    _clean = _json_paste.strip()
                    if _clean.startswith("```"): _clean = "\n".join(_clean.split("\n")[1:])
                    if _clean.endswith("```"):   _clean = "\n".join(_clean.split("\n")[:-1])
                    _extracted = json.loads(_clean)
                    st.session_state["_pdf_extracted"]     = _extracted
                    st.session_state["_pdf_json_paste"]    = _json_paste
                    st.session_state["_pdf_import_status"] = {{
                        "type": "ok",
                        "msg": (
                            f"✅ JSON loaded — report dated {{_extracted.get('report_date','?')}}. "
                            "Review and adjust below, then press Confirm & Apply."
                        ),
                    }}
                    st.rerun()
                    ...
                    st.session_state["_pdf_import_status"] = {{
                        "type": "error",
                        "msg": f"Could not parse JSON: {{_pe}}. Make sure you copied the full output.",
                    }}
                    st.rerun()

        with _clear_col2:
            if st.button("🗑️ Clear", key="pdf_clear_paste_btn", use_container_width=True):
                for _k in ["_pdf_extracted", "_pdf_json_paste", "_pdf_import_status",
                           "_pdf_apply_summary",
            ...
            with st.expander("📝 Review & Adjust Extracted Data", expanded=True):
                ...
                with _ed_c2:
                    if _ex.get("extraction_notes"):
                        st.caption(f"ℹ️ AI notes: {_ex['extraction_notes']}")

                st.markdown("**Storage vessel opening stocks (bbl TOV)**")
                _sv_cols = st.columns(5)
                _sv_names = ["Chapel", "JasmineS", "Westmore", "Duke", "Starturn"]
                for _svi, _svn in enumerate(_sv_names):
                    with _sv_cols[_svi]:
                        _ex["storage_volumes"][_svn] = st.number_input(
                            _svn, 0, 800_000,
                            int(_ex.get("storage_volumes", {}).get(_svn, 0)),
                            step=5_000, key=f"pdf_sv_{_svn}",
                        )

                st.markdown("**Mother vessel stocks (bbl TOV)**")
                _mv_cols = st.columns(3)
                _mv_pairs = [("Bryanston", "Bryanston"), ("GreenEagle", "GreenEagle"), ("SanJulian", "MT SanBarth")]
                for _mvi, (_mvk, _mvl) in enumerate(_mv_pairs):
                    with _mv_cols[_mvi]:
                        _ex["mother_volumes"][_mvk] = st.number_input(
                            _mvl, 0, 800_000,
                            int(_ex.get("mother_volumes", {}).get(_mvk, 0)),
                            step=10_000, key=f"pdf_mv_{_mvk}",
                        )

                st.markdown("**Daughter vessel positions**")
                _dv_hdr = st.columns([2, 2, 3, 2, 2])
                for _dh, _dl in zip(_dv_hdr, ["Vessel", "Cargo (bbl)", "Status", "At storage", "Target mother"]):
                    _dh.markdown(
                        f'<div style="font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase">{_dl}</div>',
                        unsafe_allow_html=True
                    )

                _ALL_STATUS_OPTS = [
                    "LOADING","IDLE_A","PF_LOADING","PF_SWAP",
                    "SAILING_AB","SAILING_CROSS_BW_AC","SAILING_BW_TO_FWY","SAILING_AB_LEG2",
                    "SAILING_BA","SAILING_B_TO_FWY","SAILING_FWY_TO_BW","SAILING_CROSS_BW_IN_AC","SAILING_BW_TO_A",
                    "DISCHARGING","HOSE_CONNECT_B","BERTHING_B","WAITING_BERTH_B",
                    "WAITING_MOTHER_RETURN","WAITING_MOTHER_CAPACITY","WAITING_RETURN_STOCK",
                    "CAST_OFF_B","IDLE_B","DOCUMENTING","WAITING_CAST_OFF","CAST_OFF",
                    "HOSE_CONNECT_A","BERTHING_A","WAITING_BERTH_A","WAITING_STOCK","WAITING_DEAD_STOCK",
                    "SAILING_D_CHANNEL","SAILING_CH_TO_BW_OUT","SAILING_CROSS_BW_OUT",
                    "SAILING_B_TO_BW_IN","SAILING_CROSS_BW_IN","SAILING_BW_TO_CH_IN","SAILING_CH_TO_D",
                    "WAITING_FAIRWAY","WAITING_TIDAL","WAITING_DAYLIGHT",
                ]
                _STORAGE_EDIT_OPTS = ["", "Chapel", "JasmineS", "Westmore", "Duke", "Starturn", "Ibom"]
                _MOTHER_EDIT_OPTS  = ["", "Bryanston", "GreenEagle", "SanJulian"]

                for _dvi, _dv in enumerate(_ex.get("daughter_vessels", [])):
                    _dc = st.columns([2, 2, 3, 2, 2])
                    ...
                    with _dc[1]:
                        _dv["cargo_bbl"] = st.number_input(
                            "cargo", 0, 200_000, int(_dv.get("cargo_bbl", 0)),
                            step=1_000, key=f"pdf_dv_cargo_{_dvi}",
                            label_visibility="collapsed",
                        )
                    with _dc[2]:
                        _cur_st = _dv.get("status", "IDLE_A")
                        _st_idx = _ALL_STATUS_OPTS.index(_cur_st) if _cur_st in _ALL_STATUS_OPTS else 0
                        _dv["status"] = st.selectbox(
                            "status", _ALL_STATUS_OPTS, index=_st_idx,
                            key=f"pdf_dv_status_{_dvi}",
                            label_visibility="collapsed",
                        )
                    with _dc[3]:
                        _cur_stor = _dv.get("assigned_storage", "") or ""
                        _stor_idx = _STORAGE_EDIT_OPTS.index(_cur_stor) if _cur_stor in _STORAGE_EDIT_OPTS else 0
                        _dv["assigned_storage"] = st.selectbox(
                            "storage", _STORAGE_EDIT_OPTS, index=_stor_idx,
                            key=f"pdf_dv_stor_{_dvi}",
                            label_visibility="collapsed",
                        )
                    with _dc[4]:
                        _cur_mom = _dv.get("target_mother", "") or ""
                        _mom_idx = _MOTHER_EDIT_OPTS.index(_cur_mom) if _cur_mom in _MOTHER_EDIT_OPTS else 0
                        _dv["target_mother"] = st.selectbox(
                            "mother", _MOTHER_EDIT_OPTS, index=_mom_idx,
                            key=f"pdf_dv_mom_{_dvi}",
                            label_visibility="collapsed",
                        )
                ...
                with _confirm_col:
                    if st.button(
                        ...
                        for _mvk in ["Bryanston", "GreenEagle", "SanJulian"]:
                        ...
                        for _dv in _ex.get("daughter_vessels", []):
                            ...
                            # Derive location (sim_value) from status/storage
                            if _vstatus in {"LOADING", "IDLE_A", "HOSE_CONNECT_A", "BERTHING_A",
                        ...
                        st.session_state["vp_mother_vols"]   = {
                            ...
                            "sanjulian":  int(_ex.get("mother_volumes", {}).get("SanJulian", 0)),
                        }
                        st.session_state["vp_mother_apis"]  = {
                            "bryanston": 33.0, "greeneagle": 38.0, "sanjulian": 33.0,
                        }
        ...
        _data_left, _data_right = st.columns([3, 2])
        with _data_left:
            ...
            _sheet_id_raw = st.text_input(
                ...
                help=(
                    ...
                    "• Or just the ID portion: ABC123...\n\n"
                    "The app will extract the ID automatically from a full URL."
                ),
            )
            # Auto-extract ID from a full Google Sheets URL if the user pastes one.
            # URL format: https://docs.google.com/spreadsheets/d/<ID>/edit?...
            import re as _re
            _url_match = _re.search(
                r"docs\.google\.com/spreadsheets/d/([a-zA-Z0-9_-]+)", _sheet_id_raw or ""
            )
            sheet_id = _url_match.group(1) if _url_match else (_sheet_id_raw or "").strip()
            if _url_match and _sheet_id_raw.strip():
                st.caption(f"✅ Sheet ID extracted: `{sheet_id}`")
    ...
    with _ctrl_help:
        with st.expander("📋 Full sheet schema", expanded=False):
            st.markdown("""
...
""")
    ...
    with st.expander("🚀 Deploy as public URL", expanded=False):
            st.markdown("""
1. Push `tanker_app.py` + `tanker_simulation_v5.py` + `requirements.txt` to GitHub
...
""")
    ...
    if use_gs and sheet_id and creds_f:
        # Cache JSON across reruns (Streamlit file uploader stream empties after first read)
        _raw = creds_f.read()
        if _raw:
            st.session_state["_gs_creds_json"] = _raw.decode("utf-8")
            st.session_state["_gs_sheet_id"]   = sheet_id
        creds_json  = st.session_state.get("_gs_creds_json")
        _cached_sid = st.session_state.get("_gs_sheet_id", sheet_id)

        # ── Step-by-step diagnostic panel ────────────────────────────────────
        st.markdown("---")
        st.markdown("#### 🔍 Google Sheets Connection Diagnostics")
        st.caption("Each step must show ✅ before data can be read. "
                   "The first ❌ tells you exactly what to fix.")

        _ok = True  # gates each subsequent step

        # Step 1 — JSON readable
        _step1 = st.empty()
        if not creds_json:
            _step1.error("❌ **Step 1 — Read JSON:** File could not be read. "
                         "Please re-upload the JSON file.", icon="❌")
            _ok = False
        else:
            _step1.success(f"✅ **Step 1 — Read JSON:** {len(creds_json):,} bytes read.")

        # Step 2 — Valid JSON + service account fields
        _step2 = st.empty()
        _creds_obj = None
        if _ok:
            try:
                _creds_obj = json.loads(creds_json)
                _missing = [f for f in ("type","client_email","private_key","project_id")
                            if f not in _creds_obj]
                if _missing:
                    _step2.error(
                        f"❌ **Step 2 — Validate JSON:** Missing required fields: "
                        f"`{'`, `'.join(_missing)}`.  \n\n"
                        "This does not look like a Google service account key. "
                        "Go to **Google Cloud Console → IAM & Admin → Service Accounts "
                        "→ [your account] → Keys → Add Key → JSON** and download a fresh key.",
                        icon="❌")
                    _ok = False
                elif _creds_obj.get("type") != "service_account":
                    _step2.error(
                        f"❌ **Step 2 — Validate JSON:** `type` is "
                        f"`{_creds_obj.get('type')}` — expected `service_account`. "
                    ...
                    _step2.success(
                        f"✅ **Step 2 — Validate JSON:** Service account key for "
                        f"`{_creds_obj.get('client_email')}`  \n"
                        f"Project: `{_creds_obj.get('project_id')}`")
            except json.JSONDecodeError as _je:
                _step2.error(f"❌ **Step 2 — Validate JSON:** File is not valid JSON: {_je}",
                             icon="❌")
                _ok = False

        # Step 3 — Build credentials object
        _step3 = st.empty()
        _gc = None
        if _ok:
            try:
                ...
                _method = ""
                # Method 1: service_account_from_dict (gspread 5+) — preferred
                if hasattr(gspread, "service_account_from_dict"):
                    _gc = gspread.service_account_from_dict(_creds_obj, scopes=_scopes)
                    _method = "service_account_from_dict"
                else:
                    # Method 2: Client + login() (gspread 6 without the helper)
                    _creds = Credentials.from_service_account_info(_creds_obj, scopes=_scopes)
                    try:
                        _gc = gspread.Client(auth=_creds)
                        _gc.login()
                        _method = "Client + login()"
                    except (TypeError, AttributeError):
                        # Method 3: legacy authorize (gspread < 5)
                        _gc = gspread.authorize(_creds)
                        _method = "authorize (legacy)"
                _step3.success(
                    f"✅ **Step 3 — Build credentials:** gspread client created "
                    f"(method: `{_method}`, version: `{gspread.__version__}`).")
            except ImportError:
                _step3.error(
                    "❌ **Step 3 — Build credentials:** `gspread` or `google-auth` not installed.  \n"
                    "Run: `pip install gspread google-auth` in your deployment environment.",
                    icon="❌")
                _ok = False
            except Exception as _e3:
                _step3.error(
                    f"❌ **Step 3 — Build credentials:** `{type(_e3).__name__}: {_e3}`  \n\n"
                    "The private key in your JSON may be malformed. "
        ...
        if _ok:
            try:
                _spreadsheet = _gc.open_by_key(_cached_sid)
                _sheet_names = [ws.title for ws in _spreadsheet.worksheets()]
                _step4.success(
                    f"✅ **Step 4 — Open spreadsheet:** Found **{len(_sheet_names)}** tabs: "
                    f"`{'`, `'.join(_sheet_names)}`")
            except Exception as _e4:
                # Get the fullest possible error message
                _e4_str = ""
                try:
                    import traceback as _tb
                    _e4_str = _tb.format_exc()
                except Exception:
                    pass
                _e4_short = str(_e4) or repr(_e4) or type(_e4).__name__
                _e4_display = _e4_short or type(_e4).__name__

                if "403" in _e4_str or "PERMISSION_DENIED" in _e4_str or "403" in _e4_short:
                    _step4.error(
                        f"❌ **Step 4 — Permission denied (403)**  \n\n"
                        "The sheet exists but the service account doesn't have access.  \n"
                        f"**Fix:** Open the Google Sheet → Share → add "
                        f"`{_creds_obj.get('client_email')}` → Editor → Send.",
                        icon="❌")
                elif ("PermissionError" in type(_e4).__name__ or
                      "PermissionError" in _e4_str or
                      _e4_short in ("", "PermissionError")):
                    _step4.error(
                        f"❌ **Step 4 — PermissionError (API not enabled)**  \n\n"
                        f"Error detail: `{_e4_display}`  \n\n"
                        "This usually means one or both Google APIs are **not enabled** "
                        "in your Cloud project. The app needs both:  \n\n"
                        "**1. Google Sheets API**  \n"
                        "**2. Google Drive API**  \n\n"
                        "**To enable them:**  \n"
                        "1. Go to [console.cloud.google.com](https://console.cloud.google.com)  \n"
                        f"2. Select project: `{_creds_obj.get('project_id', 'journey-management-planner')}`  \n"
                        ...
                        "*(If APIs are already enabled, also verify the sheet is shared with "
                        f"`{_creds_obj.get('client_email')}` as Editor)*",
                        icon="❌")
                elif "404" in _e4_str or "SPREADSHEET_NOT_FOUND" in _e4_str:
                    _step4.error(
                        f"❌ **Step 4 — Spreadsheet not found (404)**  \n\n"
                        f"Sheet ID used: `{_cached_sid}`  \n"
                        "Check the URL — the ID is between `/d/` and `/edit`.",
                        icon="❌")
                else:
                    _step4.error(
                        f"❌ **Step 4 — Open spreadsheet failed**  \n\n"
                        f"`{type(_e4).__name__}: {_e4_display}`  \n\n"
                        f"Full traceback:  \n```\n{_e4_str[:800]}\n```",
                        icon="❌")
                _ok = False

        # Step 5 — Read volumes tab
        _step5 = st.empty()
        if _ok:
            try:
                _ws_v = _spreadsheet.worksheet("volumes")
                _raw_v = _ws_v.get_all_values()
                _n_rows = len([r for r in _raw_v if any(v.strip() for v in r)])
                _step5.success(
                    f"✅ **Step 5 — Read volumes tab:** {_n_rows} non-empty rows found.")
                # Actually parse
                gs_vols = gs_load_volumes(_cached_sid, creds_json)
                if gs_vols:
                    _ts = gs_vols.pop("_timestamp", "")
                    st.sidebar.success(
                        f"✅ Volumes: {len(gs_vols)} fields synced"
                        + (f" (as of {_ts})" if _ts else ""))
                    _step5.success(
                        f"✅ **Step 5 — Read volumes tab:** "
                        f"{len(gs_vols)} fields parsed successfully "
                        + (f"(as of {_ts})" if _ts else ""))
                else:
                    _step5.warning(
                        "⚠️ **Step 5 — Read volumes tab:** Tab found but no data rows parsed.  \n"
                        "Make sure you have entered at least one data row in the **volumes** tab "
                        "(row 3 onward) with numeric values in the volume columns.",
                        icon="⚠️")
            except Exception as _e5:
                _e5_str = str(_e5) or type(_e5).__name__
                if "WorksheetNotFound" in type(_e5).__name__ or "not found" in _e5_str.lower():
                    _step5.error(
                        "❌ **Step 5 — Read volumes tab:** Tab named `volumes` not found.  \n\n"
                        f"Tabs found: `{'`, `'.join([ws.title for ws in _spreadsheet.worksheets()])}`  \n\n"
                        "The tab must be named **exactly** `volumes` (lowercase, no spaces).",
                        icon="❌")
                else:
                    _step5.error(
                        f"❌ **Step 5 — Read volumes tab:** `{type(_e5).__name__}: {_e5_str}`",
                        icon="❌")

        # Step 6 — Read fleet tab
        _step6 = st.empty()
        if _ok:
            try:
                _ws_f = _spreadsheet.worksheet("fleet")
                _raw_f = _ws_f.get_all_values()
                fleet_df = gs_load_fleet(_cached_sid, creds_json)
                if not fleet_df.empty:
                    st.sidebar.success(f"✅ Fleet: {len(fleet_df)} vessel rows synced")
                    _step6.success(
                        f"✅ **Step 6 — Read fleet tab:** "
                        f"{len(fleet_df)} vessel rows parsed successfully.")
                else:
                    _step6.warning(
                        "⚠️ **Step 6 — Read fleet tab:** Tab found but no vessel rows parsed.  \n"
                        "Make sure vessel names match exactly: "
                        "Sherlock, Laphroaig, Rathbone, SantaMonica, Bedford, "
                        "Balham, Woodstock, Bagshot, Watson, Amyla.",
                        icon="⚠️")
            except Exception as _e6:
                _e6_str = str(_e6) or type(_e6).__name__
                if "WorksheetNotFound" in type(_e6).__name__ or "not found" in _e6_str.lower():
                    _step6.error(
                        "❌ **Step 6 — Read fleet tab:** Tab named `fleet` not found.  \n\n"
                        "The tab must be named **exactly** `fleet` (lowercase, no spaces).",
                        icon="❌")
                else:
                    _step6.error(
                        f"❌ **Step 6 — Read fleet tab:** `{type(_e6).__name__}: {_e6_str}`",
                        icon="❌")

    _context_chips = []
    _context_chips.append(
        ('Live Sheets' if (gs_vols or not fleet_df.empty) else 'Manual 08:00 inputs',
         'context-chip-ok' if (gs_vols or not fleet_df.empty) else 'context-chip-warn')
    )
    _context_chips.append(
        ('Day 1 manual Point B active' if startup_day_disable_point_b_priority else 'Day 1 auto-priority active',
         'context-chip-ok' if startup_day_disable_point_b_priority else 'context-chip')
    )
    _context_chips.append(
        (f'Validation seed: {len(point_b_startup_seed)} daughter(s)' if enable_point_b_startup_seed else 'Validation seed off',
         'context-chip-warn' if enable_point_b_startup_seed else 'context-chip')
    )
    _context_chips.append(
        (f'Export seeds: {len(mother_export_seed)}' if mother_export_seed else 'No export seed',
         'context-chip-warn' if mother_export_seed else 'context-chip')
    )
    _context_chips.append(
        (f'Custom vessels: {len(st.session_state.get("custom_vessels", []))}' if st.session_state.get("custom_vessels") else 'No custom vessels',
         'context-chip' if st.session_state.get("custom_vessels") else 'context-chip')
    )
    _context_chips.append(
        (f'Unavailability windows: {len(st.session_state.get("vessel_resumptions", {}))}' if st.session_state.get("vessel_resumptions") else 'No shuttle unavailability',
         'context-chip' if st.session_state.get("vessel_resumptions") else 'context-chip')
    )
    _context_chips.append(
        ('Shuttle unavailability active' if st.session_state.get("vessel_resumptions") else 'No shuttle unavailability windows',
         'context-chip-warn' if st.session_state.get("vessel_resumptions") else 'context-chip')
    )
    _context_chips.append(
        ('Tidal file loaded' if tide_file is not None else 'Daylight-only routing',
         'context-chip-ok' if tide_file is not None else 'context-chip-warn')
    )
    _context_chips.append(
        ('Optimizer on' if run_opt else 'Optimizer off',
         'context-chip-ok' if run_opt else 'context-chip')
    )
    _mto_label = (
        f'🔄 MTO: ON (max {st.session_state.get("mto_max_parcels", 1)} parcel(s))'
        if multiple_transient_operation else 'MTO: off'
    )
    _context_chips.append(
        (_mto_label,
         'context-chip-ok' if multiple_transient_operation else 'context-chip')
    )
    if '_duplicate_noms' in locals() and _duplicate_noms:
        _context_chips.append((f'Duplicate nominations: {len(_duplicate_noms)}', 'context-chip-risk'))

    st.markdown(
        '<div class="context-strip">'
        '<div class="context-title">Active Run Context</div>'
        + ''.join(f'<span class="{klass}">{label}</span>' for label, klass in _context_chips)
        + '</div>',
        unsafe_allow_html=True,
    )
    ...
    if not gs_vols:
        with st.expander("✏️ Enter 08:00 storage & mother volumes",
            ...
            mv = st.columns(4)
            # Default startup: Bryanston 450k @ 33°, GreenEagle 300k @ 38°, SanJulian 450k @ 33°
            _mother_vol_defaults = {"Bryanston": 450_000, "GreenEagle": 300_000, "SanJulian": 450_000}
            for j,mn in enumerate(["Bryanston","GreenEagle","SanJulian"]):
                with mv[j]:
                    _mother_cap_ui = int(MOTHER_CAP_BY_NAME.get(mn, mod.MOTHER_CAPACITY_BBL))
                    manual_mother[mn.lower()] = st.number_input(
                        mn, 0, _mother_cap_ui, _mother_vol_defaults.get(mn, 0), step=10_000, key=f"mv_{mn}")
            ...
            _mother_api_defaults = {"Bryanston": 33.00, "GreenEagle": 38.00, "SanJulian": 33.00}
            manual_mother_api = {}
            for _j2, _mn2 in enumerate(["Bryanston", "GreenEagle", "SanJulian"]):
    ...
    if missing_vessels:
        with st.expander(
            ...
            for vn in missing_vessels:
                ...
                if _is_bia_loc:
                    # Determine which mothers are primary (never SanJulian for export)
                    _primary_mothers = [
                        m for m in _mother_opts
                        if m != getattr(mod, "MOTHER_QUATERNARY_NAME", "SanJulian")
                    ]
                    ...
                    _show_nom = st_v in {
                        ...
                        # Nominated mother (for discharge sequencing / validation seed)
                    if _show_nom or (_bia_sim_val in _all_mothers):
                        _nom_default_idx = 0
                        ...
                        # Record nomination for Day 1 summary
                    ...
                    # Cargo API — show when vessel has cargo on board
                ...
                manual_states[vn] = {
                    ...
                    "target_mother":         _loc_entry.get("target_mother")
                                             or (_bia_nominated_mother if _is_bia_loc else None),
                    "notes":                 "",
    ...
    # Get primary mothers (exclude SanJulian)
    _exp_primary_mothers = [
        m for m in list(getattr(mod, "MOTHER_NAMES",
                                ["Bryanston", "GreenEagle", "SanJulian"]))
        if m != getattr(mod, "MOTHER_QUATERNARY_NAME", "SanJulian")
    ]
    ...
    params = dict(
        ...
        # Mother defaults: Bryanston 450k@33°, GreenEagle 300k@38°, SanJulian 450k@33°
        ...
        sanjulian = _p("sanjulian", manual_mother, "sanjulian", 450_000),
        ...
        sanjulian_api  = gs_vols.get("sanjulian_api",  manual_mother_api.get("sanjulian",  33.0)),
    )
    ...
    # ── SanJulian transload — append to discharging section if active ──────────
    # SanJulian behaves like a daughter when transloading: BERTHING_B →
    ...
    _sj_tl_st = None
    if "SanJulian" in tl_df.columns and not _d1_tl.empty:
        _sj_tl_st = str(_d1_tl.iloc[0]["SanJulian"]) if "SanJulian" in _d1_tl.columns else None
    if _sj_tl_st and _sj_tl_st not in (None, "IDLE_B", "nan"):
        _sj_log_ev = log_df[(log_df["Vessel"] == "SanJulian") &
                             (log_df["Event"] == "SJ_TRANSLOAD_START")]
        _sj_disch_ev = _d1_log[(log_df["Vessel"] == "SanJulian") &
        ...
        _discharging.append({"vessel": "SanJulian", "mother": _sj_mother,
...
Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} | Tanker Operations Simulation v5
    ...
    # Build vessel list: any non-SanJulian vessel that has at least one
    ...
    _mother_cols  = ["Bryanston_bbl","GreenEagle_bbl","SanJulian_bbl"]
    _storage_names = ["Chapel","JasmineS","Westmore","Duke","Starturn","PGM"]
    _mother_names  = ["Bryanston","GreenEagle","SanJulian"]
    ...
    for _day in range(1, _total_sim_days + 1):
        ...
        _mapi_cols = ["Bryanston_api","GreenEagle_api","SanJulian_api"]
  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _mto_dcell():
            ...
            # Group nominations by transient vessel — each unique transient gets
            # its own block so multiple MTO ops on the same day are all visible.
            import re as _re
            ...
            _dis_by_transient = {}  # transient_name -> list of discharge records
            for _dr in _mto_dis:
                # Detail contains "to {transient_name}:"
                _dt_match = _re.search(r"to\s+(\w+):", _dr.get("Detail", ""))
                _trn_key = _dt_match.group(1) if _dt_match else None
                if _trn_key and _trn_key in _seen_transients:
                    _dis_by_transient.setdefault(_trn_key, []).append(_dr)
                else:
                    # Fallback: attach to first transient if can't parse
...
<div style="font-size:9px;color:#94a3b8;margin-top:8px">Generated: {_dt.datetime.now().strftime("%Y-%m-%d %H:%M")} | Tanker Operations v5</div>
    ...
    all_moth = sum(S.get(f"final_{n}",0) for n in ["Bryanston","GreenEagle","SanJulian"])
    ...
    _all_moth_vol = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","GreenEagle","SanJulian"])
    _blended_moth = (
        sum(S.get(f"final_{n}", 0) * _mapi.get(n, 0.0)
            for n in ["Bryanston","GreenEagle","SanJulian"]) / _all_moth_vol
    ...
    _final_mothers = sum(S.get(f"final_{n}", 0) for n in ["Bryanston","GreenEagle","SanJulian"])
...
<div class="summary-card">
  ...
  <ul>
    ...
    # Determine which primary mother SanJulian is currently transloading to
    # by reading the most recent SJ_TRANSLOAD_START event from the log
    _sj_active_target = None
    _sj_active_status = None
    if not log_df.empty and "SanJulian" in tl_df.columns:
        _sj_log = log_df[log_df["Vessel"] == "SanJulian"]
        ...
        if not _sj_start.empty:
            ...
            if _last_start_idx > _last_done_idx:
                ...
                # Find current SanJulian status from last row of timeline
                _last_sj_status = (
                    tl_df["SanJulian"].dropna().iloc[-1]
                    if "SanJulian" in tl_df.columns and not tl_df["SanJulian"].dropna().empty
                    else None
                )
    ...
    for i,(mn,mk) in enumerate([("Bryanston","bryanston"),
        ...
        with mc[i]:
            ...
            if mn == "SanJulian":
                _sj_xld = S.get("sanjulian_total_transloaded", 0.0)
                kpi("MT SanBarth ⇄",
                    f"{fv:,.0f} bbl",
                ...
                # Show SanJulian active transload indicator if it is currently
    ...
    extra  = [c for c in ["Chapel_bbl","JasmineS_bbl","Westmore_bbl",
                           ...
                           "SanJulian_api"] if c in filt.columns]
    _filt_show = filt[show_c+extra].rename(columns={
        ...
        "GreenEagle_api": "GreenEagle API°", "SanJulian_api": "SanJulian API°",
    })
    ...
    # Build list of primary mothers (exclude SanJulian)
    ...
    with d3:
        ...
        for mn in ["Bryanston","GreenEagle","SanJulian"]:
