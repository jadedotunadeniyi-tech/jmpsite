  ...
  .jmp-bia-entry{display:inline-block;border-radius:4px;padding:2px 6px;
    ...
    for _day in range(_jmp_d0, _jmp_d1 + 1):
        ...
        def _mto_dcell():
            ...
            # Group nominations by transient vessel (handles multiple MTO events per day).
            # Each nomination row corresponds to one transient vessel receiving cargo.
            # Dischargers are matched by voyage context (Detail field names the transient).
            # Build a list of (nom_row, [dis_rows]) per MTO event.
            _mto_events = []
            for _nom in _mto_nom:
                _trn_name = _nom["Vessel"]
                # Find discharger rows whose Detail mentions this transient vessel
                _paired_dis = [
                    r for r in _mto_dis
                    if _trn_name in str(r.get("Detail", ""))
                ]
                # Fallback: if no Detail match, assign unmatched dischargers sequentially
                if not _paired_dis:
                    _paired_dis = [r for r in _mto_dis if r not in
                                   [x for ev in _mto_events for x in ev[1]]]
                _mto_events.append((_nom, _paired_dis))

            # If there are dischargers with no matched nomination, add a synthetic event
            _matched_dis = {id(r) for _, dl in _mto_events for r in dl}
            _unmatched_dis = [r for r in _mto_dis if id(r) not in _matched_dis]
            if _unmatched_dis and not _mto_nom:
                _mto_events.append((None, _unmatched_dis))

            # Build HTML for each MTO event block
            _blocks = []
            for _idx, (_nom, _dis_rows) in enumerate(_mto_events):
                _transient  = _nom["Vessel"] if _nom else "?"
                _tc = VESSEL_COLORS.get(_transient, "#334155")
                _time = _nom["Time"][11:16] if _nom else ""
                _trn_vcode = str(_nom.get("VoyageCode", "")).strip() if _nom else ""
                _trn_detail = str(_nom.get("Detail", "")) if _nom else ""
                _trn_onboard = ""
                try:
                    import re as _re
                ...
                # Divider between multiple MTO events on same day
                _sep = (
                    '<hr style="border:none;border-top:1px solid #bbf7d0;margin:4px 0">'
                    if _idx > 0 else ""
                )
                _blocks.append(
                    f'{_sep}'
    ...
    for _fd in range(1, _total_sim_days + 1):
        ...
        def _mto_fdcell():
            ...
            # Group nominations by transient vessel (handles multiple MTO events per day)
            _fmto_events = []
            for _fnom in _fmto_nom:
                _ftrn_name = _fnom["Vessel"]
                _fpaired_dis = [
                    r for r in _fmto_dis
                    if _ftrn_name in str(r.get("Detail", ""))
                ]
                if not _fpaired_dis:
                    _fpaired_dis = [r for r in _fmto_dis if r not in
                                    [x for ev in _fmto_events for x in ev[1]]]
                _fmto_events.append((_fnom, _fpaired_dis))
            _fmatched_dis = {id(r) for _, dl in _fmto_events for r in dl}
            _funmatched_dis = [r for r in _fmto_dis if id(r) not in _fmatched_dis]
            if _funmatched_dis and not _fmto_nom:
                _fmto_events.append((None, _funmatched_dis))

            _fblocks = []
            for _fidx, (_fnom, _fdis_rows) in enumerate(_fmto_events):
                _ftransient = _fnom["Vessel"] if _fnom else "?"
                _ftc = VESSEL_COLORS.get(_ftransient, "#334155")
                _ftime = _fnom["Time"][11:16] if _fnom else ""
                _ftrn_vcode = str(_fnom.get("VoyageCode", "")).strip() if _fnom else ""
                _ftrn_detail = str(_fnom.get("Detail", "")) if _fnom else ""
                _ftrn_onboard = ""
                try:
                    import re as _re2
