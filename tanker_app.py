    ...
    with _ctrl_data:
        ...
        if st.session_state.get("_pdf_extracted"):
            ...
            with st.expander("📝 Review & Adjust Extracted Data", expanded=True):
                ...
                # Header row — 8 columns: Vessel | Cargo | Nom.Load | Status | Storage | Target | API
                _dv_hdrs = ["Vessel","Cargo (bbl)","Nom. Load","Status","At storage","Target mother / MTO","API (°)"]
                _dv_hdr_cols = st.columns([2, 2, 2, 3, 2, 2, 1.5])
                ...
                for _dvi, _dv in enumerate(_ex.get("daughter_vessels", [])):
                    _dc = st.columns([2, 2, 2, 3, 2, 2, 1.5])
                    ...
                    with _dc[2]:
                        if _dv_st_cur in _LOADING_STATUSES:
                            ...
                            _dv["nominated_load_bbls"] = st.number_input(
                                ...
                                label_visibility="collapsed",
                                help=(
                                    "**Nominated load** — total volume this vessel should load "
                                    "before departing on startup day. Remaining = Nom − Current cargo."
                                ),
                            )
                        else:
                            st.markdown(
                    ...
                    # Col 3: Status
                    with _dc[3]:
                        _st_idx = _ALL_STATUS_OPTS.index(_dv_st_cur) if _dv_st_cur in _ALL_STATUS_OPTS else 0
                    ...
                    # Col 5: Target mother / MTO
                    with _dc[5]:
                        _is_mto_r   = _dv.get("is_mto_receiver", False)
                        _mto_tv_cur = _dv.get("mto_target_vessel", "") or ""
                        if _is_mto_r:
                            # MTO receiver — badge + allow toggling
                            _dv["is_mto_receiver"] = True
                            st.markdown(
                                '<div style="padding-top:6px;font-size:11px;color:#7c3aed;font-weight:600">📦 MTO Receiver</div>',
                                unsafe_allow_html=True)
                            _dv["target_mother"] = ""
                        elif _mto_tv_cur or _dv_st_cur in {"DISCHARGING","HOSE_CONNECT_B","BERTHING_B","WAITING_BERTH_B","CAST_OFF_B"}:
                            # Show both MTO vessel picker and mother picker
                            _mtv_idx = _MTO_VESSEL_OPTS.index(_mto_tv_cur) if _mto_tv_cur in _MTO_VESSEL_OPTS else 0
                            _new_mtv = st.selectbox(
                                "MTO→", _MTO_VESSEL_OPTS, index=_mtv_idx,
                                key=f"pdf_dv_mtotgt_{_dvi}",
                                label_visibility="collapsed",
                                help="If discharging to an MTO shuttle, select it here. Otherwise leave blank and use the mother dropdown.",
                            )
                            _dv["mto_target_vessel"] = _new_mtv
                            if not _new_mtv:
                                # No MTO — show mother dropdown
                    ...
                    # Col 6: Cargo API (for loaded/transit vessels)
                    with _dc[6]:
                        _dv_cargo_now = int(_dv.get("cargo_bbl", 0))
                        if _dv_cargo_now > 0 and _dv_st_cur in _CARGO_STATUSES:
                            ...
                            _dv["cargo_api"] = st.number_input(
                                ...
                                label_visibility="collapsed",
                                help="Cargo API gravity (°API) for blending calculations.",
                            )
                        else:
                            st.markdown(
                                '<div style="padding-top:28px;font-size:11px;color:#94a3b8">—</div>',
